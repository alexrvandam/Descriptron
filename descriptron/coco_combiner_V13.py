...
def combine_coco_json_files(input_json_files, output_json_path, skip_categories=[], images_folders=[]):
    ...
    for json_file in input_json_files:
        ...
        # Check if categories exist, if not create a default one for annotations with category_id = -1
        if not coco_data.get('categories'):
            logging.warning(f"No categories found in {json_file}. Creating default category for keypoints.")
            # Check if any annotations have keypoints to determine if we need a keypoint category
            has_keypoints = any('keypoints' in ann for ann in coco_data.get('annotations', []))
            if has_keypoints:
                # Create a default category with keypoints support
                default_category = {
                    'id': -1,
                    'name': 'keypoints',
                    'supercategory': 'none',
                    'keypoints': ['point_' + str(i) for i in range(1, 9)],  # 8 keypoints as seen in data
                    'skeleton': []  # Empty skeleton, can be filled later if needed
                }
                coco_data['categories'] = [default_category]
                logging.info(f"Created default keypoints category")
        ...
        for category in coco_data.get('categories', []):
            ...
            if cat_name not in category_name_to_new_id:
                ...
                # === new  ADD THIS TO PRESERVE ROSETTA ONTOLOGY ===
                if 'ontology' in category:
                    new_category['ontology'] = category['ontology']
                # =============================================
                # === ADD THIS TO PRESERVE LINES ===
                if 'lines' in category:
                    new_category['lines'] = category['lines']
                # ==================================
                # end new #
            ...
            # === V13 FIX: First-wins for old_category_id_to_new_id ===
            # When COCO JSONs have duplicate category entries from multiple
            # annotation rounds (e.g. id=2 → "left_elytron" then later
            # id=2 → "pronotum"), the last write silently corrupts the mapping.
            # First-wins preserves the original annotation intent.
            old_id = category['id']
            new_id = category_name_to_new_id[cat_name]
            if old_id not in old_category_id_to_new_id:
                old_category_id_to_new_id[old_id] = new_id
            elif old_category_id_to_new_id[old_id] != new_id:
                logging.warning(
                    f"Duplicate category id={old_id} in {json_file}: "
                    f"keeping first mapping (→new_id={old_category_id_to_new_id[old_id]}), "
                    f"ignoring \"{cat_name}\" (→new_id={new_id})"
                )

        # FIXED: Local mapping for images - handle both STRING and INTEGER IDs
        old_image_id_to_new_id = {}
        if coco_data.get('images'):
            for image in coco_data.get('images', []):
                old_img_id = image['id']  # Can be string or int
                ...
                logging.info(f"Mapped old image ID '{old_img_id}' (type: {type(old_img_id).__name__}) to new ID {local_new_id}")
        ...
        for annotation in coco_data.get('annotations', []):
            ...
            if new_cat_id is None:
                logging.warning(f"Annotation ID {annotation.get('id', 'unknown')} has category ID {old_cat_id} which is not mapped. Skipping.")
                continue  # Skip annotations with categories that were skipped

            old_img_id = annotation['image_id']  # Can be string or int
            # If the image was not already added, create a new image entry.
            if old_img_id not in old_image_id_to_new_id:
                local_new_id = new_image_id
                new_image_id += 1

                # Handle string IDs for file_name
                if isinstance(old_img_id, str):
                    file_name = old_img_id
                else:
                ...
                old_image_id_to_new_id[old_img_id] = local_new_id
                logging.info(f"Created new image entry for old image ID '{old_img_id}' -> new ID {local_new_id}")
            ...
            if 'keypoints' in annotation:
                ...
                logging.info(f"Preserved {len(annotation['keypoints'])} keypoint values for annotation {new_annotation_id-1}")
    ...
    # Check keypoints in output
    annotations_with_keypoints = sum(1 for ann in combined_output['annotations'] if 'keypoints' in ann)
    logging.info(f"Annotations with keypoints: {annotations_with_keypoints}")
