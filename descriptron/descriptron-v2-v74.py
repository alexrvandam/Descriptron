...
# === ONTOLOGY ROSETTA PATCH ===
# Ontology / Rosetta integration
try:
    from descriptron_ontology import OntologyManager
    _onto_mgr = OntologyManager()  # loads templates from descriptron_ontology_templates.json
    _ONTOLOGY_AVAILABLE = True
    print(f"Ontology loaded: {len(_onto_mgr.get_template_groups())} template groups")
except Exception as _onto_err:
    _ONTOLOGY_AVAILABLE = False
    _onto_mgr = None
    print(f"Ontology not available: {_onto_err}")


# === CUSTOM_CATEGORY_ONTOLOGY_PATCH_HELPER v1 ===
def _ask_parent_region_for_custom_category(new_label: str):
    """
    When the user creates a custom category via Select Label → Custom,
    ask them which existing region is the parent (for attribute + ontology
    inheritance). Returns (parent_name, ontology_curie) or ("", "").
    """
    if not _ONTOLOGY_AVAILABLE or _onto_mgr is None:
        return "", ""

    existing_regions = _onto_mgr.get_current_template_region_names()
    if not existing_regions:
        return "", ""

    # Build a small popup to select parent region
    dlg = tk.Toplevel(root)
    dlg.title(f"Configure '{new_label}'")
    dlg.geometry("420x280")
    dlg.transient(root)
    dlg.grab_set()

    tk.Label(dlg, text=f"New category: {new_label}",
             font=("Arial", 11, "bold")).pack(pady=(10, 2))
    tk.Label(dlg,
             text="Select an existing region to inherit\nattributes (texture, setae, color, etc.)\nand ontology URI from:",
             font=("Arial", 9), fg="#555", justify=tk.LEFT).pack(pady=(2, 8))

    parent_var = tk.StringVar(value="(none — start with empty schema)")
    options = ["(none — start with empty schema)"] + existing_regions
    parent_cb = ttk.Combobox(dlg, textvariable=parent_var, values=options,
                               state="readonly", width=40)
    parent_cb.pack(pady=4)

    tk.Label(dlg, text="Optional: override ontology CURIE\n(leave blank to inherit from parent)",
             font=("Arial", 9), fg="#555", justify=tk.LEFT).pack(pady=(10, 2))
    ont_var = tk.StringVar(value="")
    ont_entry = tk.Entry(dlg, textvariable=ont_var, width=40)
    ont_entry.pack(pady=4)
    tk.Label(dlg, text="e.g. HAO:0001234, UBERON:0001234",
             font=("Arial", 8), fg="#999").pack()

    result = {"parent": "", "ontology": ""}

    def on_ok():
        p = parent_var.get()
        if p == "(none — start with empty schema)":
            result["parent"] = ""
        else:
            result["parent"] = p
        result["ontology"] = ont_var.get().strip()
        dlg.destroy()
    ...
    tk.Button(btn_frame, text="Skip", command=on_cancel, width=10).pack(side=tk.LEFT, padx=5)

    root.wait_window(dlg)
    return result["parent"], result["ontology"]
# === END CUSTOM_CATEGORY_ONTOLOGY_PATCH_HELPER v1 ===
...
# v74: on a Mac (Aqua) Tk ignores button background colours, so the colour-coded
# toolbar came out grey. There, use tkmacosx's button (drawn on a canvas, honours bg).
# Nothing changes on Windows / Linux / X11. DESCRIPTRON_MAC_BUTTONS=0 turns it off,
# =1 forces it on (for testing on another system). Falls back silently if the
# package is missing (pip installs it on macOS only).
def _use_colour_buttons_on_mac():
    flag = os.environ.get("DESCRIPTRON_MAC_BUTTONS", "")
    if flag == "0":
        return
    try:
        ...
        aqua = False
    if not aqua and flag != "1":
        return
    try:
        from tkmacosx import Button as _MacButton
        import tkinter.font as _tkfont
        _f = _tkfont.nametofont("TkDefaultFont")
        _char, _line = max(_f.measure("0"), 6), max(_f.metrics("linespace"), 12)
    except Exception as e:
        print(f"[v74] Mac colour buttons not available ({e}); using standard buttons")
        return

    def _px(kw):
        # Tk counts a text button's width/height in characters/lines, tkmacosx in pixels
        if not kw.get("image"):
    ...
    class _ColourButton(_MacButton):
        def __init__(self, master=None, cnf={}, **kw):
            kw = {**(cnf or {}), **kw}
            kw.setdefault("activeforeground", "black")   # tkmacosx default is white text on press
            kw.setdefault("padx", 8)
            kw.setdefault("pady", 3)
            super().__init__(master, **_px(kw))
        ...
        def configure(self, cnf=None, **kw):
            ...
            return super().configure(**_px({**(cnf or {}), **kw}))
        config = configure

        # it is a Canvas, whose lift()/lower() move canvas ITEMS; the layout needs the widget's
        lift = tkraise = tk.Misc.tkraise
        lower = tk.Misc.lower

    tk.Button = _ColourButton
    print("[v74] colour buttons on (tkmacosx)")


_use_colour_buttons_on_mac()
...
scale_bar_info = None  # {mask_idx, length_px, scale_value, unit, px_per_unit}
...
# v70: COCO visibility per keypoint, PARALLEL to point_coords.
#   2 = annotated/confirmed (green)   1 = predicted, unconfirmed (goldenrod)
#   0 = absent on this specimen (red)
# Deliberately separate from point_labels, which stays a strict 1/0 array because
# SAM2 consumes it as foreground/background prompts — a third value there would
# make flagged keypoints act as background prompts during segmentation.
keypoint_visibility = []


def _sync_kp_vis(default=2):
    """Keep keypoint_visibility the same length as point_coords."""
    global keypoint_visibility
    n = len(point_coords)
    if len(keypoint_visibility) < n:
        keypoint_visibility += [default] * (n - len(keypoint_visibility))
    elif len(keypoint_visibility) > n:
        keypoint_visibility = keypoint_visibility[:n]
    return keypoint_visibility


KP_COLOR = {2: "green", 1: "goldenrod", 0: "red"}
multi_mask_mode = False  # Default is single mask mode
point_orders = []  # Stores the order (superscript labels) of keypoints
keypoint_bank = []  # Hidden keypoint sets: [{'name':str,'coords':list,'labels':list,'orders':list,'kp_labels':list}, ...]
...
def pick_point(event):
    ...
    if [x, y] not in point_coords:
        ...
        keypoint_visibility.append(2 if label == 1 else 0)   # v70
...
def redraw_points():
    ...
    if point_coords:
        for i, point in enumerate(point_coords):
            ...
            vis = _sync_kp_vis()
            color = KP_COLOR.get(vis[i], "green")          # v70: 2 green, 1 goldenrod, 0 red
...
def clear_points():
    global point_coords, point_labels, point_orders, keypoint_labels
    canvas.delete("point")
    canvas.delete("point_number")
    point_coords = []
    point_labels = []
...
# ── Keypoint bank helpers ────────────────────────────────────────────────────

def _kp_snapshot(name=None):
    """Capture the current active keypoint set into a bank-compatible dict."""
    if not name:
        name = f"Set {len(keypoint_bank) + 1}"
    return {
        'name': name,
        'coords': [list(c) for c in point_coords],
        'labels': list(point_labels),
        'orders': list(point_orders),
        'kp_labels': list(keypoint_labels),
    }


def _kp_restore(snap):
    """Load a bank dict back into the active point_coords lists (replaces active set)."""
    global point_coords, point_labels, point_orders, keypoint_labels
    point_coords = [list(c) for c in snap['coords']]
    point_labels = list(snap['labels'])
    point_orders = list(snap['orders'])
    keypoint_labels = list(snap['kp_labels'])


def _update_hide_kp_btn():
    """Sync the hide-keypoints button label and colour to current state."""
    try:
        n = (1 if point_coords else 0) + len(keypoint_bank)
        if n == 0 or (n == 1 and point_coords):
            hide_keypoints_btn.config(text="Hide Keypoints", relief=tk.RAISED, bg=globals().get("_KP_BG", "#d9d9d9"))
        elif not point_coords:
            hide_keypoints_btn.config(text="Show Keypoints", relief=tk.SUNKEN, bg="lightcoral")
        else:
            hide_keypoints_btn.config(text=f"Kp Sets ({n})", relief=tk.RAISED, bg="lightyellow")
    except Exception:
        pass


def toggle_hide_keypoints():
    """Main handler for the Hide/Show Keypoints button."""
    global point_coords, point_labels, point_orders, keypoint_labels, keypoint_bank

    has_active = bool(point_coords)
    total = (1 if has_active else 0) + len(keypoint_bank)

    if total == 0:
        messagebox.showinfo("Info", "No keypoints to hide or show.")
        return

    if total == 1:
        if has_active:
            keypoint_bank.append(_kp_snapshot())
            point_coords = []; point_labels = []; point_orders = []; keypoint_labels = []
            canvas.delete("point"); canvas.delete("point_number")
        else:
            snap = keypoint_bank.pop()
            _kp_restore(snap)
            redraw_points(); update_superscripts()
        _update_hide_kp_btn()
        return
...
def _show_kp_manager():
    """Popup for managing multiple keypoint sets: hide, show, hide-all, show-all."""
    global point_coords, point_labels, point_orders, keypoint_labels, keypoint_bank

    popup = tk.Toplevel(root)
    popup.title("Keypoint Set Manager")
    popup.geometry("480x400")
    popup.transient(root)
    popup.grab_set()

    tk.Label(popup, text="Click to select  •  Double-click to deselect\n"
             "[VISIBLE] = active set    [HIDDEN] = stored in bank",
             justify=tk.LEFT, fg="gray").pack(pady=(8, 2), anchor=tk.W, padx=12)

    lf = tk.Frame(popup)
    lf.pack(fill=tk.BOTH, expand=True, padx=12, pady=4)
    sb = tk.Scrollbar(lf, orient=tk.VERTICAL)
    lb = tk.Listbox(lf, selectmode=tk.MULTIPLE, yscrollcommand=sb.set,
                    height=10, font=("Courier", 10), exportselection=False)
    sb.config(command=lb.yview); sb.pack(side=tk.RIGHT, fill=tk.Y)
    lb.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

    # Build the entry list: active set first, then bank sets
    entries = []
    if point_coords:
        entries.append(('active', None))
        lb.insert(tk.END, f"[VISIBLE] Active set              {len(point_coords):3d} pts")
    for i, snap in enumerate(keypoint_bank):
        entries.append(('bank', i))
        lb.insert(tk.END, f"[HIDDEN ] {snap['name']:<22s} {len(snap['coords']):3d} pts")
        lb.itemconfig(len(entries) - 1, fg='gray')
    ...
    def hide_selected():
        ...
        for i in lb.curselection():
            kind, _ = entries[i]
            if kind == 'active' and point_coords:
                keypoint_bank.append(_kp_snapshot())
                point_coords = []; point_labels = []; point_orders = []; keypoint_labels = []
                canvas.delete("point"); canvas.delete("point_number")
        popup.destroy(); _update_hide_kp_btn()
    ...
    def show_selected():
        ...
        for i in lb.curselection():
            kind, ref = entries[i]
            if kind == 'bank':
                target = keypoint_bank[ref]          # save reference
                if point_coords:                     # push active to bank
                    keypoint_bank.append(_kp_snapshot())
                    point_coords = []; point_labels = []; point_orders = []; keypoint_labels = []
                keypoint_bank.remove(target)         # remove by identity
                _kp_restore(target)
                redraw_points(); update_superscripts()
    ...
    def show_all():
        global point_coords, point_labels, point_orders, keypoint_labels, keypoint_bank
        all_c = [list(c) for c in point_coords]
        all_l = list(point_labels)
        for snap in keypoint_bank:
            all_c.extend([list(c) for c in snap['coords']])
            all_l.extend(snap['labels'])
        keypoint_bank.clear()
        point_coords = all_c; point_labels = all_l
        n = len(all_c)
        point_orders = list(range(1, n + 1))
        keypoint_labels = list(range(1, n + 1))
        redraw_points(); update_superscripts()
        popup.destroy(); _update_hide_kp_btn()

    bf = tk.Frame(popup)
    bf.pack(pady=8)
    tk.Button(bf, text="Hide Selected", command=hide_selected, bg="lightcoral", width=13).pack(side=tk.LEFT, padx=4)
    tk.Button(bf, text="Show Selected", command=show_selected, bg="lightgreen",  width=13).pack(side=tk.LEFT, padx=4)
    tk.Button(bf, text="Hide All",      command=hide_all,      bg="salmon",      width=10).pack(side=tk.LEFT, padx=4)
    tk.Button(bf, text="Show All",      command=show_all,      bg="palegreen",   width=10).pack(side=tk.LEFT, padx=4)
    tk.Button(bf, text="Cancel",        command=popup.destroy,                   width=8 ).pack(side=tk.LEFT, padx=4)


# ── End keypoint bank helpers ────────────────────────────────────────────────
...
def apply_sam2_prompt_with_points():
   ...
   # Redraw masks to display the result immediately
    ...
    _clear_bboxes()   # v74: boxes are used once
...
def save_points_to_via2(save_path=None, quiet=False):
    """
    Save point coordinates, labels, and superscript (order) in COCO format with image path included for Detectron2.

    v70: pass save_path to write without opening a dialog (used by the folder
    browser's auto-save), and quiet=True to suppress the confirmation popup.
    """
    ...
    # v70: visibility is taken from the Positive/Negative point label instead of being
    # hard-coded to 2.  Positive (label 1) -> v=2  (landmark annotated and visible)
    #      Negative (label 0) -> v=0  (landmark absent / not scoreable on this specimen)
    # This keeps landmark NUMBERING stable when a point cannot be placed, which is what
    # downstream correspondence needs.  COCO requires v=0 keypoints to be written as
    # (0, 0); the clicked position is preserved in the non-standard "keypoints_clicked"
    ...
    for i, (point, label) in enumerate(zip(point_coords, point_labels)):
        ...
        if vis > 0:
            keypoints.extend([point[0], point[1], vis])
            visible_pts.append(point)
            num_keypoints += 1
        else:
            keypoints.extend([0, 0, 0])
        keypoints_clicked.extend([point[0], point[1], vis])
        point_order.append(i + 1)  # Save the order in which the point was clicked (superscript label)

    if not visible_pts:          # degenerate case: every landmark marked missing
        visible_pts = [[0, 0]]
    ...
    annotation = {
        ...
        "keypoints": keypoints,
        "num_keypoints": num_keypoints,   # COCO: count of v>0 only
        "area": 1,  # COCO format requires this but it's not relevant for keypoints
        # bbox is computed from VISIBLE points only, so (0,0) placeholders for missing
        # landmarks cannot drag the box back to the image origin.
        "bbox": [min([x[0] for x in visible_pts]), min([x[1] for x in visible_pts]),
                 max([x[0] for x in visible_pts]) - min([x[0] for x in visible_pts]),
                 max([x[1] for x in visible_pts]) - min([x[1] for x in visible_pts])],
        "iscrowd": 0,
        "point_order": point_order,  # Store the superscript label order
        "keypoints_clicked": keypoints_clicked  # non-standard: clicked xy even for v=0
    }
    ...
    # Prompt user to save the COCO JSON (for the individual mask) unless a path was
    # supplied by the caller (v70 folder browser auto-save).
    ...
    if save_path:
        # v70 FIX: coco_output_mask is a session-long accumulator that is never reset
        # (initialised once at module load, appended to on every save), so dumping it
        # wrote image 1 + image 2 + ... into image N's keypoint file.  Invisible in
        # single-image sessions, badly wrong when stepping through a folder.  Write a
        # per-image document instead and leave the accumulator untouched, so the
        # mask/contour path that also uses it is unaffected.
        per_image = {
            "images": [image_metadata],
            "annotations": [annotation],
            "categories": coco_output_mask.get("categories", [])
        }
        with open(save_path, 'w') as json_file:
            json.dump(per_image, json_file, indent=4)
        if not quiet:
...
def toggle_keypoint_edit_mode():
    ...
    if keypoint_edit_mode:
        # Deactivate any active brush tool so it doesn't fire during keypoint dragging
        _deactivate_brush_tool()
        keypoint_edit_btn.config(relief=tk.SUNKEN, text="Keypoint Edit: ON")
        canvas.bind("<ButtonPress-1>", start_move_keypoint)
        ...
        canvas.unbind("<ButtonPress-1>")
        canvas.unbind("<ButtonRelease-1>")
        # Keep right-click bound so line scale-bar detection still works outside keypoint mode
        canvas.bind("<Button-3>", on_right_click)
...
def on_right_click(event):
    """
    Right-click handler — always bound to the canvas.

    • Near a line annotation → 'Set as Scale Bar' (works in any mode, click
      anywhere along the visible line, not just near control points).
    • In Keypoint Edit mode and near a keypoint → keypoint actions.
    Both can appear together if the click is close to both.
    """
    x = canvas.canvasx(event.x) / zoom_level
    y = canvas.canvasy(event.y) / zoom_level

    # Keypoint check — only when Keypoint Edit mode is active (unchanged behaviour)
    kp_index = None
    if keypoint_edit_mode:
    ...
    # Line annotation check — always active.
    # Uses segment distance so clicking anywhere along the drawn line works,
    # not just near the sparse control points.
    LINE_THRESH = 20  # image-space pixels
    line_mask_index = None
    for i, mdata in enumerate(segmentation_masks):
        if not mdata.get('is_line', False):
            continue
        pts = mdata.get('line_points', [])
        hit = False
        for j in range(len(pts) - 1):
            if _pt_to_seg_dist(x, y, pts[j][0], pts[j][1], pts[j+1][0], pts[j+1][1]) < LINE_THRESH:
                hit = True
                break
        if not hit and pts:
            if abs(pts[-1][0] - x) < LINE_THRESH and abs(pts[-1][1] - y) < LINE_THRESH:
                hit = True
        if hit:
            line_mask_index = i
            break

    if kp_index is None and line_mask_index is None:
        return

    menu = tk.Menu(canvas, tearoff=0)
    if kp_index is not None:
        menu.add_command(label="Remove Keypoint", command=lambda: remove_keypoint(kp_index))
        menu.add_command(label="Rename Keypoint", command=lambda: rename_keypoint(kp_index))
        menu.add_separator()
        menu.add_command(label="Mark Confirmed (green, v=2)",
                         command=lambda: set_keypoint_visibility(kp_index, 2))
        menu.add_command(label="Mark Needs Check (goldenrod, v=1)",
                         command=lambda: set_keypoint_visibility(kp_index, 1))
        menu.add_command(label="Mark Absent (red, v=0)",
                         command=lambda: set_keypoint_visibility(kp_index, 0))
    if line_mask_index is not None:
        if kp_index is not None:
            menu.add_separator()
        menu.add_command(label="Set as Scale Bar",
                         command=lambda idx=line_mask_index: designate_scale_bar(idx))
    menu.post(event.x_root, event.y_root)


def _calc_line_length_px(line_points):
    """Return the total pixel length of a polyline."""
    import math
    total = 0.0
...
def designate_scale_bar(mask_idx):
    """
    Prompts the user to define the real-world measurement represented by the
    selected line, calculates px/unit, and stores the result in the annotation.
    """
    global scale_bar_info, segmentation_masks, coco_output_accumulate

    mdata = segmentation_masks[mask_idx]
    line_points = mdata.get('line_points', [])
    if len(line_points) < 2:
        messagebox.showerror("Error", "This line has fewer than 2 points and cannot be used as a scale bar.")
        return

    length_px = _calc_line_length_px(line_points)

    # --- Dialog window ---
    dlg = tk.Toplevel()
    dlg.title("Set Scale Bar")
    dlg.grab_set()

    tk.Label(dlg, text=f"Line length: {length_px:.1f} px", font=("Helvetica", 10, "bold")).grid(
        row=0, column=0, columnspan=3, padx=10, pady=(10, 4))
    tk.Label(dlg, text="Scale bar value:").grid(row=1, column=0, sticky="e", padx=5, pady=5)

    scale_val_var = tk.StringVar(value="1")
    tk.Entry(dlg, textvariable=scale_val_var, width=10).grid(row=1, column=1, padx=5, pady=5)

    unit_var = tk.StringVar(value="um")
    unit_menu = tk.OptionMenu(dlg, unit_var, "um", "mm", "cm", "nm", "m")
    ...
    unit = result['unit']
    px_per_unit = length_px / scale_value

    # Store in the global scale_bar_info
    scale_bar_info = {
        'mask_idx': mask_idx,
        'length_px': length_px,
    ...
    # Persist into mask_data so it survives redraws
    mdata['scale_bar'] = scale_bar_info.copy()

    # Update the annotation in coco_output_accumulate so it ends up in the JSON
    ann_id = mdata.get('annotation_id')
    if ann_id is not None:
        for ann in coco_output_accumulate.get("annotations", []):
            if ann["id"] == ann_id:
                ann['scale_bar'] = {
                    'length_px': length_px,
                    'scale_value': scale_value,
    ...
    messagebox.showinfo(
        "Scale Bar Set",
        f"Scale bar: {length_px:.1f} px = {scale_value} {unit}\n"
        f"({px_per_unit:.4f} px/{unit})"
    )


def remove_keypoint(index):
    global point_coords, keypoint_labels, point_labels, point_orders

    total = len(point_coords)
    # Ensure keypoint_labels is in sync before touching anything
    if len(keypoint_labels) != total:
        keypoint_labels = list(range(1, total + 1))

    if index < 0 or index >= total:
        return

    # Remove the keypoint from every parallel list
    del point_coords[index]
    if index < len(keypoint_visibility):
        del keypoint_visibility[index]        # v70: keep parallel list aligned
    if index < len(point_labels):
        del point_labels[index]
    if index < len(point_orders):
        del point_orders[index]
    if index < len(keypoint_labels):
        del keypoint_labels[index]

    # Renumber sequentially so display matches COCO array position
    keypoint_labels[:] = list(range(1, len(keypoint_labels) + 1))
    point_orders[:] = list(range(1, len(point_orders) + 1))

    redraw_points()
    update_superscripts()

def set_keypoint_visibility(index, v):
    """v70: set one keypoint's COCO visibility (2 confirmed / 1 needs check / 0 absent)."""
    _sync_kp_vis()
    if 0 <= index < len(keypoint_visibility):
        keypoint_visibility[index] = int(v)
        if index < len(point_labels):
            point_labels[index] = 1 if v > 0 else 0   # keep SAM2 array strictly binary
        redraw_points()


def rename_keypoint(index):
    global keypoint_labels, point_coords, point_labels, point_orders

    # Sync keypoint_labels with point_coords — may be empty if
    # initialize_keypoint_labels() was never called before right-clicking.
    total = len(point_coords)
    if len(keypoint_labels) != total:
        keypoint_labels = list(range(1, total + 1))
    ...
    new_label = simpledialog.askstring(
        "Rename Keypoint",
        f"Enter new position (1\u2013{total}) for keypoint #{index + 1}:\n"
        "(Moves it to that slot and renumbers all others.)"
    )
    ...
    try:
        ...
        messagebox.showerror("Error",
            ...
            "Keypoint positions are always sequential integers.")
        return

    if new_val < 1 or new_val > total:
        messagebox.showerror("Error", f"Please enter a number between 1 and {total}.")
        return

    if new_val == keypoint_labels[index]:
        return  # nothing to do

    # Reorder keypoint_labels
    item = keypoint_labels.pop(index)
    keypoint_labels.insert(new_val - 1, item)
    keypoint_labels[:] = list(range(1, len(keypoint_labels) + 1))

    # Keep point_coords and related lists in the same order
    coord = point_coords.pop(index)
    point_coords.insert(new_val - 1, coord)
    if index < len(point_labels):
        lbl = point_labels.pop(index)
        point_labels.insert(new_val - 1, lbl)
    if index < len(point_orders):
        ord_ = point_orders.pop(index)
        point_orders.insert(new_val - 1, ord_)
        point_orders[:] = list(range(1, len(point_orders) + 1))
...
def load_image_for_prompt():
    ...
    canvas.delete("all")
    bbox_rects = []
    bounding_boxes.clear()   # v74: a box from the previous image must not carry over
    canvas.config(scrollregion=(0, 0, image.shape[1], image.shape[0]))
...
# ─────────────────────────────────────────────────────────────────────────────
# v70: folder-based image browsing for annotation sessions.
#
# Previously every image had to be opened one at a time through the file dialog.
# The prediction workflow already had next/prev navigation, but it was driven by
# a prediction JSON (load_image_list / prediction_image_names), so it was unusable
# for a folder of UN-annotated images.  These helpers add that path.
# ─────────────────────────────────────────────────────────────────────────────
IMAGE_EXTS = (".jpg", ".jpeg", ".png", ".tif", ".tiff", ".bmp", ".webp")
folder_images = []        # absolute paths, sorted
folder_index = 0
folder_autosave = {"on": True}   # auto-save keypoints when leaving an image
folder_status_label = None       # set when the button row is built


def _display_image_path(path):
    """Render one image on the canvas.

    v70 FIX: this must render through apply_zoom(), not draw the image 1:1.
    zoom_level is a module-level global that is never reset between images, while
    click handling divides canvas coords by it (line ~425).  Drawing unscaled while
    zoom_level was still, say, 1.8 from the previous image meant every saved keypoint
    was silently divided by 1.8 — points looked right on screen and were wrong in the
    file, and drifted as soon as you zoomed.  Pan offset is reset for the same reason.
    """
    global image, file_path_base, canvas, bbox_rects, mode, zoom_level
    mode = "prompt"
    file_path_base = os.path.splitext(path)[0]
    image = np.array(Image.open(path).convert("RGB"))
    canvas.delete("all")
    bbox_rects = []
    bounding_boxes.clear()   # v74: a box from the previous image must not carry over
    zoom_level = 1.0                     # reset zoom for the new image
    try:                                 # reset pan to top-left
        canvas.xview_moveto(0)
        canvas.yview_moveto(0)
    except Exception:
        pass
    apply_zoom()                         # canonical renderer: image + points at zoom
    canvas.update_idletasks()
    try:
        button_frame.lift()
    except Exception:
...
folder_kp_cache = {}   # image path -> keypoints placed this session


def _folder_store_points():
    """Remember this image's keypoints so they come back when you navigate back."""
    if folder_images:
        folder_kp_cache[folder_images[folder_index]] = {
            "coords": [list(p) for p in point_coords],
            "labels": list(point_labels),
            "orders": list(point_orders),
            "vis": list(_sync_kp_vis()),
        }


def _folder_restore_points(path):
    """Reinstate cached keypoints for an image and redraw them at current zoom."""
    global point_coords, point_labels, point_orders
    try:
        clear_points()
    except Exception:
        pass
    st = folder_kp_cache.get(path)
    if st and st["coords"]:
        point_coords = [list(p) for p in st["coords"]]
        point_labels = list(st["labels"])
        point_orders = list(st["orders"])
        keypoint_visibility.clear()
        keypoint_visibility.extend(st.get("vis", [2] * len(point_coords)))
        try:
            redraw_points()
        except Exception:
            pass
...
def _folder_has_annotation(path):
    """True if this image already has keypoints (this session or on disk)."""
    st = folder_kp_cache.get(path)
    if st and st["coords"]:
        return True
    return os.path.exists(os.path.splitext(path)[0] + "_keypoints.json")


def _folder_update_label():
    if folder_status_label is not None and folder_images:
        p = folder_images[folder_index]
        name = os.path.basename(p)
        done = "✓" if _folder_has_annotation(p) else "·"
        folder_status_label.config(
            text=f"{done} {folder_index + 1} / {len(folder_images)}  {name[:26]}")


def _folder_autosave_current():
    """Auto-save on leaving an image: keypoints per image, plus an incremental
    snapshot of the accumulated mask annotations.

    NOTE on masks: labelled masks are NOT per-image files.  apply_label() pushes them
    into coco_output_accumulate, which persists across images and is normally written
    once at the end by the marmot (save & close) button.  Over a long folder run that
    means the whole session sits in memory, so we also dump a snapshot each time we
    advance — a crash then costs one image, not the whole run.
    """
    if not folder_autosave["on"] or not folder_images:
        return
    base = os.path.splitext(folder_images[folder_index])[0]
    if point_coords:
        try:
            save_points_to_via2(save_path=base + "_keypoints.json", quiet=True)
            logging.info(f"[v70] auto-saved keypoints -> {base}_keypoints.json")
        except Exception as e:
            logging.error(f"[v70] keypoint auto-save failed: {e}")
    try:
        if coco_output_accumulate.get("annotations"):
            snap = os.path.join(os.path.dirname(folder_images[folder_index]),
                                "_descriptron_session_masks.json")
            with open(snap, "w") as f:
                json.dump(coco_output_accumulate, f)
            logging.info(f"[v70] mask snapshot ({len(coco_output_accumulate['annotations'])} "
                         f"annotations) -> {snap}")
    except Exception as e:
        logging.error(f"[v70] mask snapshot failed: {e}")


def load_image_folder(folder=None):
    """Pick a folder of images and load the first one; enables Next/Prev."""
    global folder_images, folder_index
    if folder is None:
        folder = filedialog.askdirectory(title="Select a folder of images to annotate")
    if not folder:
        return
    imgs = [os.path.join(folder, f) for f in sorted(os.listdir(folder))
            if f.lower().endswith(IMAGE_EXTS)
            and not f.startswith(".")]   # [v72] skip macOS "._*" resource forks / hidden files (PIL can't open them)
    if not imgs:
        messagebox.showinfo("Load Folder", "No images found in that folder.")
        return
    folder_images = imgs
    folder_index = 0
    folder_kp_cache.clear()
    _display_image_path(folder_images[0])
    _folder_restore_points(folder_images[0])
    _folder_update_label()
    messagebox.showinfo("Load Folder",
                        f"Loaded {len(imgs)} images.\n\nUse Next ▶ / ◀ Prev to move "
                        f"through them.\nAuto-save writes <image>_keypoints.json "
                        f"beside each image.")


def folder_goto(delta):
    """Move +1 / -1 through the loaded folder, auto-saving the current image."""
    global folder_index, segmentation_masks, coco_output_accumulate
    if not folder_images:
        messagebox.showinfo("No Folder", "Use 'Load Folder' first.")
        return
    new = folder_index + delta
    if new < 0:
        messagebox.showinfo("Start of Folder", "This is the first image.")
        return
    if new >= len(folder_images):
        messagebox.showinfo("End of Folder", "This is the last image.")
        return
    # Masks that were drawn but never sent through Apply Label are not in
    # coco_output_accumulate, so advancing would discard them.  Warn before that
    # happens rather than losing work silently.
    if segmentation_masks:
        if not messagebox.askyesno(
                "Unlabelled masks",
                f"{len(segmentation_masks)} mask(s) on this image have not been "
                f"committed with 'Apply Label' and will be discarded.\n\nMove on anyway?"):
            return
    _folder_autosave_current()
    _folder_store_points()          # keep them for when you navigate back
    folder_index = new
    _display_image_path(folder_images[folder_index])
    _folder_restore_points(folder_images[folder_index])
    _folder_update_label()


def _enable_drag_and_drop(widget):
    """Optional: real drag-and-drop if tkinterdnd2 is installed. Silent no-op otherwise."""
    try:
        ...
        def _drop(event):
            ...
            if os.path.isdir(p):
                ...
                load_image_folder(os.path.dirname(p))
        widget.drop_target_register(DND_FILES)
        widget.dnd_bind("<<Drop>>", _drop)
        logging.info("[v70] drag-and-drop enabled")
    except Exception:
        pass   # tkinterdnd2 not installed; folder button still works
...
def _clear_bboxes():
    """v74 fix: forget boxes once SAM2 has used them (or a new image is shown).
    The canvas rectangle vanished after a prediction but the box stayed in
    bounding_boxes, so the NEXT prediction silently reused it as well.
    Point prompts are NOT cleared here: they share point_coords with keypoints."""
    for i in range(len(bounding_boxes)):
        try:
            canvas.delete(f"bbox_{i}")
        except Exception:
            pass
...
def _draw_brush_cursor(cx, cy, color):
    """Draw the zoom-correct circular cursor overlay (canvas coordinates)."""
    r = brush_size * zoom_level
    canvas.delete("brush_cursor")
    canvas.create_oval(cx - r, cy - r, cx + r, cy + r,
                       outline=color, width=2, tags="brush_cursor")


def show_brush_cursor(event):
    """Hover handler: show a circle that exactly matches the brush footprint at current zoom."""
    cx = canvas.canvasx(event.x)
    cy = canvas.canvasy(event.y)
    if current_tool == "paintbrush":
        _draw_brush_cursor(cx, cy, "green")
    elif current_tool == "eraser":
        _draw_brush_cursor(cx, cy, "blue")


def hide_brush_cursor(event=None):
    canvas.delete("brush_cursor")


def paint(event):
    if segmentation_masks:
        # Image-space position of the cursor
        x = int(canvas.canvasx(event.x) / zoom_level)
        y = int(canvas.canvasy(event.y) / zoom_level)

        mask = segmentation_masks[current_mask_index]['segmentation']

        # True circular brush via cv2.circle (not a rectangular slice)
        cv2.circle(mask, (x, y), brush_size, 1, -1)
        redraw_masks()
        update_annotation_in_cache()

        # Keep the cursor circle visible and zoom-correct while dragging
        _draw_brush_cursor(canvas.canvasx(event.x), canvas.canvasy(event.y), "green")
...
def erase(event):
    if segmentation_masks:
        ...
        # True circular eraser via cv2.circle (not a rectangular slice)
        cv2.circle(mask, (x, y), brush_size, 0, -1)
        redraw_masks()
        update_annotation_in_cache()

        # Keep the cursor circle visible and zoom-correct while dragging
        _draw_brush_cursor(canvas.canvasx(event.x), canvas.canvasy(event.y), "blue")


def _deactivate_brush_tool():
    """Fully deactivate whichever brush tool is currently on."""
    global current_tool
    if current_tool == "paintbrush":
...
def select_eraser():
    ...
    if current_tool == "eraser":
        ...
        _deactivate_brush_tool()          # clear paintbrush if it was on
        current_tool = "eraser"
        canvas.bind("<B1-Motion>", erase)
        canvas.bind("<Motion>", show_brush_cursor)
        canvas.bind("<Leave>", hide_brush_cursor)
...
def select_paintbrush():
    ...
    if current_tool == "paintbrush":
        ...
        _deactivate_brush_tool()          # clear eraser if it was on
...
def load_json_keypoints_into_gui(annotations_for_image):
    ...
    for ann in annotations_for_image:
        if "keypoints" in ann and ann["keypoints"]:
            ...
            # v70b: a landmark's identity IS its index in the keypoints array, so the
            # slots must be loaded POSITIONALLY.  The old loop skipped v=0 slots and
            # numbered whatever survived 1..N, and save_points_to_via2 renumbers the
            # same way on the way out — so one absent landmark silently renamed every
            # landmark after it (with slot 5 absent, landmark 6 came back as landmark
            # 5), and a corrected file fed back as a reference would then transfer the
            # wrong points.  Absent slots are now kept as v=0 placeholders: they draw
            # red at their clicked position (or the origin if nothing was ever placed),
            # so dragging one into place and right-clicking it green is how you ADD a
            # landmark the references never carried.
            clicked = ann.get("keypoints_clicked") or []
            order = ann.get("point_order") or []
            for i in range(0, len(kpts), 3):
                slot = i // 3
                v = int(kpts[i + 2])
                if v > 0:
                    x, y = kpts[i], kpts[i + 1]
                else:                     # COCO stores absent landmarks as (0, 0)
                    x, y = ((clicked[i], clicked[i + 1])
                            if len(clicked) >= i + 3 else (0, 0))
                # v=2 -> green (label 1); v=1 goldenrod and v=0 red -> label 0, so a
                # flagged or absent point is never handed to SAM2 as a fg prompt.
                point_coords.append([x, y])
                point_labels.append(1 if v == 2 else 0)
                keypoint_visibility.append(v)           # v70: preserve 2 vs 1 vs 0
                point_orders.append(int(order[slot]) if len(order) > slot else slot + 1)
...
def apply_category_label():
    ...
    if label in category_name_to_id:
        ...
        # === CUSTOM_CATEGORY_ONTOLOGY_PATCH_HOOK v1 ===
        # Register the custom category with OntologyManager so it inherits
        # a proper attribute schema (texture, setae, color, etc.) via parent region.
        if _ONTOLOGY_AVAILABLE and _onto_mgr is not None:
            try:
                parent_region, ont_override = _ask_parent_region_for_custom_category(label)
                reg_info = _onto_mgr.register_custom_category(
                    category_name=label,
                    parent_category=parent_region,
                    ontology_curie=ont_override,
                    supercategory=supercategory,
                )
                # Mirror the ontology URI into the category dict so COCO exports carry it
                if reg_info.get("ontology"):
                    new_category["ontology"] = reg_info["ontology"]
            except Exception as _reg_err:
                print(f"[warn] Could not register custom category '{label}': {_reg_err}")
        # === END CUSTOM_CATEGORY_ONTOLOGY_PATCH_HOOK v1 ===
...
def _mask_has_interior_holes(mask_8bit, min_area=64):
    """True if the mask encloses background regions (i.e. the user erased inside it).

    A hole is a background component that does not touch the image border.
    """
    inv = (mask_8bit == 0).astype(np.uint8)
    n, lab, stats, _ = cv2.connectedComponentsWithStats(inv, 8)
    if n <= 1:
        return False
    border = set(lab[0, :]) | set(lab[-1, :]) | set(lab[:, 0]) | set(lab[:, -1])
    for i in range(1, n):
        if i not in border and stats[i, cv2.CC_STAT_AREA] >= min_area:
            return True
    return False


def _mask_to_uncompressed_rle(mask_8bit):
    """Encode a binary mask as uncompressed COCO RLE {'size': [h, w], 'counts': [...]}.

    Column-major (Fortran) order starting with a run of 0s, which is what
    load_json_contours._decode_segmentation_to_mask() expects, so the export
    round-trips back into the GUI.
    """
    h, w = mask_8bit.shape[:2]
    flat = (mask_8bit > 0).astype(np.uint8).T.reshape(-1)   # Fortran order
    counts = []
    prev = 0
    run = 0
    for v in flat:
...
def _segmentation_from_mask(mask_8bit):
    """Polygon COCO segmentation, exactly as before (Detectron2-compatible).

    Kept unchanged so the primary .json stays a plain polygon COCO file that
    Detectron2 and pycocotools consume directly. Note this cannot represent
    erased interior regions: cv2.RETR_EXTERNAL returns only outermost contours,
    and multi-ring polygons would not help because every consumer here (and
    pycocotools) unions the rings rather than treating inner ones as holes.
    Regions erased with the eraser are therefore preserved separately, as RLE,
    in the companion .rlejson written by save_final_via2_json().
    """
    contours, _ = cv2.findContours(mask_8bit, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    coco_segmentation = []
    for contour in contours:
...
def save_mask_and_contour_via2(overwrite=False):
    ...
    # Compute contours
    coco_segmentation = _segmentation_from_mask(mask_8bit)

    # v70 fix: polygons cannot encode erased interior regions, so carry an RLE
    # copy alongside whenever the mask actually has holes. It is stripped from
    # the primary .json (keeping it plain Detectron2 COCO) and used to build the
    # companion .rlejson in save_final_via2_json().
    _rle_segmentation = (_mask_to_uncompressed_rle(mask_8bit)
                         if _mask_has_interior_holes(mask_8bit) else None)
    ...
    # v70 fix: register this image in the accumulator. Only the keypoint and
    # line paths did so, so a session of plain polygon masks exported with
    # "images": [] -- which leaves every annotation dangling and makes readers
    # that resolve annotations through the images list (SAM2-PAL among them)
    # silently load nothing. The id matches the annotation's image_id below.
    try:
        _img_dir, _img_name = os.path.split(file_path_base)
        _img_base, _img_ext = os.path.splitext(_img_name)
        if not _img_ext:
            _img_ext = ".jpg"
        if file_path_base not in [im.get('id') for im in coco_output_accumulate.get('images', [])]:
            coco_output_accumulate.setdefault('images', []).append({
                "id": file_path_base,
                "file_name": _img_base + _img_ext,
                "height": int(image.shape[0]),
                "width": int(image.shape[1]),
            })
    except Exception as _e:
        logging.warning(f"Could not register image metadata for export: {_e}")
    ...
    # v70 fix: attach the hole-preserving RLE when there is one. Stripped before
    # the primary .json is written; used to build the companion .rlejson.
    if _rle_segmentation is not None:
        annotation["segmentation_rle"] = _rle_segmentation
    ...
    # If this mask already has an annotation in the accumulators (from a previous
    # label application), remove it first so we don't end up with duplicate entries
    # for the same mask under different labels.
    old_ann_id = mask_data.get('annotation_id')
    if old_ann_id is not None:
        coco_output_accumulate["annotations"] = [
            a for a in coco_output_accumulate["annotations"] if a["id"] != old_ann_id
        ]
        coco_output_mask["annotations"] = [
            a for a in coco_output_mask["annotations"] if a["id"] != old_ann_id
        ]
    ...
    # Store annotation_id in mask_data so delete_current_mask can clean up the accumulate
    segmentation_masks[current_mask_index]['annotation_id'] = annotation_id
...
def save_final_via2_json():
    ...
    """
    global coco_output_accumulate, file_path_base, segmentation_masks, labels, point_coords, point_orders, point_labels

    if not coco_output_accumulate['annotations'] and not point_coords:
    ...
    if trash_category_id is not None:
        ...
        # If "Trash" category not found, use all annotations (copy to avoid mutating accumulator)
        filtered_annotations = list(coco_output_accumulate['annotations'])
    # --- Minimal Modification End ---

    # --- MANUAL KEYPOINTS (from point_coords placed on canvas) ---
    if point_coords:
        # Reuse or create the line_keypoints category
        manual_kp_cat_id = None
        for cat in coco_output_accumulate.get('categories', []):
            if cat['name'] == 'line_keypoints':
                manual_kp_cat_id = cat['id']
                break
        if manual_kp_cat_id is None:
            existing_ids = [cat['id'] for cat in coco_output_accumulate['categories']]
            manual_kp_cat_id = max(existing_ids) + 1 if existing_ids else 1
            coco_output_accumulate['categories'].append({
                'id': manual_kp_cat_id,
                'name': 'line_keypoints',
        ...
        for i, coord in enumerate(point_coords):
            coco_kps.extend([coord[0], coord[1], 2])
            order = point_orders[i] if i < len(point_orders) else i + 1
            kp_order.append(order)

        xs = [c[0] for c in point_coords]
        ys = [c[1] for c in point_coords]
        manual_kp_ann = {
            'id': int(time.time() * 1000) + 1,
            'image_id': coco_output_accumulate['images'][0]['id'] if coco_output_accumulate['images'] else file_path_base,
            'category_id': manual_kp_cat_id,
            'keypoints': coco_kps,
            'num_keypoints': len(point_coords),
            'point_order': kp_order,
            'bbox': [min(xs), min(ys), max(xs) - min(xs), max(ys) - min(ys)],
            'area': 1,
            'iscrowd': 0,
        }
        filtered_annotations.append(manual_kp_ann)
    ...
    if line_keypoints_data:
        ...
        for cat_id, kp_list in keypoints_by_line.items():
            ...
            for kp_data in kp_list:
                ...
                kp_orders.append(kp_data['number'])
    ...
    # v70 fix: two outputs.
    #   .json     polygon COCO, unchanged, for Detectron2 / pycocotools
    #   .rlejson  identical apart from annotations whose mask contains erased
    #             interior regions, whose "segmentation" is uncompressed RLE.
    # Polygons cannot express holes (pycocotools unions multi-ring polygons), so
    # a mask erased around a paper point mount round-trips only via the RLE file.
    # Both are written every time; .rlejson is byte-equivalent in content when
    # nothing was erased.
    _poly_annotations, _rle_annotations = [], []
    _n_with_holes = 0
    for _ann in filtered_annotations:
        _rle = _ann.get("segmentation_rle")
        _poly = {k: v for k, v in _ann.items() if k != "segmentation_rle"}
        _poly_annotations.append(_poly)
        if _rle is not None:
            _rle_ann = dict(_poly)
            _rle_ann["segmentation"] = _rle
            _rle_annotations.append(_rle_ann)
            _n_with_holes += 1
        else:
            _rle_annotations.append(_poly)

    # Define the final JSON structure using filtered annotations
    final_coco_json = {
        "images": coco_output_accumulate["images"],
        "annotations": _poly_annotations,
    ...
    # Companion RLE file
    final_rle_filename = f"{file_path_base}_final_{timestamp}.rlejson"
    try:
        with open(final_rle_filename, 'w') as _rle_file:
            json.dump({
                "images": coco_output_accumulate["images"],
                "annotations": _rle_annotations,
                "categories": coco_output_accumulate["categories"]
            }, _rle_file, indent=4)
        logging.info(f"Wrote {final_rle_filename} "
                     f"({_n_with_holes} annotation(s) with erased regions as RLE)")
    except Exception as _e:
        logging.warning(f"Could not write RLE companion file: {_e}")
        final_rle_filename = None

    # Summary message
    line_kp_count = len(line_keypoints_data)
    _msg = f"Final COCO JSON saved as '{final_json_filename}'"
    if line_kp_count > 0:
        _msg += f"\n\nIncluded {line_kp_count} keypoints extracted from line annotations."
    if final_rle_filename:
        _msg += (f"\n\nRLE companion saved as '{final_rle_filename}'"
                 f"\n{_n_with_holes} annotation(s) had erased regions, which only "
                 f"the RLE file preserves. Use the .json for Detectron2 training "
                 f"and the .rlejson where exact mask shape matters (e.g. SAM2-PAL "
                 f"templates).")
    messagebox.showinfo("Success", _msg)
...
def load_json_contours(json_path):
    ...
    def _decode_segmentation_to_mask(contours):
        mask = np.zeros((image.shape[0], image.shape[1]), dtype=np.uint8)
        if isinstance(contours, dict) and 'counts' in contours:
            h, w = contours['size']
            counts = contours['counts']
            flat = np.zeros(h * w, dtype=np.uint8)
            idx = 0; val = 0
            for run in counts:
                flat[idx:idx + run] = val
                idx += run; val = 1 - val
            mask = flat.reshape((w, h)).T.astype(np.uint8)
    ...
    # Reset keypoint bank when loading fresh JSON
    keypoint_bank.clear()
    _kp_groups = []  # temporary collector for keypoint annotation groups

    # Process each annotation in the JSON file
    for annotation in data["annotations"]:
        if annotation.get('is_trait_only'):
            continue

        # --- Line annotations: reconstruct mask and restore line metadata ---
        if annotation.get('is_line'):
            if 'segmentation' in annotation:
                mask = _decode_segmentation_to_mask(annotation["segmentation"])
                ...
                for field in ('line_points', 'line_thickness', 'num_line_keypoints', 'scale_bar'):
                    if field in annotation:
                        mask_data[field] = annotation[field]
                segmentation_masks.append(mask_data)
                label = category_id_to_name.get(category_id, 'unlabeled')
                labels[len(segmentation_masks) - 1] = label
            continue  # line annotations have no standalone segmentation to process further

        # --- Normal mask annotations ---
        if 'segmentation' in annotation:
            mask = _decode_segmentation_to_mask(annotation["segmentation"])
            category_id = annotation.get('category_id', 1)
        ...
        # --- Keypoints: collect by annotation group (exclude line-extracted) ---
        if 'keypoints' in annotation and not annotation.get('source_line_category'):
            kp_data = annotation["keypoints"]
            n = len(kp_data) // 3
            pt_order = annotation.get('point_order', list(range(1, n + 1)))
            grp = {'name': f"Set {len(_kp_groups) + 1}", 'coords': [], 'labels': [], 'orders': [], 'kp_labels': []}
            for i in range(n):
                x, y, v = kp_data[i * 3], kp_data[i * 3 + 1], kp_data[i * 3 + 2]
                if v > 0:
                    grp['coords'].append([x, y])
                    grp['labels'].append(1 if v == 2 else 0)
                    grp['orders'].append(pt_order[i] if i < len(pt_order) else i + 1)
                    grp['kp_labels'].append(pt_order[i] if i < len(pt_order) else i + 1)
            if grp['coords']:
                _kp_groups.append(grp)

    # Assign keypoint groups: first → active, extras → bank
    if _kp_groups:
        first = _kp_groups[0]
        point_coords[:] = first['coords']
        point_labels[:] = first['labels']
        point_orders[:] = first['orders']
        keypoint_labels[:] = first['kp_labels']
        for snap in _kp_groups[1:]:
            keypoint_bank.append(snap)

    print(f"Loaded {len(segmentation_masks)} masks, {len(point_coords)} active keypoints, "
          f"{len(keypoint_bank)} hidden keypoint set(s) from {json_path}")
...
def save_line_annotation_to_accumulate(mask_data, category_name, category_id, line_points, line_thickness, num_keypoints):
    ...
    # Store annotation_id in mask_data so delete_current_mask can clean up the accumulate
    mask_data['annotation_id'] = annotation_id
...
def _d2_capture_opts(val_json, val_fraction, group_regex):
    """v74: remember the held-out validation settings for the next Detectron2 run (returns None)."""
    _D2_OPTS.clear()
    _D2_OPTS.update(val_json=val_json.strip(), val_fraction=val_fraction.strip(), group_regex=group_regex.strip())
...
#                          dataset_name, keypoints, train_keypoints_only, train_segmentation_only, popup):
    ...
    script_path = os.path.join(script_dir, 'detectron2/detectron2_training_and_filterV10_and_kpts-18.py')  # v18: held-out validation
    if not os.path.exists(script_path):
        script_path = os.path.join(script_dir, 'detectron2/detectron2_training_and_filterV10_and_kpts-17.py')
    ...
    if script_path.endswith("-18.py"):                  # v74 -> v18 held-out validation options
        if _D2_OPTS.get("val_json"):
            command += ['--val-json', _D2_OPTS["val_json"]]
        else:
            try:
                command += ['--val-fraction', str(float(_D2_OPTS.get("val_fraction") or 0.2))]
            except ValueError:
                pass
        if _D2_OPTS.get("group_regex"):
            command += ['--group-regex', _D2_OPTS["group_regex"]]
...
def open_training_popup():
    ...
    # v74: held-out validation for Detectron2 v18 (v17 validated on its own training images)
    tk.Label(popup, text="Validation COCO JSON (optional; e.g. val.json from Utilities > Prepare for Detectron2):").pack(padx=10, pady=(10, 2))
    _val_json_entry = tk.Entry(popup, width=50); _val_json_entry.pack(padx=10, pady=2)
    tk.Button(popup, text="Browse", command=lambda: select_file_detectron2(_val_json_entry, "Select validation COCO JSON", popup)).pack(padx=10, pady=2)
    tk.Label(popup, text="Otherwise hold out this share of specimens:").pack(padx=10, pady=(6, 2))
    _val_frac_entry = tk.Entry(popup, width=8); _val_frac_entry.insert(0, "0.2"); _val_frac_entry.pack(padx=10, pady=2)
    tk.Label(popup, text="Specimen ID pattern in file names (regex, first group = ID; optional):").pack(padx=10, pady=(6, 2))
    _group_regex_entry = tk.Entry(popup, width=50); _group_regex_entry.pack(padx=10, pady=2)
    tk.Label(popup, fg="gray30", text="e.g. (CASENT\\d+) keeps all views of one specimen together").pack(padx=10, pady=(0, 4))

    # Start Training button
    tk.Button(
        popup,
        text="Start Training",
        command=lambda: _d2_capture_opts(_val_json_entry.get(), _val_frac_entry.get(), _group_regex_entry.get()) or run_detectron2_script(
            coco_json_entry.get(),
            img_dir_entry.get(),
...
def delete_current_mask():
    global segmentation_masks, current_mask_index, coco_output_accumulate, coco_output_mask, labels, scale_bar_info
    if segmentation_masks:
        mask_data = segmentation_masks[current_mask_index]

        # Remove the corresponding annotation from both COCO accumulators so it
        # does not appear in the final JSON after the mask is gone.
        ann_id = mask_data.get('annotation_id')
        if ann_id is not None:
            coco_output_accumulate["annotations"] = [
                a for a in coco_output_accumulate["annotations"] if a["id"] != ann_id
            ]
            coco_output_mask["annotations"] = [
                a for a in coco_output_mask["annotations"] if a["id"] != ann_id
            ]

        # If this mask was the designated scale bar, clear that too
        if scale_bar_info is not None and scale_bar_info.get('mask_idx') == current_mask_index:
            scale_bar_info = None

        # Remove the label entry for this index and re-key the labels dict so
        # the indices remain contiguous after deletion.
        labels.pop(current_mask_index, None)
        new_labels = {}
        for idx, lbl in labels.items():
            new_idx = idx if idx < current_mask_index else idx - 1
            new_labels[new_idx] = lbl
        labels = new_labels

        # Also shift scale_bar_info mask index if it refers to a later mask
        if scale_bar_info is not None:
            si = scale_bar_info['mask_idx']
            if si > current_mask_index:
                scale_bar_info['mask_idx'] = si - 1
...
def run_measurement_script():
    ...
    def submit():
        ...
        script_dir = Path(__file__).parent  # Directory where the GUI script is located
        measurement_script = script_dir / 'measure' / 'measurement_script_to_try_after_kpts_prediction_measure_kpts_V35.py' ### previous was V34
...
def run_semi_landmarking_script():
    """
    Opens a dialog to collect inputs and runs the V34 GPA semi-landmarking script.
    Supports four analysis modes:
      1. Semi-landmarks (full contour GPA)
      2. Keypoints only (Procrustes on keypoints)
      3. Combined (semi-landmarks + keypoints with TPS sliding)
      4. Margin arc (keypoint-cutoff arc extraction + open-curve GPA)
    """
    input_window = tk.Toplevel()
    input_window.title("Run Semi-Landmarking / GPA Analysis (V34)")
    input_window.geometry("720x700")
    ...
    category_name    = tk.StringVar()
    num_landmarks    = tk.IntVar(value=100)
    group_labels     = tk.StringVar()
    perform_manova   = tk.BooleanVar()
    alignment_method = tk.StringVar(value='reflect_mirrored')   # V42: mirror-image specimens reflected first
    anchor_method    = tk.StringVar(value='none')
    analysis_mode    = tk.StringVar(value='semilandmarks')

    # Margin arc vars
    margin_kp_pairs      = tk.StringVar()
    num_dorsal_landmarks = tk.IntVar(value=60)
    outlier_sd           = tk.DoubleVar(value=2.0)
    remove_outliers      = tk.BooleanVar()
    margin_arc_select    = tk.StringVar(value='dorsal')
    margin_split_method  = tk.StringVar(value='x_extrema')

    # TPS sliding vars
    slide_semilandmarks = tk.BooleanVar()
    slide_tolerance     = tk.DoubleVar(value=5.0)
    slide_iterations    = tk.IntVar(value=5)
    slide_method        = tk.StringVar(value='none')   # V42: geomorph sliding (none / procd / bending)

    # Keypoint filter
    keypoints_indices = tk.StringVar()
    require_kp_and_seg = tk.BooleanVar()

    # Line categories
    line_categories = tk.StringVar()
    ...
    # -- Core I/O --
    tk.Label(sf, text="=== Core I/O ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="COCO JSON File:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=json_path, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_file(
        json_path, [("JSON", "*.json"), ("All", "*.*")])).grid(row=row, column=2, padx=5); row += 1

    tk.Label(sf, text="Image Directory:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=image_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_directory(image_dir)).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(sf, text="Output Directory:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=output_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_directory(output_dir)).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(sf, text="Category Name (blank=all):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=category_name, width=50).grid(row=row, column=1, padx=5, pady=3); row += 1

    # -- Analysis Mode --
    tk.Label(sf, text="=== Analysis Mode ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    modes = [
        ("Semi-landmarks (full contour GPA)", "semilandmarks"),
        ("Keypoints only (Procrustes)", "keypoints"),
        ("Combined (semi-landmarks + keypoints, TPS sliding)", "combined"),
        ("Margin arc (keypoint cutoff -> select dorsal/ventral/etc -> open-curve GPA)", "dorsal_margin"),
    ]
    for label, val in modes:
        tk.Radiobutton(sf, text=label, variable=analysis_mode, value=val).grid(
            row=row, column=0, columnspan=3, sticky='w', padx=20, pady=1); row += 1

    # -- Semi-landmark settings --
    tk.Label(sf, text="=== Semi-landmark Settings ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="Num landmarks (contour):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=num_landmarks, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(sf, text="Anchor method:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.OptionMenu(sf, anchor_method, 'none', 'origin', 'curvature', 'keypoint').grid(
        row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(sf, text="Alignment:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.OptionMenu(sf, alignment_method, 'reflect_mirrored', 'without_reflection', 'with_reflection').grid(
        row=row, column=1, sticky='w', padx=5); row += 1

    # -- Keypoint settings --
    tk.Label(sf, text="=== Keypoint Settings ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="Keypoint indices (e.g. 1,2,3 or 1-5):").grid(
        row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=keypoints_indices, width=30).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Require both keypoints AND segmentation",
                   variable=require_kp_and_seg).grid(
        row=row, column=1, sticky='w', padx=5); row += 1

    # -- TPS Sliding --
    tk.Label(sf, text="=== TPS Sliding ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="Sliding (as geomorph):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.OptionMenu(sf, slide_method, 'none', 'procd', 'bending').grid(row=row, column=1, sticky='w', padx=5)
    tk.Label(sf, text="none = fixed; procd = Procrustes distance; bending = bending energy",
             fg="#555").grid(row=row, column=2, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Keypoint-anchored sliding (older option)",
                   variable=slide_semilandmarks).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Slide tolerance (px):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=slide_tolerance, width=10).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Slide iterations:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=slide_iterations, width=10).grid(
        row=row, column=1, sticky='w', padx=5); row += 1

    # -- Margin Arc Extraction Settings --
    tk.Label(sf, text="=== Margin Arc Extraction ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="Split method:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.OptionMenu(sf, margin_split_method, 'x_extrema', 'y_extrema',
                  'curvature', 'keypoints').grid(
        row=row, column=1, sticky='w', padx=5)
    tk.Label(sf, text="(auto modes need NO keypoints)").grid(
        row=row, column=2, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Keypoint pairs (only if split=keypoints):").grid(
        row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=margin_kp_pairs, width=50).grid(
        row=row, column=1, padx=5, pady=3); row += 1
    tk.Label(sf, text="Arc to keep:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.OptionMenu(sf, margin_arc_select, 'dorsal', 'ventral', 'anterior',
                  'posterior', 'shorter', 'longer').grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Num arc landmarks:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=num_dorsal_landmarks, width=10).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Outlier SD threshold:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=outlier_sd, width=10).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Remove outliers (not just flag)",
                   variable=remove_outliers).grid(
        row=row, column=1, sticky='w', padx=5); row += 1

    # -- Line categories --
    tk.Label(sf, text="=== Line / Open Curve ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1
    tk.Label(sf, text="Line categories (comma-sep):").grid(
        row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=line_categories, width=50).grid(
        row=row, column=1, padx=5, pady=3); row += 1

    # -- Group / MANOVA --
    tk.Label(sf, text="=== Groups & Statistics ===", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1

    tk.Label(sf, text="Group labels file:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=group_labels, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_file(
        group_labels, [("CSV/TSV", "*.csv *.tsv"), ("All", "*.*")])).grid(
        row=row, column=2, padx=5); row += 1
    tk.Checkbutton(sf, text="Perform MANOVA", variable=perform_manova).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    ...
    def submit():
        ...
        script_dir = Path(__file__).parent
        # V42 (V34 + mirror-image handling, checked against geomorph) preferred; V34 fallback
        semi_script = script_dir / 'measure' / 'semi_landmark_and_kpts_procrustesV42_GPA.py'
        if not semi_script.is_file():
            semi_script = script_dir / 'measure' / 'semi_landmark_and_kpts_procrustesV34_GPA.py'
        if not semi_script.is_file():
            semi_script = script_dir / 'semi_landmark_and_kpts_procrustesV34_GPA.py'
        if 'V34' in semi_script.name and alignment_method.get() == 'reflect_mirrored':
            alignment_method.set('without_reflection')       # V34 does not know the V42 mode
        if not semi_script.is_file():
            messagebox.showerror("Script Error",
                                 f"V34 GPA script not found at:\n{semi_script}")
            return
        ...
        cmd = ('conda run -n measure_env python'
               f' "{semi_script}"'
               f' --json "{json_val}"'
               f' --image_dir "{image_dir_val}"'
               f' --output_dir "{output_dir.get() or "./outputs"}"'
               )

        cat_val = category_name.get()
        if cat_val:
            cmd += f' --category_name "{cat_val}"'

        # mode-specific flags
        if mode == 'keypoints':
            cmd += ' --require_keypoints_and_segmentation'
        elif mode == 'combined':
            cmd += ' --slide_semilandmarks'
            cmd += ' --require_keypoints_and_segmentation'
            cmd += f' --slide_tolerance {slide_tolerance.get()}'
            cmd += f' --slide_iterations {slide_iterations.get()}'
        elif mode == 'dorsal_margin':
            cmd += ' --dorsal_margin_mode'
            mkp = margin_kp_pairs.get()
            if mkp:
                cmd += f' --margin_keypoint_pairs "{mkp}"'
            cmd += f' --margin_arc_select {margin_arc_select.get()}'
            cmd += f' --margin_split_method {margin_split_method.get()}'
            cmd += f' --num_dorsal_landmarks {num_dorsal_landmarks.get()}'
            cmd += f' --outlier_sd_threshold {outlier_sd.get()}'
            if remove_outliers.get():
                cmd += ' --remove_outliers'
            cmd += ' --pre_align_full_contour'                
            if margin_split_method.get() == 'keypoints':
                cmd += ' --require_keypoints_and_segmentation'

        # common flags
        cmd += f' --num_landmarks {num_landmarks.get()}'
        cmd += f' --anchor_method {anchor_method.get()}'
        cmd += f' --alignment_method {alignment_method.get()}'
        if 'V42' in semi_script.name and slide_method.get() != 'none':
            cmd += f' --slide_method {slide_method.get()}'

        if slide_semilandmarks.get() and mode != 'combined':
            cmd += ' --slide_semilandmarks'
            cmd += f' --slide_tolerance {slide_tolerance.get()}'
            cmd += f' --slide_iterations {slide_iterations.get()}'

        kp_idx_val = keypoints_indices.get()
        if kp_idx_val:
            cmd += f' --keypoints_indices "{kp_idx_val}"'
        if require_kp_and_seg.get() and mode not in ('combined', 'dorsal_margin'):
            cmd += ' --require_keypoints_and_segmentation'

        lc = line_categories.get()
        if lc:
            cmd += f' --line_categories "{lc}"'

        gl = group_labels.get()
        if gl:
            cmd += f' --group_labels "{gl}"'
        if perform_manova.get():
            cmd += ' --perform_manova'

        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running V34 GPA Analysis")
                tk.Label(loading, text=f"Mode: {mode}\nPlease wait...").pack(padx=20, pady=20)
                loading.update()
                subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
                loading.destroy()
                messagebox.showinfo("Success", f"V34 GPA ({mode}) completed successfully.")
            except subprocess.CalledProcessError as e:
                try: loading.destroy()
                except: pass
    ...
    btn_frame.grid(row=row, column=0, columnspan=3, pady=10); row += 1
    tk.Button(btn_frame, text="Run Analysis", command=submit, bg="lightgreen",
...
def run_color_segmentation_script():
    """
    Opens a dialog to run the FHS + CLAHE + LAB averaging + pattern
    analysis pipeline (V20) with all options exposed.
    """
    input_window = tk.Toplevel()
    input_window.title("Run Color Segmentation & Pattern Analysis (V20)")
    input_window.geometry("750x650")
    ...
    # pipeline steps
    run_segmentation  = tk.BooleanVar(value=True)
    run_normalization = tk.BooleanVar(value=True)
    remove_shine      = tk.BooleanVar(value=True)
    run_lab_average   = tk.BooleanVar(value=True)
    visualize         = tk.BooleanVar(value=True)
    skip_preprocess   = tk.BooleanVar(value=False)
    
    # threshold mode
    threshold_mode = tk.StringVar(value='median')  # median, adaptive, fixed
    lab_threshold  = tk.DoubleVar(value=25.0)
 
    # pattern analysis
    run_pattern    = tk.BooleanVar(value=True)
    pattern_viz    = tk.BooleanVar(value=True)
    gran_bands     = tk.IntVar(value=20)
    gran_max_px    = tk.IntVar(value=100)
 
    # EFA
    run_efa        = tk.BooleanVar(value=False)
    efa_harmonics  = tk.IntVar(value=20)
    efa_min_area   = tk.IntVar(value=100)
    efa_viz        = tk.BooleanVar(value=False)
    ...
    tk.Label(sf, text="Input Masks Dir (required):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=input_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_directory(input_dir)).grid(
        row=row, column=2, padx=5); row += 1
 
    tk.Label(sf, text="Output Directory (required):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=output_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(sf, text="Browse", command=lambda: browse_directory(output_dir)).grid(
        row=row, column=2, padx=5); row += 1
 
    tk.Label(sf, text="Category name (for phylo CSV):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=category_name, width=30).grid(row=row, column=1, sticky='w', padx=5); row += 1
 
    # --- Pipeline Steps ---
    tk.Label(sf, text="═══ Pipeline Steps ═══", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1
 
    tk.Checkbutton(sf, text="Remove shine (inpainting)", variable=remove_shine).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Felzenszwalb segmentation", variable=run_segmentation).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="CLAHE normalization", variable=run_normalization).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="LAB averaging (colour clustering)", variable=run_lab_average).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Visualize (dendrograms, scatters)", variable=visualize).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="SKIP preprocessing (use existing fhs_* files)", variable=skip_preprocess, fg='red').grid(
        row=row, column=1, sticky='w', padx=5); row += 1
        
    # --- Threshold Mode ---
    tk.Label(sf, text="═══ ΔE Threshold Mode ═══", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1
 
    for label, val in [("Median-calibrated (recommended)", "median"),
                       ("Per-image adaptive (silhouette)", "adaptive"),
                       ("Fixed threshold", "fixed")]:
        tk.Radiobutton(sf, text=label, variable=threshold_mode, value=val).grid(
            row=row, column=0, columnspan=3, sticky='w', padx=20, pady=1); row += 1
 
    tk.Label(sf, text="Fixed ΔE (if fixed mode):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=lab_threshold, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
 
    # --- Pattern Analysis ---
    tk.Label(sf, text="═══ Pattern Analysis ═══", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1
 
    tk.Checkbutton(sf, text="FFT granularity + colour adjacency", variable=run_pattern).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="Pattern visualizations", variable=pattern_viz).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Granularity bands:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=gran_bands, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="Max filter size (px):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=gran_max_px, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
 
    # --- EFA ---
    tk.Label(sf, text="═══ EFA Marking Shape ═══", font=("", 10, "bold")).grid(
        row=row, column=0, columnspan=3, sticky='w', padx=5, pady=(10,2)); row += 1
 
    tk.Checkbutton(sf, text="Run Elliptical Fourier Analysis", variable=run_efa).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Checkbutton(sf, text="EFA overlay visualization", variable=efa_viz).grid(
        row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="EFA harmonics:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=efa_harmonics, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    tk.Label(sf, text="EFA min area (px²):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(sf, textvariable=efa_min_area, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
 
    # --- Submit ---
    def submit():
        input_val = input_dir.get()
        output_val = output_dir.get()
        if not input_val:
        ...
        script_dir = Path(__file__).parent
        # === DESCRIPTRON_PATCH: V21_LOOKUP ===
        # V21 (with hue-scale + correct HSV source) preferred; V20 fallback
        color_script = script_dir / 'measure' / 'FHS_and_CLAHE_V21.py'
        if not color_script.is_file():
            color_script = script_dir / 'FHS_and_CLAHE_V21.py'
        if not color_script.is_file():
            color_script = script_dir / 'measure' / 'FHS_and_CLAHE_V20.py'
        if not color_script.is_file():
            color_script = script_dir / 'FHS_and_CLAHE_V20.py'
        # === END DESCRIPTRON_PATCH: V21_LOOKUP ===
        if not color_script.is_file():
            messagebox.showerror("Script Error",
                                 f"V20 color script not found at:\n{color_script}")
            return
 
        cmd = (f'conda run -n measure_env python "{color_script}"'
               f' --input_dir "{input_val}"'
               f' --output_dir "{output_val}"')
 
        if skip_preprocess.get():
            cmd += ' --skip_preprocessing --run_lab_average'
        else:
            if remove_shine.get():
                cmd += ' --remove_shine'
            if run_segmentation.get():
                cmd += ' --run_segmentation'
            if run_normalization.get():
                cmd += ' --run_normalization'
            if run_lab_average.get():
                cmd += ' --run_lab_average'
            if visualize.get():
                cmd += ' --visualize'
#        if remove_shine.get():
...
#            cmd += ' --visualize'
        ...
        # threshold mode
        mode = threshold_mode.get()
        if mode == 'median':
            cmd += ' --median_threshold'
        elif mode == 'adaptive':
            cmd += ' --adaptive_threshold'
        else:
            cmd += f' --lab_threshold {lab_threshold.get()}'
 
        # pattern analysis
        if run_pattern.get():
            cmd += ' --run_pattern_analysis'
            cmd += f' --granularity_bands {gran_bands.get()}'
            cmd += f' --granularity_max_px {gran_max_px.get()}'
        if pattern_viz.get():
            cmd += ' --pattern_viz'
        ...
        if run_efa.get():
            cmd += ' --run_efa'
            cmd += f' --efa_harmonics {efa_harmonics.get()}'
            cmd += f' --efa_min_area {efa_min_area.get()}'
        if efa_viz.get():
            cmd += ' --efa_viz'
        ...
        # LOG THE COMMAND
        print(f"\n{'='*60}")
        print(f"COLOR SEGMENTATION COMMAND:")
        print(f"{'='*60}")
        print(cmd)
        print(f"{'='*60}\n")
        ...
        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running Color Segmentation & Pattern Analysis")
                tk.Label(loading, text="FHS → CLAHE → LAB averaging → pattern analysis...\nPlease wait...").pack(padx=20, pady=20)
                loading.update()
                subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
                loading.destroy()
                messagebox.showinfo("Success", "Color segmentation & pattern analysis completed.")
            except subprocess.CalledProcessError as e:
                try: loading.destroy()
                except: pass
                messagebox.showerror("Error", f"Script error:\n{e}")
...
def run_texture_phenomics_script():
    """
    Opens a dialog to collect inputs and runs the texture phenomics script.
    Extracts GLCM + LBP + granularity + color features per category from COCO JSON,
    then runs PCA / UMAP / clustering for morphospace visualisation.
    """
    input_window = tk.Toplevel()
    input_window.title("Run Texture Phenomics Analysis")
    input_window.geometry("680x520")

    json_path    = tk.StringVar()
    image_dir    = tk.StringVar()
    output_dir   = tk.StringVar(value='./texture_out')
    category_name = tk.StringVar()
    tree_path    = tk.StringVar()
    group_labels = tk.StringVar()
    gran_bands   = tk.IntVar(value=20)
    gran_max_px  = tk.IntVar(value=80)
    n_clusters   = tk.IntVar(value=3)
    umap_nn      = tk.IntVar(value=15)
    ...
    row = 0
    tk.Label(input_window, text="COCO JSON File:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=json_path, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(
        json_path, [("JSON", "*.json"), ("All", "*.*")])).grid(row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Image Directory:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=image_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(image_dir)).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Output Directory:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=output_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(output_dir)).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Category (blank=all):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=category_name, width=50).grid(row=row, column=1, padx=5, pady=3); row += 1

    tk.Label(input_window, text="Newick tree (optional):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=tree_path, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(
        tree_path, [("Newick", "*.nwk *.tre *.tree"), ("All", "*.*")])).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Group labels CSV (optional):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=group_labels, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(
        group_labels, [("CSV/TSV", "*.csv *.tsv"), ("All", "*.*")])).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Granularity bands:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=gran_bands, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(input_window, text="Granularity max px:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=gran_max_px, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(input_window, text="Num clusters:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=n_clusters, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(input_window, text="UMAP neighbors:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=umap_nn, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    workers = tk.IntVar(value=0)
    tk.Label(input_window, text="Workers (0=auto):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=workers, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    ...
    def submit():
        ...
        script_dir = Path(__file__).parent
        tex_script = script_dir / 'measure' / 'texture_phenomics_from_coco.py'
        if not tex_script.is_file():
            tex_script = script_dir / 'texture_phenomics_from_coco.py'
        if not tex_script.is_file():
            messagebox.showerror("Script Error",
                                 f"Texture script not found at:\n{tex_script}")
            return
        ...
        cmd = ('conda run -n measure_env python'
               ...
               f' --json "{json_val}"'
               f' --image_dir "{image_dir_val}"'
               f' --output_dir "{output_dir.get() or "./texture_out"}"'
               )
        ...
        tree_val = tree_path.get()
        if tree_val:
            cmd += f' --tree "{tree_val}"'

        gl_val = group_labels.get()
        if gl_val:
            cmd += f' --group_labels "{gl_val}"'

        cmd += f' --granularity_bands {gran_bands.get()}'
        cmd += f' --granularity_max_px {gran_max_px.get()}'
        cmd += f' --n_clusters {n_clusters.get()}'
        cmd += f' --umap_n_neighbors {umap_nn.get()}'
        cmd += f' --workers {workers.get()}'

        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running Texture Phenomics")
                tk.Label(loading, text="Extracting GLCM + LBP + granularity + color...\nPlease wait...").pack(padx=20, pady=20)
                loading.update()
                subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
                loading.destroy()
                messagebox.showinfo("Success", "Texture phenomics analysis completed successfully.")
            except subprocess.CalledProcessError as e:
                try: loading.destroy()
                except: pass
                messagebox.showerror("Error", f"Script error:\n{e}")
    ...
    tk.Button(input_window, text="Run Analysis", command=submit, bg="lightgreen").grid(
        row=row, column=1, sticky='e', padx=5, pady=10)
    tk.Button(input_window, text="Cancel", command=input_window.destroy).grid(
        row=row, column=2, sticky='w', padx=5, pady=10)


def run_texture_homology_script():
    """
    Opens a dialog to run the homology-aware texture phenomics script.
    Uses TPS-warped quadrilateral grid cells from V34 GPA output.
    Requires: V34 GPA output dir + original COCO JSON + image directory.
    """
    input_window = tk.Toplevel()
    input_window.title("Run Texture Phenomics (Homology-Aware TPS Grid)")
    input_window.geometry("700x500")

    gpa_dir       = tk.StringVar()
    json_path     = tk.StringVar()
    image_dir     = tk.StringVar()
    output_dir    = tk.StringVar(value='./texture_homology_out')
    category_name = tk.StringVar()
    grid_rows     = tk.IntVar(value=8)
    grid_cols     = tk.IntVar(value=10)
    n_clusters    = tk.IntVar(value=3)
    umap_nn       = tk.IntVar(value=15)
    workers       = tk.IntVar(value=0)
    viz_specimens = tk.IntVar(value=5)
    ...
    row = 0
    tk.Label(input_window, text="V34 GPA Output Dir (required):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=gpa_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(gpa_dir)).grid(
        row=row, column=2, padx=5); row += 1

    tk.Label(input_window, text="Original COCO JSON (for masks):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=json_path, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(
        json_path, [("JSON", "*.json"), ("All", "*.*")])).grid(row=row, column=2, padx=5); row += 1
    ...
    tk.Label(input_window, text="Category (must match COCO):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=category_name, width=50).grid(row=row, column=1, padx=5, pady=3); row += 1

    tk.Label(input_window, text="Grid rows:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=grid_rows, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1

    tk.Label(input_window, text="Grid cols:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=grid_cols, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    ...
    tk.Label(input_window, text="Grid overlay specimens:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=viz_specimens, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    ...
    def submit():
        ...
        image_dir_val = image_dir.get()
        if not gpa_val:
            messagebox.showerror("Input Error", "Please select the V34 GPA output directory.")
            return
        if not json_val:
            messagebox.showerror("Input Error", "Please select the original COCO JSON.")
            return
        if not image_dir_val:
        ...
        script_dir = Path(__file__).parent
        tex_script = script_dir / 'measure' / 'texture_phenomics_homology.py'
        if not tex_script.is_file():
            tex_script = script_dir / 'texture_phenomics_homology.py'
        if not tex_script.is_file():
            messagebox.showerror("Script Error",
                                 f"Homology texture script not found at:\n{tex_script}")
            return
        ...
        cmd = ('conda run -n measure_env python'
               ...
               f' --json "{json_val}"'
               f' --image_dir "{image_dir_val}"'
               f' --output_dir "{output_dir.get() or "./texture_homology_out"}"'
               )
        ...
        cmd += f' --grid_rows {grid_rows.get()}'
        cmd += f' --grid_cols {grid_cols.get()}'
        cmd += f' --n_clusters {n_clusters.get()}'
        cmd += f' --workers {workers.get()}'
        cmd += f' --viz_specimens {viz_specimens.get()}'
        cmd += f' --umap_n_neighbors {umap_nn.get()}'

        # LOG THE COMMAND to console
        print(f"\n{'='*60}")
        print(f"TEXTURE HOMOLOGY COMMAND:")
        print(f"{'='*60}")
        print(cmd)
        print(f"{'='*60}\n")
        ...
        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running Homology Texture Phenomics")
                tk.Label(loading, text="TPS quad grid warping + texture extraction...\nPlease wait...").pack(padx=20, pady=20)
                loading.update()
                subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
                loading.destroy()
                messagebox.showinfo("Success", "Homology-aware texture phenomics completed.")
            except subprocess.CalledProcessError as e:
                try: loading.destroy()
                except: pass
                messagebox.showerror("Error", f"Script error:\n{e}")
...
def run_color_homology_script():
    """
    Opens a dialog to run the homology-aware color phenomics script.
    Uses TPS-warped quadrilateral grid cells from V34 GPA output.
    Can point to raw images or V20 color-segmented output.
    """
    input_window = tk.Toplevel()
    input_window.title("Run Color Phenomics (Homology-Aware TPS Grid)")
    input_window.geometry("700x480")
 
    gpa_dir       = tk.StringVar()
    json_path     = tk.StringVar()
    image_dir     = tk.StringVar()
    output_dir    = tk.StringVar(value='./color_homology_out')
    category_name = tk.StringVar()
    grid_rows     = tk.IntVar(value=8)
    grid_cols     = tk.IntVar(value=10)
    n_clusters    = tk.IntVar(value=0)
    workers       = tk.IntVar(value=0)
    viz_specimens = tk.IntVar(value=5)
    color_bins    = tk.IntVar(value=8)
    # === DESCRIPTRON_PATCH: COLOR_MODE_VARS ===
    color_mode    = tk.StringVar(value='combined')  # pattern/hue/combined
    hue_weight    = tk.DoubleVar(value=1.0)         # block variance ratio
    # === END DESCRIPTRON_PATCH: COLOR_MODE_VARS ===
    ...
    tk.Label(input_window, text="Image Dir (raw or V20 output):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=image_dir, width=50).grid(row=row, column=1, padx=5, pady=3)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(image_dir)).grid(
        row=row, column=2, padx=5); row += 1
    ...
    tk.Label(input_window, text="Color bins (entropy):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=color_bins, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
 
    tk.Label(input_window, text="Clusters (0=auto silhouette):").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=n_clusters, width=10).grid(row=row, column=1, sticky='w', padx=5); row += 1
    ...
    # === DESCRIPTRON_PATCH: COLOR_MODE_WIDGETS ===
    tk.Label(input_window, text="Color mode:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    try:
        from tkinter import ttk as _ttk
        _color_mode_combo = _ttk.Combobox(input_window, textvariable=color_mode,
            values=['pattern', 'hue', 'combined'], state='readonly', width=15)
        _color_mode_combo.grid(row=row, column=1, sticky='w', padx=5)
    except Exception:
        tk.Entry(input_window, textvariable=color_mode, width=15).grid(
            row=row, column=1, sticky='w', padx=5)
    row += 1

    tk.Label(input_window, text="Hue weight (combined only):").grid(
        row=row, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(input_window, textvariable=hue_weight, width=10).grid(
        row=row, column=1, sticky='w', padx=5)
    row += 1
    # === END DESCRIPTRON_PATCH: COLOR_MODE_WIDGETS ===
    def submit():
        gpa_val = gpa_dir.get()
        json_val = json_path.get()
        image_dir_val = image_dir.get()
        ...
        script_dir = Path(__file__).parent
        # === DESCRIPTRON_PATCH: COLOR_V2_LOOKUP ===
        # v2 (with color_mode pattern/hue/combined) preferred; v1 fallback
        color_script = script_dir / 'measure' / 'color_phenomics_homology_v2_1.py'
        if not color_script.is_file():
            color_script = script_dir / 'color_phenomics_homology_v2_1.py'
        if not color_script.is_file():
            color_script = script_dir / 'measure' / 'color_phenomics_homology.py'
        if not color_script.is_file():
            color_script = script_dir / 'color_phenomics_homology.py'
        # === END DESCRIPTRON_PATCH: COLOR_V2_LOOKUP ===
        if not color_script.is_file():
            messagebox.showerror("Script Error",
                                 f"Color homology script not found at:\n{color_script}")
            return
        ...
        cmd = ('conda run -n measure_env python'
               ...
               f' --json "{json_val}"'
               f' --image_dir "{image_dir_val}"'
               f' --output_dir "{output_dir.get() or "./color_homology_out"}"'
               )
        ...
        cmd += f' --grid_rows {grid_rows.get()}'
        cmd += f' --grid_cols {grid_cols.get()}'
        cmd += f' --landmark_step 5'
        cmd += f' --color_bins {color_bins.get()}'
        cmd += f' --n_clusters {n_clusters.get()}'
        cmd += f' --workers {workers.get()}'
        cmd += f' --viz_specimens {viz_specimens.get()}'

        # === DESCRIPTRON_PATCH: COLOR_MODE_CMD ===
        # v2 additions: only appended if the GUI vars exist (patched run)
        try:
            cmd += f' --color_mode {color_mode.get()}'
            if color_mode.get() == 'combined':
                cmd += f' --hue_weight {hue_weight.get()}'
        except NameError:
            pass  # v1 color_phenomics_homology.py — no mode flag needed
        # === END DESCRIPTRON_PATCH: COLOR_MODE_CMD ===
 
        # LOG THE COMMAND
        print(f"\n{'='*60}")
        print(f"COLOR HOMOLOGY COMMAND:")
        print(f"{'='*60}")
        print(cmd)
        print(f"{'='*60}\n")
        ...
        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running Color Phenomics (Homology)")
                tk.Label(loading, text="TPS grid color extraction...\nPlease wait...").pack(padx=20, pady=20)
                loading.update()
                subprocess.run(cmd, shell=True, executable='/bin/bash', check=True)
                loading.destroy()
                messagebox.showinfo("Success", "Homology-aware color phenomics completed.")
            except subprocess.CalledProcessError as e:
                try: loading.destroy()
                except: pass
                messagebox.showerror("Error", f"Script error:\n{e}")
...
def run_build_treatment_docx_script():
    """
    Opens a dialog to collect inputs and runs measure/build_species_treatment_docx.py,
    which assembles a review-ready DOCX taxonomic treatment (Material examined,
    Diagnosis, Description, dichotomous Key, range maps and species plates) from the
    final (post-deconfabulation) TOWLEY/BioRAG descriptions.
    """
    input_window = tk.Toplevel()
    input_window.title("Build Species Treatment DOCX")
    input_window.geometry("760x520")

    # User inputs
    descriptions_dir = tk.StringVar()
    plates_dir = tk.StringVar()
    key_file = tk.StringVar()
    localities_path = tk.StringVar()
    output_docx = tk.StringVar()
    genus = tk.StringVar()
    title = tk.StringVar()
    authors = tk.StringVar()
    map_mode = tk.StringVar(value='simple')
    ocr_labels = tk.BooleanVar(value=False)
    emit_template = tk.BooleanVar(value=False)

    def browse_file(var, filetypes, ttl):
        p = filedialog.askopenfilename(title=ttl, filetypes=filetypes)
        if p:
            var.set(p)

    def browse_directory(var, ttl):
        p = filedialog.askdirectory(title=ttl)
        if p:
            var.set(p)

    def save_as_file(var, filetypes, ttl):
        p = filedialog.asksaveasfilename(title=ttl, defaultextension=".docx",
                                         filetypes=filetypes)
        if p:
            var.set(p)

    row = 0
    tk.Label(input_window, text="Descriptions Dir (Required):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=descriptions_dir, width=55).grid(row=row, column=1, padx=5, pady=5)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(descriptions_dir, "Select Descriptions Directory (contains {species}/{species}.txt)")).grid(row=row, column=2, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Species Plates Dir (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=plates_dir, width=55).grid(row=row, column=1, padx=5, pady=5)
    tk.Button(input_window, text="Browse", command=lambda: browse_directory(plates_dir, "Select Species Plates Directory")).grid(row=row, column=2, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Taxonomic Key File (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=key_file, width=55).grid(row=row, column=1, padx=5, pady=5)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(key_file, [("Text Files", "*.txt"), ("All Files", "*.*")], "Select taxonomic_key.txt")).grid(row=row, column=2, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Localities CSV/TSV (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=localities_path, width=55).grid(row=row, column=1, padx=5, pady=5)
    tk.Button(input_window, text="Browse", command=lambda: browse_file(localities_path, [("CSV/TSV", "*.csv *.tsv"), ("All Files", "*.*")], "Select localities CSV/TSV")).grid(row=row, column=2, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Output DOCX (Required):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=output_docx, width=55).grid(row=row, column=1, padx=5, pady=5)
    tk.Button(input_window, text="Save As", command=lambda: save_as_file(output_docx, [("Word Document", "*.docx"), ("All Files", "*.*")], "Save Treatment DOCX As")).grid(row=row, column=2, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Genus (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=genus, width=55).grid(row=row, column=1, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Title (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=title, width=55).grid(row=row, column=1, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Authors (Optional):").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(input_window, textvariable=authors, width=55).grid(row=row, column=1, padx=5, pady=5)

    row += 1
    tk.Label(input_window, text="Map Mode:").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    tk.OptionMenu(input_window, map_mode, 'auto', 'simple', 'none').grid(row=row, column=1, sticky='w', padx=5, pady=5)

    row += 1
    tk.Checkbutton(input_window, text="Read specimen labels with Florence-2 (--ocr-labels)",
                   variable=ocr_labels).grid(row=row, column=1, sticky='w', padx=5, pady=5)

    row += 1
    tk.Checkbutton(input_window,
                   text="Emit blank locality template CSV instead (fill Output with a .csv path)",
                   variable=emit_template).grid(row=row, column=1, sticky='w', padx=5, pady=5)

    def _run_treatment_cmd(cmd, out_path, what):
        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running Treatment Builder")
                tk.Label(loading, text=f"Please wait while the {what} is generated...").pack(padx=20, pady=20)
                loading.update()
                process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                ...
                def read_output():
                    ...
                    loading.destroy()
                    if process.returncode == 0:
                        logging.info(f"{what} completed successfully.\n{stdout}")
                        messagebox.showinfo("Success", f"{what} written to:\n{out_path}\n\n{stdout.strip()[-400:]}")
                    else:
                        logging.error(f"Error building {what}:\n{stderr}")
                        messagebox.showerror("Error", f"{what} failed:\n{stderr[-1000:]}")
                ...
                try:
                    ...
                    pass
                logging.exception("Unexpected error in run_build_treatment_docx_script")
                messagebox.showerror("Error", f"An unexpected error occurred:\n{e}")

        threading.Thread(target=run_script).start()

    def submit():
        descriptions_val = descriptions_dir.get().strip()
        output_val = output_docx.get().strip()
        if not descriptions_val:
            messagebox.showerror("Input Error", "Please select a Descriptions directory.")
            return
        if not output_val:
            messagebox.showerror("Input Error", "Please specify an Output path.")
            return

        script_dir = Path(__file__).parent
        docx_script = script_dir / 'measure' / 'build_species_treatment_docx.py'
        if not docx_script.is_file():
            messagebox.showerror("Script Error", f"Treatment DOCX script not found at: {docx_script}")
            return

        # Template-only mode: just enumerate specimens into a blank localities CSV.
        if emit_template.get():
            cmd = [
                "conda", "run", "-n", "measure_env", "python",
                str(docx_script),
                '--descriptions-dir', descriptions_val,
                '--emit-locality-template', output_val,
            ]
            _run_treatment_cmd(cmd, output_val, "locality template CSV")
            input_window.destroy()
            return
        ...
        cmd = [
            ...
            '--descriptions-dir', descriptions_val,
            '--output', output_val,
            '--map-mode', map_mode.get().strip() or 'simple',
        ]
        if plates_dir.get().strip():
            cmd += ['--plates-dir', plates_dir.get().strip()]
        if key_file.get().strip():
            cmd += ['--key-file', key_file.get().strip()]
        if localities_path.get().strip():
            cmd += ['--localities', localities_path.get().strip()]
        if genus.get().strip():
            cmd += ['--genus', genus.get().strip()]
        if title.get().strip():
            cmd += ['--title', title.get().strip()]
        if authors.get().strip():
            cmd += ['--authors', authors.get().strip()]
        if ocr_labels.get():
            cmd += ['--ocr-labels']

        _run_treatment_cmd(cmd, output_val, "treatment DOCX")
        input_window.destroy()
...
def open_pal_popup():
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    v21_exists = os.path.exists(os.path.join(script_dir, 'sam2_pal_batch_v21.py'))
    v20_exists = os.path.exists(os.path.join(script_dir, 'sam2_pal_batch_v20.py'))
    v19_exists = os.path.exists(os.path.join(script_dir, 'sam2_pal_batch_v19.py'))
    ...
    if v21_exists:
        # v21 letterboxes every frame instead of stretching targets to the template
        # size, which is the fix for the template/target dimension mismatch
        pal_script_name = 'sam2_pal_batch_v21.py'
        version_str = "v21"
    elif v20_exists:
        pal_script_name = 'sam2_pal_batch_v20.py'
        version_str = "v20"
    elif v19_exists:
        pal_script_name = 'sam2_pal_batch_v19.py'
        version_str = "v19"
    elif v18_exists:
...
# ============================================================================
# ONTOLOGY ROSETTA — GUI Functions
# ============================================================================

def open_template_popup():
    """Open a popup to select and load a morphology ontology template."""
    global categories, category_name_to_id, category_id_to_supercategory, category_id_to_name
    global coco_output_mask, coco_output_accumulate

    if not _ONTOLOGY_AVAILABLE:
        messagebox.showerror("Error", "Ontology module not available.\nPlace descriptron_ontology.py and descriptron_ontology_templates.json in the gui/ folder.")
        return

    popup = tk.Toplevel(root)
    popup.title("Load Morphology Template")
    popup.geometry("500x450")

    groups = _onto_mgr.get_template_groups()

    ttk.Label(popup, text="Taxonomic Group:", font=("Arial", 10, "bold")).pack(pady=(10, 2))
    group_var = tk.StringVar(value=groups[0] if groups else "")
    group_combo = ttk.Combobox(popup, textvariable=group_var, values=groups, state="readonly", width=35)
    group_combo.pack(pady=5)

    ttk.Label(popup, text="View:", font=("Arial", 10, "bold")).pack(pady=(10, 2))
    view_var = tk.StringVar()
    view_combo = ttk.Combobox(popup, textvariable=view_var, state="readonly", width=35)
    view_combo.pack(pady=5)

    # Region preview
    preview_frame = ttk.LabelFrame(popup, text="Template Preview", padding=5)
    preview_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
    preview_text = tk.Text(preview_frame, height=12, font=("Courier", 9), state="disabled")
    preview_text.pack(fill=tk.BOTH, expand=True)

    def update_views(*_):
        g = group_var.get()
        views = _onto_mgr.get_views_for_group(g)
        view_combo['values'] = views
        if views:
            view_var.set(views[0])
            update_preview()

    def update_preview(*_):
        tmpl = _onto_mgr.get_template(group_var.get(), view_var.get())
        preview_text.configure(state="normal")
        preview_text.delete("1.0", tk.END)
        if tmpl:
            for r in tmpl.get("regions", []):
                ont = r.get("ontology", "")
                ont_str = f"  [{ont}]" if ont else ""
                preview_text.insert(tk.END, f"  {r['label']}{ont_str}\n")
            attrs = tmpl.get("attributes", {})
            if attrs:
                preview_text.insert(tk.END, f"\nAttributes: {', '.join(attrs.keys())}\n")
        preview_text.configure(state="disabled")

    group_combo.bind("<<ComboboxSelected>>", update_views)
    view_combo.bind("<<ComboboxSelected>>", update_preview)

    def do_load():
        g = group_var.get()
        v = view_var.get()
        if not g or not v:
            messagebox.showerror("Error", "Select a group and view")
            return

        cats, meta = _onto_mgr.load_template(g, v)

        # Populate global Descriptron categories
        categories.clear()
        category_name_to_id.clear()
        category_id_to_supercategory.clear()
        category_id_to_name.clear()
        ...
        for cat in cats:
            ...
            m = meta.get(cat['name'], {})
            category_id_to_supercategory[cat['id']] = cat.get('supercategory', 'sclerite')

        coco_output_mask["categories"] = [c.copy() for c in categories]
        coco_output_accumulate["categories"] = [c.copy() for c in categories]

        update_label_dropdown()

        n_ont = sum(1 for name in meta if meta[name].get('ontology'))
        messagebox.showinfo("Template Loaded",
                            f"Loaded {g} / {v}\n"
                            f"{len(cats)} categories ({n_ont} with ontology URIs)\n"
                            f"Attributes: {', '.join(list(meta.get(cats[0]['name'], {}).get('attributes', {}).keys())[:5])}")
        popup.destroy()

    ttk.Button(popup, text="Load Template", command=do_load).pack(pady=10)
...
def import_gbif_coco():
    """Import a GBIF annotator COCO JSON export with full ontology metadata."""
    global segmentation_masks, current_mask_index, labels, image
    global categories, category_name_to_id, category_id_to_supercategory, category_id_to_name
    global coco_output_mask, coco_output_accumulate
    ...
    json_path = filedialog.askopenfilename(
        title="Import GBIF Annotator COCO JSON",
        filetypes=[("JSON Files", "*.json")]
    )
    if not json_path:
        return
    ...
    # Use the ontology manager to parse
    cats, meta, ann_attrs, annotations = _onto_mgr.import_gbif_coco(coco_data)

    # Populate global categories
    categories.clear()
    category_name_to_id.clear()
    category_id_to_supercategory.clear()
    category_id_to_name.clear()
    ...
    # Now load the annotations using existing load_json_contours logic
    # but only for geometric annotations (skip is_trait_only)
    segmentation_masks = []
    labels = {}

    for ann in annotations:
        if ann.get('is_trait_only', False):
            continue  # attribute-only, no geometry
        ...
        if 'segmentation' in ann:
            ...
            if isinstance(seg, list) and len(seg) > 0 and isinstance(seg[0], list):
                ...
                if image is not None:
                    ...
                    mask_data = {'segmentation': mask, 'category_id': cat_id}

                    # Preserve line fields
                    if ann.get('is_line'):
                        mask_data['is_line'] = True
                        if 'line_points' in ann:
                            mask_data['line_points'] = ann['line_points']
                        if 'line_thickness' in ann:
                            mask_data['line_thickness'] = ann['line_thickness']
                        if 'num_line_keypoints' in ann:
                            mask_data['num_line_keypoints'] = ann['num_line_keypoints']

                    segmentation_masks.append(mask_data)
                    idx = len(segmentation_masks) - 1
                    labels[idx] = category_id_to_name.get(cat_id, 'unlabeled')

    n_attrs = sum(1 for v in ann_attrs.values() if v)
    n_ont = sum(1 for m in meta.values() if m.get('ontology'))
    template_str = f"\nTemplate: {_onto_mgr.current_group}/{_onto_mgr.current_view}" if _onto_mgr.current_group else ""

    messagebox.showinfo("GBIF COCO Import",
                        f"Imported from: {os.path.basename(json_path)}\n"
                        f"{len(cats)} categories ({n_ont} with ontology URIs)\n"
                        f"{len(segmentation_masks)} geometric annotations\n"
                        f"{n_attrs} annotations with attributes"
                        f"{template_str}")

    if segmentation_masks:
        current_mask_index = 0
        apply_zoom()
        redraw_masks(multi=multi_mask_mode)


def export_enhanced_coco():
    """Export enhanced COCO JSON with ontology metadata."""
    ...
    if not coco_output_accumulate.get('annotations'):
        messagebox.showerror("Error", "No annotations to export!")
        return

    # Ask for GBIF occurrence ID (optional)
    gbif_id = simpledialog.askstring("GBIF ID", "GBIF Occurrence ID (optional):", parent=root)
    gbif_id = gbif_id.strip() if gbif_id else ""

    # Build the base COCO (same as save_final_via2_json but without saving)
    trash_cat_id = None
    for cat in coco_output_accumulate['categories']:
        if cat['name'] == 'Trash':
            trash_cat_id = cat['id']
    ...
    filtered = [a for a in coco_output_accumulate['annotations']
                if trash_cat_id is None or a.get('category_id') != trash_cat_id]
    ...
    enhanced = _onto_mgr.export_enhanced_coco(base_coco, gbif_occurrence_id=gbif_id)

    save_path = filedialog.asksaveasfilename(
        title="Save Enhanced COCO JSON",
        defaultextension=".json",
        initialfile=f"{file_path_base}_enhanced_coco.json" if file_path_base else "enhanced_coco.json",
        filetypes=[("JSON Files", "*.json")]
    )
    if save_path:
        ...
        n_ont = sum(1 for c in enhanced['categories'] if c.get('ontology'))
        n_attr = sum(1 for a in enhanced['annotations'] if a.get('attributes'))
        has_vocab = 'attribute_vocabulary' in enhanced

        messagebox.showinfo("Enhanced COCO Saved",
                            f"Saved to: {os.path.basename(save_path)}\n"
                            f"{len(enhanced['annotations'])} annotations\n"
                            f"{n_ont} categories with ontology URIs\n"
                            f"{n_attr} annotations with attributes\n"
                            f"Attribute vocabulary: {'Yes' if has_vocab else 'No'}")


def export_knowledge_graph():
    """Export a JSON-LD knowledge graph from current annotations."""
    global coco_output_accumulate, file_path_base
    ...
    # Gather metadata
    specimen_id = simpledialog.askstring("Specimen ID", "Specimen identifier:", parent=root)
    specimen_id = specimen_id.strip() if specimen_id else os.path.basename(file_path_base or "unknown")

    sci_name = simpledialog.askstring("Taxon", "Scientific name (optional):", parent=root)
    sci_name = sci_name.strip() if sci_name else ""

    # Build COCO for the knowledge graph
    trash_cat_id = None
    for cat in coco_output_accumulate['categories']:
        if cat['name'] == 'Trash':
            trash_cat_id = cat['id']
    ...
    # Enhance first (to get ontology on categories)
    enhanced = _onto_mgr.export_enhanced_coco(base_coco)

    kg = _onto_mgr.export_knowledge_graph(
        enhanced,
    ...
    save_path = filedialog.asksaveasfilename(
        title="Save Knowledge Graph (JSON-LD)",
        defaultextension=".json",
        initialfile=f"{specimen_id}_knowledge_graph.json",
        filetypes=[("JSON-LD", "*.json *.jsonld")]
    )
    if save_path:
        with open(save_path, 'w') as f:
            json.dump(kg, f, indent=2)

        messagebox.showinfo("Knowledge Graph Saved",
                            f"Saved to: {os.path.basename(save_path)}\n"
                            f"{kg['summary']['nodeCount']} nodes\n"
                            f"{kg['summary']['edgeCount']} edges")


def open_attribute_editor(annotation_id, category_name):
    """Open a popup to set attributes for an annotation (texture, sculpture, etc.).

    # === ENRICHED ATTR EDITOR PATCH ===
    Includes all core attributes for the current taxonomic group (enriched),
    plus a free-text notes field matching the GBIF annotator's r.notes.
    """
    if not _ONTOLOGY_AVAILABLE or _onto_mgr is None:
        return

    attr_names = _onto_mgr.get_attributes_for_category(category_name)
    if not attr_names:
        # Even without template attributes, still offer notes
        pass

    existing = _onto_mgr.get_annotation_attributes(annotation_id)

    popup = tk.Toplevel(root)
    popup.title(f"Attributes: {category_name}")
    ...
    label_text = _onto_mgr.category_label.get(category_name, category_name)
    ttk.Label(header_frame, text=label_text,
              font=("Arial", 11, "bold")).pack(side=tk.LEFT)

    ont_uri = _onto_mgr.category_ontology.get(category_name, "")
    if ont_uri:
        ttk.Label(header_frame, text=f"  [{ont_uri}]",
                  font=("Arial", 9), foreground="#3498db").pack(side=tk.LEFT)

    group_view = ""
    if _onto_mgr.current_group:
        group_view = f"{_onto_mgr.current_group} / {_onto_mgr.current_view}"
    ttk.Label(popup, text=group_view,
              font=("Arial", 8), foreground="gray").pack(padx=10, anchor=tk.W)

    # Scrollable attribute area
    canvas = tk.Canvas(popup, highlightthickness=0)
    sb = ttk.Scrollbar(popup, orient="vertical", command=canvas.yview)
    attr_frame = ttk.Frame(canvas)
    attr_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
    canvas.create_window((0, 0), window=attr_frame, anchor="nw")
    canvas.configure(yscrollcommand=sb.set)

    vars_dict = {}
    row = 0
    for attr_name in attr_names:
        values = _onto_mgr.get_attribute_values(category_name, attr_name)
        if not values:
            continue

        # Attribute label (capitalize, replace underscores)
        display_name = attr_name.replace("_", " ").title()
        ttk.Label(attr_frame, text=f"{display_name}:",
                  font=("Arial", 9)).grid(row=row, column=0, sticky=tk.W, pady=3, padx=(5, 2))

        var = tk.StringVar(value=existing.get(attr_name, ""))
        combo = ttk.Combobox(attr_frame, textvariable=var,
                             values=[""] + values, width=28)
        combo.grid(row=row, column=1, padx=5, pady=3, sticky=tk.W)
        vars_dict[attr_name] = var
        row += 1
    ...
    # ── Notes (free text) ──
    notes_frame = ttk.LabelFrame(popup, text="Notes", padding=5)
    notes_frame.pack(fill=tk.X, padx=10, pady=(0, 5))

    notes_text = tk.Text(notes_frame, height=3, font=("Arial", 9), wrap=tk.WORD)
    ...
    def do_save():
        attrs = {}
        for attr_name, var in vars_dict.items():
            val = var.get().strip()
            if val:
                attrs[attr_name] = val
        # Save notes
        notes_val = notes_text.get("1.0", tk.END).strip()
        if notes_val:
            attrs["_notes"] = notes_val
        _onto_mgr.set_annotation_attributes(annotation_id, attrs)
        popup.destroy()
    ...
    ttk.Button(btn_frame, text="Save Attributes", command=do_save).pack(side=tk.RIGHT, padx=5)
    ttk.Button(btn_frame, text="Clear All", command=do_clear).pack(side=tk.RIGHT, padx=5)
    ttk.Button(btn_frame, text="Cancel", command=popup.destroy).pack(side=tk.RIGHT, padx=5)


def open_biorag_popup():
    """Open popup to configure and run the full BioRAG pipeline (v73: v2 workflow by default)."""
    import tkinter as tk
    from tkinter import filedialog, messagebox
    import subprocess, threading

    popup = tk.Toplevel()
    popup.title("BioRAG Pipeline (v2: evidence-tiered key + treatments)")
    popup.geometry("780x900")

    coco_json_var = tk.StringVar()
    image_dir_var = tk.StringVar()
    group_labels_var = tk.StringVar()
    output_base_var = tk.StringVar()
    family_var = tk.StringVar()
    api_key_file_var = tk.StringVar()
    keypoints_json_var = tk.StringVar()
    pdf_dir_var = tk.StringVar()
    user_prompts_var = tk.StringVar()
    exclude_list_var = tk.StringVar()
    label_dir_var = tk.StringVar()
    model_var = tk.StringVar(value="claude-sonnet-4-6")
    conda_env_var = tk.StringVar(value="measure_env")
    num_landmarks_var = tk.IntVar(value=100)
    k_var = tk.IntVar(value=3)
    force_var = tk.BooleanVar(value=False)
    dry_run_var = tk.BooleanVar(value=False)
    # --- BioRAG v2 options (v73) ---
    workflow_var = tk.StringVar(value="v2")
    taxon_profile_var = tk.StringVar()
    llm_backend_var = tk.StringVar(value="claude-code")
    localities_var = tk.StringVar()
    title_var = tk.StringVar()
    authors_var = tk.StringVar()
    keep_flagged_var = tk.BooleanVar(value=False)
    # --- character reliability + wording gate (steps 23.2-23.65) ---
    desc_roots_var = tk.StringVar()          # semicolon-separated folders of isolated-structure images
    desc_retest_var = tk.StringVar()
    min_fine_var = tk.DoubleVar(value=0.85)
    min_coarse_var = tk.DoubleVar(value=0.80)
    gate_engine_var = tk.StringVar(value="python")
    gate_coarsen_var = tk.BooleanVar(value=False)

    def browse_file(var, ftypes=None):
        if ftypes is None:
            ftypes = [("All Files", "*.*")]
    ...
    r = 0
    tk.Label(popup, text="COCO JSON (Required):", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=coco_json_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(coco_json_var, [("JSON", "*.json")])).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Image Directory (Required):", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=image_dir_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_dir(image_dir_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Group Labels CSV (Required):", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=group_labels_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(group_labels_var, [("CSV", "*.csv")])).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Output Base Directory (Required):", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=output_base_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_dir(output_base_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Family Name:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=family_var, width=55).grid(row=r, column=1, padx=5, pady=3)

    r += 1
    tk.Label(popup, text="API Key File:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=api_key_file_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(api_key_file_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Keypoints JSON (Optional):").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=keypoints_json_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(keypoints_json_var, [("JSON", "*.json")])).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Literature PDFs Directory:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=pdf_dir_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_dir(pdf_dir_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="User Prompt Questions:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=user_prompts_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(user_prompts_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Exclude List CSV:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=exclude_list_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(exclude_list_var, [("CSV", "*.csv")])).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Label Images Directory:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=label_dir_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_dir(label_dir_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="VLM Model:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=model_var, width=30).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    r += 1
    tk.Label(popup, text="Conda Environment:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=conda_env_var, width=30).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    r += 1
    tk.Label(popup, text="Semilandmarks:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=num_landmarks_var, width=10).grid(row=r, column=1, sticky='w', padx=5, pady=3)
    tk.Label(popup, text="RAG k:").grid(row=r, column=1, padx=(120, 0), sticky='w', pady=3)
    tk.Entry(popup, textvariable=k_var, width=10).grid(row=r, column=1, padx=(180, 0), sticky='w', pady=3)

    r += 1
    tk.Checkbutton(popup, text="Force re-run (ignore existing outputs)", variable=force_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    r += 1
    tk.Checkbutton(popup, text="Dry run (print commands only)", variable=dry_run_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    # --- BioRAG v2 options (v73) ---
    from tkinter import ttk
    measure_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "measure")

    def _python_for_env():
        cb = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "..", "conda", "envs",
                          conda_env_var.get(), "bin", "python")
        return cb if os.path.exists(cb) else "python"

    def create_draft_profile():
        if not coco_json_var.get() or not group_labels_var.get():
            messagebox.showerror("Error", "Select the COCO JSON and Group Labels CSV first.")
            return
        out = filedialog.asksaveasfilename(title="Save draft taxon profile", defaultextension=".yaml",
                                           initialfile="taxon_profile_DRAFT.yaml",
                                           filetypes=[("YAML", "*.yaml")])
        if not out:
            return
        cmd = [_python_for_env(), os.path.join(measure_dir, "biorag_make_taxon_profile_v1.py"),
               "--coco_json", coco_json_var.get(), "--group_labels", group_labels_var.get(),
               "--family", family_var.get(), "--output", out]
        if keypoints_json_var.get():
            cmd += ["--keypoints_json", keypoints_json_var.get()]
        if user_prompts_var.get():
            cmd += ["--user_prompts", user_prompts_var.get()]
        res = subprocess.run(cmd, capture_output=True, text=True)
        if res.returncode == 0:
            taxon_profile_var.set(out)
            messagebox.showinfo("Taxon profile", res.stdout.strip() + "\n\nEdit species names/status, "
                                "sex-specific structures and standard ratios before publication.")
        else:
            messagebox.showerror("Taxon profile", res.stderr[-1500:])

    r += 1
    tk.Label(popup, text="— BioRAG v2 (evidence-tiered) —", font=("Arial", 9, "bold")).grid(row=r, column=0, columnspan=3, pady=(8, 2))
    r += 1
    tk.Label(popup, text="Workflow:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    ttk.Combobox(popup, textvariable=workflow_var, values=["v2", "v1"], width=10, state="readonly").grid(row=r, column=1, sticky='w', padx=5, pady=3)
    tk.Label(popup, text="v2 = data-driven key + tiered treatments (recommended); v1 = legacy").grid(row=r, column=1, padx=(110, 0), sticky='w')
    r += 1
    tk.Label(popup, text="Taxon Profile (YAML):").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=taxon_profile_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(taxon_profile_var, [("YAML", "*.yaml *.yml")])).grid(row=r, column=2, padx=5)
    r += 1
    tk.Button(popup, text="Create draft profile from COCO + group labels", command=create_draft_profile).grid(row=r, column=1, sticky='w', padx=5, pady=2)
    r += 1
    tk.Label(popup, text="LLM Backend:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    ttk.Combobox(popup, textvariable=llm_backend_var, values=["api", "claude-code", "none"], width=12, state="readonly").grid(row=r, column=1, sticky='w', padx=5, pady=3)
    tk.Label(popup, text="api = API key file; claude-code = Claude subscription (claude -p)").grid(row=r, column=1, padx=(130, 0), sticky='w')
    r += 1
    tk.Label(popup, text="Verified Localities TSV:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=localities_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Browse", command=lambda: browse_file(localities_var, [("TSV/CSV", "*.tsv *.csv")])).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(popup, text="Monograph Title:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=title_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    r += 1
    tk.Label(popup, text="Authors:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=authors_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    r += 1
    tk.Checkbutton(popup, text="Keep values flagged by the outlier / label-swap screen (default: exclude)", variable=keep_flagged_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    # --- character reliability and the wording gate (steps 23.2-23.65) ---
    def add_desc_root():
        d = filedialog.askdirectory(title="Folder of isolated-structure images (semilandmark output)")
        if d:
            cur = desc_roots_var.get()
            desc_roots_var.set(f"{cur};{d}" if cur else d)

    r += 1
    tk.Label(popup, text="— Character reliability and the wording gate —",
             font=("Arial", 9, "bold")).grid(row=r, column=0, columnspan=3, pady=(8, 2))
    r += 1
    tk.Label(popup, text="Structure images:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Entry(popup, textvariable=desc_roots_var, width=55).grid(row=r, column=1, padx=5, pady=3)
    tk.Button(popup, text="Add folder", command=add_desc_root).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(popup, text="the semilandmark output (*_fg_<structure>.png). Leave empty to skip steps 23.2-23.65",
             fg="#555").grid(row=r, column=1, sticky='w', padx=5)
    r += 1
    tk.Label(popup, text="Repeat as scored / banded:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    tk.Spinbox(popup, from_=0.5, to=1.0, increment=0.01, textvariable=min_fine_var,
               width=6).grid(row=r, column=1, sticky='w', padx=5, pady=3)
    tk.Spinbox(popup, from_=0.5, to=1.0, increment=0.01, textvariable=min_coarse_var,
               width=6).grid(row=r, column=1, sticky='w', padx=(70, 0), pady=3)
    tk.Label(popup, text="a character is kept when a second reading of the same images agrees this often",
             fg="#555").grid(row=r, column=1, sticky='w', padx=(140, 0))
    r += 1
    tk.Label(popup, text="Remove unreproducible wording with:").grid(row=r, column=0, sticky='e', padx=5, pady=3)
    ttk.Combobox(popup, textvariable=gate_engine_var, values=["python", "llm"], width=10,
                 state="readonly").grid(row=r, column=1, sticky='w', padx=5, pady=3)
    tk.Label(popup, text="python = by rule, deterministic, removes only what it matched (recommended)",
             fg="#555").grid(row=r, column=1, sticky='w', padx=(110, 0))
    r += 1
    tk.Checkbutton(popup, text="Also replace a band-level character's word with its band name (off: bands score, they do not reword)",
                   variable=gate_coarsen_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    def run_pipeline():
        if not coco_json_var.get() or not image_dir_var.get() or not group_labels_var.get() or not output_base_var.get():
            messagebox.showerror("Error", "COCO JSON, Image Directory, Group Labels, and Output Base are required.")
            return
        if llm_backend_var.get() == "claude-code":
        ...
        conda_base = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "..", "conda", "envs", conda_env_var.get(), "bin", "python")
        python_bin = conda_base if os.path.exists(conda_base) else "python"

        cmd = [
            python_bin, script,
            "--coco_json", coco_json_var.get(),
            "--image_dir", image_dir_var.get(),
            "--group_labels", group_labels_var.get(),
            "--output_base", output_base_var.get(),
            "--conda_env", conda_env_var.get(),
            "--model", model_var.get(),
            "--num_landmarks", str(num_landmarks_var.get()),
            "--k", str(k_var.get()),
        ]
        if family_var.get():
            cmd += ["--family", family_var.get()]
        if api_key_file_var.get():
            cmd += ["--api_key_file", api_key_file_var.get()]
        if keypoints_json_var.get():
            cmd += ["--keypoints_json", keypoints_json_var.get()]
        if pdf_dir_var.get():
            cmd += ["--pdf_dir", pdf_dir_var.get()]
        if user_prompts_var.get():
            cmd += ["--user_prompts", user_prompts_var.get()]
        if exclude_list_var.get():
            cmd += ["--exclude_list", exclude_list_var.get()]
        if label_dir_var.get():
            cmd += ["--label_dir", label_dir_var.get()]
        if force_var.get():
            cmd.append("--force")
        if dry_run_var.get():
            cmd.append("--dry_run")
        if workflow_var.get() == "v2":
            cmd += ["--workflow", "v2", "--llm_backend", llm_backend_var.get()]
            if taxon_profile_var.get():
                cmd += ["--taxon_profile", taxon_profile_var.get()]
            if localities_var.get():
                cmd += ["--localities", localities_var.get()]
            if title_var.get():
                cmd += ["--title", title_var.get()]
            if authors_var.get():
                cmd += ["--authors", authors_var.get()]
            if keep_flagged_var.get():
                cmd.append("--keep_flagged")
            roots = [x for x in desc_roots_var.get().split(";") if x.strip()]
            if roots:
                cmd += ["--descriptive_image_roots"] + roots
                cmd += ["--reliability_min_fine", str(min_fine_var.get()),
                        "--reliability_min_coarse", str(min_coarse_var.get()),
                        "--gate_engine", gate_engine_var.get()]
                if desc_retest_var.get():
                    cmd += ["--descriptive_retest", desc_retest_var.get()]
                if gate_coarsen_var.get():
                    cmd.append("--gate_coarsen")

        def _run():
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=86400)
                if result.returncode == 0:
                    messagebox.showinfo("BioRAG Pipeline", f"Pipeline completed successfully.\nOutput: {output_base_var.get()}")
                else:
                    messagebox.showerror("BioRAG Pipeline", f"Pipeline failed.\n\nStderr:\n{result.stderr[-2000:]}")
            except subprocess.TimeoutExpired:
                messagebox.showerror("BioRAG Pipeline", "Pipeline timed out (24h limit).")
            except Exception as e:
                messagebox.showerror("BioRAG Pipeline", f"Error: {e}")

        threading.Thread(target=_run, daemon=True).start()
        messagebox.showinfo("BioRAG Pipeline", "Pipeline started in background.\nCheck the logs directory for progress.")

    r += 1
    tk.Button(popup, text="Run BioRAG Pipeline", command=run_pipeline,
              bg="#2ecc71", fg="white", font=("Arial", 11, "bold"),
              padx=20, pady=5).grid(row=r, column=0, columnspan=3, pady=10)


def open_delimitation_popup():
    """Tools that ask whether a specimen belongs to a described species, and how far the
    published record can be trusted. Each runs one script on an existing monograph directory."""
    import tkinter as tk
    from tkinter import ttk

    popup = tk.Toplevel(root)
    popup.title("Quality control and comparisons — characters, key, graph, matrix, model-proposed characters")
    popup.geometry("980x940")

    mono_var = tk.StringVar()
    profile_var = tk.StringVar()
    cand_var = tk.StringVar()
    catmap_var = tk.StringVar(value='{"entire_forewing": "whole_wing"}')
    label_var = tk.StringVar(value="external material")
    env_var = tk.StringVar(value="measure_env")
    coco_var = tk.StringVar()          # the reference set's own COCO file and images: needed by the
    img_var = tk.StringVar()           # annotation screen and by the model-proposed character arm
    compiled_var = tk.StringVar()      # compiled feature directory (measured stand-ins, tables S5-S6)
    gpa_var = tk.StringVar()           # semilandmark GPA directory, so frames share the pipeline's consensus
    gui_dir = os.path.dirname(os.path.abspath(__file__))
    measure_dir = os.path.join(gui_dir, "measure")

    def py():
        cb = os.path.join(gui_dir, "..", "..", "conda", "envs", env_var.get(), "bin", "python")
        return cb if os.path.exists(cb) else "python"
    ...
    def pick_file(var, ft):
        f = filedialog.askopenfilename(filetypes=ft)
        if f:
            var.set(f)

    r = 0
    tk.Label(popup, text="Monograph directory:").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    tk.Entry(popup, textvariable=mono_var, width=62).grid(row=r, column=1, padx=5, pady=4)
    tk.Button(popup, text="Browse", command=lambda: pick_dir(mono_var)).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(popup, text="Taxon profile (YAML):").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    tk.Entry(popup, textvariable=profile_var, width=62).grid(row=r, column=1, padx=5, pady=4)
    tk.Button(popup, text="Browse",
              command=lambda: pick_file(profile_var, [("YAML", "*.yaml *.yml")])).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(popup, text="Candidate COCO (optional):").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    tk.Entry(popup, textvariable=cand_var, width=62).grid(row=r, column=1, padx=5, pady=4)
    tk.Button(popup, text="Browse",
              command=lambda: pick_file(cand_var, [("COCO JSON", "*.json")])).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(popup, text="material from outside the described set — use HUMAN-CORRECTED annotations, "
                        "not raw predictions", fg="#555").grid(row=r, column=1, sticky='w', padx=5)
    r += 1
    tk.Label(popup, text="Category map:").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    tk.Entry(popup, textvariable=catmap_var, width=62).grid(row=r, column=1, padx=5, pady=4)
    r += 1
    tk.Label(popup, text="Label:").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    tk.Entry(popup, textvariable=label_var, width=62).grid(row=r, column=1, padx=5, pady=4)
    r += 1
    tk.Label(popup, text="Conda env:").grid(row=r, column=0, sticky='e', padx=5, pady=4)
    ttk.Combobox(popup, textvariable=env_var, values=["measure_env", "biorag", "samm"], width=14,
                 state="readonly").grid(row=r, column=1, sticky='w', padx=5, pady=4)
    for lab, var, isdir in (("Reference COCO (screen, model arm):", coco_var, False),
                            ("Reference image directory:", img_var, True),
                            ("Compiled feature directory:", compiled_var, True),
                            ("Semilandmark GPA directory:", gpa_var, True)):
        r += 1
        tk.Label(popup, text=lab).grid(row=r, column=0, sticky='e', padx=5, pady=2)
        tk.Entry(popup, textvariable=var, width=62).grid(row=r, column=1, padx=5, pady=2)
        tk.Button(popup, text="Browse",
                  command=(lambda v=var: pick_dir(v)) if isdir else
                  (lambda v=var: pick_file(v, [("COCO JSON", "*.json")]))).grid(row=r, column=2, padx=5)

    def launch(cmd, title):
        if not mono_var.get() or not profile_var.get():
            messagebox.showerror("Error", "Monograph directory and taxon profile are required.")
            return

        def _run():
            try:
                res = subprocess.run(cmd, capture_output=True, text=True, timeout=86400, cwd=measure_dir)
                tail = (res.stdout or res.stderr or "")[-2500:]
                if res.returncode == 0:
                    messagebox.showinfo(title, tail or "done")
                else:
                    messagebox.showerror(title, tail)
            except Exception as e:  # noqa: BLE001
                messagebox.showerror(title, str(e))

        threading.Thread(target=_run, daemon=True).start()
        messagebox.showinfo(title, "Started in the background.")

    m = lambda *p: os.path.join(mono_var.get(), *p)

    def keydir():
        """The pipeline writes its key to one folder name, a monograph run step by step to another."""
        return "key" + "_v2" if os.path.isdir(m("key" + "_v2")) else "key"

    def launch_chain(cmds, title):
        """Several scripts in order; stops at the first that fails and says which."""
        if not mono_var.get() or not profile_var.get():
            messagebox.showerror("Error", "Monograph directory and taxon profile are required.")
            return
        ...
        def _run():
            try:
                for i, cmd in enumerate(cmds, 1):
                    res = subprocess.run(cmd, capture_output=True, text=True, timeout=86400, cwd=measure_dir)
                    if res.returncode != 0:
                        messagebox.showerror(title, f"step {i} of {len(cmds)} failed: "
                                             f"{os.path.basename(cmd[1])}\n\n" + (res.stderr or res.stdout or "")[-2200:])
                        return
                messagebox.showinfo(title, (res.stdout or "done")[-2500:])
            except Exception as e:  # noqa: BLE001
                messagebox.showerror(title, str(e))

        threading.Thread(target=_run, daemon=True).start()
        messagebox.showinfo(title, f"Started in the background ({len(cmds)} steps).")

    def need(*pairs):
        missing = [lab for lab, v in pairs if not v]
        if missing:
            messagebox.showerror("Error", "This step also needs: " + ", ".join(missing))
        return not missing

    def calibrate():
        cmd = [py(), os.path.join(measure_dir, "biorag_calibrate_v1.py"),
               "--matrix_dir", m("compiled_key_tier"), "--taxon_profile", profile_var.get(),
               "--key_tree", m("key", "key_tree.json"), "--out_dir", m("calibration"),
               "--python", py()]
        rel = m("descriptive_states", "character_reliability.json")
        if os.path.exists(rel):
            cmd += ["--reliability", rel]
        launch(cmd, "Calibrate")

    def novelty():
        cmd = [py(), os.path.join(measure_dir, "biorag_novelty_score_v1.py"),
               "--matrix_dir", m("compiled_key_tier"), "--taxon_profile", profile_var.get(),
               "--key_tree", m(keydir(), "key_tree.json"), "--out_dir", m("novelty_gated")]
        rel = m("descriptive_states", "character_reliability.json")
        if os.path.exists(rel):
            cmd += ["--reliability", rel,
                    "--descriptive_matrix", m("descriptive_states", "descriptive_states_by_specimen.tsv")]
        if cand_var.get():
            cmd += ["--candidate_coco", cand_var.get(), "--candidate_label", label_var.get(),
                    "--category_map", catmap_var.get()]
        else:
            cmd.append("--holdout_all")
        launch(cmd, "Novelty score")

    def key_holdout():
        cmd = [py(), os.path.join(measure_dir, "biorag_key_holdout_v1.py"),
               "--matrix_dir", m("compiled_key_tier"), "--taxon_profile", profile_var.get(),
               "--out_dir", m("key_holdout_v2")]
        if cand_var.get():
            cmd += ["--candidate_coco", cand_var.get(), "--category_map", catmap_var.get(),
                    "--key_tree", m(keydir(), "key_tree.json"), "--label", label_var.get(),
                    "--out_dir", m("key_external")]
        else:
            cmd += ["--baseline", m(keydir(), "identification_test.tsv")]
        launch(cmd, "Key hold-out")

    def graph():
        cmd = [py(), os.path.join(measure_dir, "biorag_graph_identify_v1.py"),
               "--jsonld_dir", m("treatments", "machine_readable", "jsonld"),
               "--matrix_dir", m("compiled_key_tier"), "--taxon_profile", profile_var.get(),
               "--out_dir", m("graph_test_loo"),
               # each specimen scored against ranges re-derived without it, every range recomputed
               # from the current matrix (the treatments may have been exported from an older one)
               "--arm", "leave_one_specimen_out", "--ranges_from", "matrix"]
        if cand_var.get():
            cmd += ["--candidate_coco", cand_var.get(), "--category_map", catmap_var.get(),
                    "--candidate_label", label_var.get()]
        launch(cmd, "Knowledge-graph test")

    def provenance():
        if not cand_var.get():
            messagebox.showerror("Error", "This check needs a candidate COCO file.")
            return
        launch([py(), os.path.join(measure_dir, "biorag_outgroup_batch_check_v1.py"),
                "--matrix_dir", m("compiled_key_tier"), "--taxon_profile", profile_var.get(),
                "--candidate_coco", cand_var.get(), "--category_map", catmap_var.get(),
                "--out_dir", m("outgroup_check")], "Provenance check")

    def figures():
        launch([py(), os.path.join(measure_dir, "biorag_reliability_figures_v1.py"),
                ...
                "--out_dir", m("descriptive_states", "figures")], "Character figures")

    def screen():
        if not need(("reference COCO", coco_var.get()), ("reference image directory", img_var.get())):
            return
        launch([py(), os.path.join(measure_dir, "biorag_annotation_screen_v1.py"),
                "--coco", coco_var.get(), "--image_dir", img_var.get(),
                "--out_dir", m("annotation_screen")], "Annotation screen")

    def compare():
        mat, prof = m("compiled_key_tier"), profile_var.get()
        launch_chain([
            [py(), os.path.join(measure_dir, "biorag_calibrate_v1.py"), "--matrix_dir", mat,
             "--taxon_profile", prof, "--key_tree", m(keydir(), "key_tree.json"),
             "--out_dir", m("calibration"), "--python", py()],
            [py(), os.path.join(measure_dir, "biorag_graph_identify_v1.py"),
             "--jsonld_dir", m("treatments", "machine_readable", "jsonld"), "--matrix_dir", mat,
             "--taxon_profile", prof, "--out_dir", m("graph_test_loo"),
             "--arm", "leave_one_specimen_out", "--ranges_from", "matrix"],
            [py(), os.path.join(measure_dir, "biorag_key_fuzzy_v1.py"), "--matrix_dir", mat,
             "--taxon_profile", prof, "--key_tree", m(keydir(), "key_tree.json"),
             "--out_dir", m("key_fuzzy_loo"), "--python", py(), "--known_arm", "leave_one_specimen_out"],
            [py(), os.path.join(measure_dir, "biorag_instrument_compare_v3.py"),
             "--calibration", m("calibration"), "--key_loo", m(keydir(), "identification_test.tsv"),
             "--graph_scores", m("graph_test_loo", "graph_specimen_scores.tsv"),
             "--fuzzy", m("key_fuzzy_loo", "fuzzy_key_specimens.tsv"),
             "--out_dir", m("instrument_comparison_v3")]], "Key, graph and matrix")

    def descriptive_tables():
        if not need(("compiled feature directory", compiled_var.get())):
            return
        launch(["bash", os.path.join(measure_dir, "biorag_rerun_descriptive_tables_v1.sh"), py(),
                mono_var.get(), profile_var.get(), compiled_var.get()], "Descriptive-character tables")

    def holdout_tests():
        launch([py(), os.path.join(measure_dir, "tests", "test_biorag_holdout_v1.py")], "Hold-out tests")

    def vlm_frames():
        if not need(("reference COCO", coco_var.get()), ("reference image directory", img_var.get())):
            return
        cmd = [py(), os.path.join(measure_dir, "biorag_homology_frame_v1.py"), "--coco", coco_var.get(),
               "--image_dir", img_var.get(), "--taxon_profile", profile_var.get(),
               "--matrix_dir", m("compiled_key_tier"), "--out_dir", m("homology_frames")]
        if gpa_var.get():
            cmd += ["--gpa_dir", gpa_var.get()]
        excl = m("annotation_screen", "exclusion_list_merged.csv")
        if os.path.exists(excl):
            cmd += ["--exclude_list", excl]
        launch_chain([cmd, [py(), os.path.join(measure_dir, "biorag_frame_orientation_v1.py"),
                            "--frames_dir", m("homology_frames"),
                            "--out", m("homology_frames", "frame_orientation.tsv")]],
                     "Homology frames")
    ...
    def vlm_characters():
        ...
        if not messagebox.askyesno(
                "Model calls",
                "This step calls a vision-language model: one call per contrast set to propose "
                "characters, then one per specimen and structure to score them (several thousand "
                "calls for ~150 specimens). It runs through your Claude Code subscription "
                "(--llm-backend claude-code), not the API, and can be resumed. Start it?"):
            return
        launch([py(), os.path.join(measure_dir, "biorag_vlm_characters_v1.py"), "--coco", coco_var.get(),
                "--image_dir", img_var.get(), "--taxon_profile", profile_var.get(),
                "--matrix_dir", m("compiled_key_tier"), "--frames_dir", m("homology_frames"),
                "--out_dir", m("vlm_characters"), "--llm-backend", "claude-code", "--resume"],
               "Model-proposed characters")

    def vlm_states():
        for d in ("vlm_combined", "vlm_characters"):
            if os.path.exists(m(d, "vlm_character_states.tsv")):
                return d
        return None
    ...
    def vlm_helps():
        d = vlm_states()
        if not d:
            messagebox.showerror("Error", "No vlm_character_states.tsv found; run the model-proposed "
                                          "characters step first.")
            return
        mat, prof, plus = m("compiled_key_tier"), profile_var.get(), m("compiled_key_tier_plus_vlm")
        launch_chain([
            [py(), os.path.join(measure_dir, "biorag_autapomorphy_v1.py"),
             "--states", m(d, "vlm_character_states.tsv"), "--out_dir", m("autapomorphies_vlm")],
            [py(), os.path.join(measure_dir, "biorag_add_discrete_characters_v1.py"), "--matrix_dir", mat,
             "--states", m(d, "vlm_character_states.tsv"), "--out_dir", plus, "--prefix", "vlm"],
            [py(), os.path.join(measure_dir, "biorag_key_builder_v1.py"), "--matrix_dir", plus,
             "--output_dir", m("key_plus_vlm"), "--taxon-profile", prof, "--llm-backend", "none"],
            [py(), os.path.join(measure_dir, "biorag_key_fuzzy_v1.py"), "--matrix_dir", plus,
             "--taxon_profile", prof, "--key_tree", m("key_plus_vlm", "key_tree.json"),
             "--out_dir", m("key_plus_vlm", "fuzzy_loo"), "--python", py(),
             "--known_arm", "leave_one_specimen_out"],
            [py(), os.path.join(measure_dir, "biorag_congruence_compare_v1.py"), "--matrix_dir", mat,
             "--taxon_profile", prof, "--extra_states", m(d, "vlm_character_states.tsv"),
             "--proposals", m(d, "vlm_proposed_characters.tsv"),
             "--key_loo", m(keydir(), "identification_test.tsv"),
             "--out_dir", m("congruence_compare")]], "Do the model's characters help?")

    def vlm_heatmaps():
        d = vlm_states()
        if not d:
            messagebox.showerror("Error", "No vlm_character_states.tsv found.")
            return
        cmd = [py(), os.path.join(measure_dir, "biorag_vlm_heatmap_figures_v1.py"),
               "--states", m(d, "vlm_character_states.tsv"), "--frames_dir", m("homology_frames"),
               "--out_dir", m(d, "figures_heatmap")]
        if os.path.exists(m("homology_frames", "frame_orientation.tsv")):
            cmd += ["--orientation", m("homology_frames", "frame_orientation.tsv")]
        launch(cmd, "Where the model looked")

    r += 1
    tk.Label(popup, text="— run against the monograph above —",
             font=("Arial", 9, "bold")).grid(row=r, column=0, columnspan=3, pady=(10, 2))
    for text, fn, hint in (
            ("Annotation screen", screen,
             "does every polygon sit on the specimen? run before the matrix is built"),
            ("Calibrate (run first)", calibrate,
             "key AND matrix on this reference set, both hold-out arms; says which to use for a name "
             "and which for a warning"),
            ("Key, graph and matrix", compare,
             "calibrate + graph + graded key + comparison: naming and novelty, nested margins, agreement"),
            ("Novelty score", novelty,
             "each species held out in turn, or a candidate set scored against all of them"),
            ("Key hold-out", key_holdout,
             "rebuild the key without each species and run it down; or run a candidate down the key"),
            ("Knowledge-graph test", graph,
             "identification and novelty from the published assertions"),
            ("Provenance check", provenance,
             "is a candidate set separated by biology, or by how it was imaged and annotated?"),
            ("Character figures", figures,
             "repeatability, congruence and the gating ablation as tables and figures"),
            ("Descriptive tables", descriptive_tables,
             "the six character sets and the gating ablation recomputed on the current matrix"),
            ("Frames + orientation", vlm_frames,
             "every specimen of a structure in one frame (shape untouched), then which frames lie end-for-end"),
            ("Propose + score (MODEL)", vlm_characters,
             "a vision-language model proposes present/absent characters and scores every specimen blind"),
            ("Do they help?", vlm_helps,
             "autapomorphies; the key with those states (graded); the matrix with them, selection leak closed"),
            ("Where the model looked", vlm_heatmaps,
             "density of reported locations per character: spread, offset, share on the specimen"),
            ("Hold-out tests", holdout_tests,
             "a withheld specimen or species must not move its own score: matrix, key and graph")):
        r += 1
        tk.Button(popup, text=text, command=fn, width=24,
                  bg="#8e44ad" if "MODEL" in text else "#2980b9", fg="white").grid(
            row=r, column=0, padx=5, pady=1, sticky='e')
        tk.Label(popup, text=hint, fg="#444").grid(row=r, column=1, columnspan=2, sticky='w', padx=5)


def open_zenodo_popup():
    """Open popup to upload BioRAG outputs to Zenodo."""
    import tkinter as tk
    from tkinter import filedialog, messagebox
    import subprocess, threading

    popup = tk.Toplevel()
    popup.title("Upload to Zenodo")
    ...
    token_path = os.path.expanduser("~/.descriptron/zenodo_token")
    ...
    r = 0
    tk.Label(popup, text="Pipeline Output Directory:", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(popup, textvariable=output_base_var, width=50).grid(row=r, column=1, padx=5, pady=5)
    tk.Button(popup, text="Browse", command=lambda: browse_dir(output_base_var)).grid(row=r, column=2, padx=5)

    r += 1
    tk.Label(popup, text="Deposit Title:", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(popup, textvariable=title_var, width=50).grid(row=r, column=1, padx=5, pady=5)

    r += 1
    tk.Label(popup, text="Authors (semicolon-separated):", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(popup, textvariable=authors_var, width=50).grid(row=r, column=1, padx=5, pady=5)

    r += 1
    tk.Label(popup, text="Description (optional):").grid(row=r, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(popup, textvariable=description_var, width=50).grid(row=r, column=1, padx=5, pady=5)

    r += 1
    tk.Label(popup, text="Zenodo API Token:", font=("Arial", 9, "bold")).grid(row=r, column=0, sticky='e', padx=5, pady=5)
    tk.Entry(popup, textvariable=token_var, width=50, show="*").grid(row=r, column=1, padx=5, pady=5)

    r += 1
    tk.Checkbutton(popup, text="Remember token (~/.descriptron/zenodo_token)", variable=remember_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    r += 1
    tk.Checkbutton(popup, text="Use Sandbox (sandbox.zenodo.org) for testing", variable=sandbox_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    r += 1
    tk.Checkbutton(popup, text="Publish immediately (otherwise creates draft)", variable=publish_var).grid(row=r, column=1, sticky='w', padx=5, pady=3)

    def run_upload():
        if not output_base_var.get() or not title_var.get() or not authors_var.get() or not token_var.get():
            messagebox.showerror("Error", "Output directory, title, authors, and API token are all required.")
            return

        if remember_var.get():
            tp = os.path.expanduser("~/.descriptron")
            os.makedirs(tp, exist_ok=True)
            with open(os.path.join(tp, "zenodo_token"), "w") as f:
                f.write(token_var.get().strip() + "\n")
            os.chmod(os.path.join(tp, "zenodo_token"), 0o600)

        script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "measure", "zenodo_upload.py")
        cmd = [
            "python", script,
            "--output_base", output_base_var.get(),
            "--title", title_var.get(),
        ...
        if description_var.get():
            cmd += ["--description", description_var.get()]
        if sandbox_var.get():
            cmd.append("--sandbox")
        if publish_var.get():
            cmd.append("--publish")
        ...
        def _run():
            try:
                ...
                if result.returncode == 0:
                    messagebox.showinfo("Zenodo Upload", f"Upload complete!\n\n{result.stdout[-1000:]}")
                else:
                    messagebox.showerror("Zenodo Upload", f"Upload failed.\n\n{result.stderr[-1000:]}")
            except Exception as e:
                messagebox.showerror("Zenodo Upload", f"Error: {e}")

        threading.Thread(target=_run, daemon=True).start()
        messagebox.showinfo("Zenodo Upload", "Upload started...")

    r += 1
    tk.Button(popup, text="Upload to Zenodo", command=run_upload,
...
def apply_label(auto=False):
    ...
    # If "Custom" is selected, prompt the user for a custom label
    if label == "Custom":
        new_label = simpledialog.askstring("Custom Label", "Enter custom label name:")
        if not new_label:
            return # User cancelled initial naming

        # 1. Ask for parent
        parent_name, ont_curie = _ask_parent_region_for_custom_category(new_label)

        # 2. Register with Ontology Manager
        if _ONTOLOGY_AVAILABLE and _onto_mgr:
            _onto_mgr.register_custom_category(new_label, parent_name, ont_curie)

        # 3. CRITICAL: Override the 'label' variable completely
        label = new_label
          
        # 4. Set supercategory for the lower block to use
        #supercategory = parent_name if parent_name else "sclerite"
        chosen_supercat = simpledialog.askstring(
            "Supercategory", 
            f"Enter supercategory for '{label}':", 
            initialvalue="sclerite"
        )
        # If you hit Cancel or leave it blank, it safely falls back to 'sclerite'
        supercategory = chosen_supercat if chosen_supercat else "sclerite"
        # 5. Update UI selection immediately
        selected_category_label.set(label)
        selected_label.set(label)

        print(f"Registered custom category: {label} (Supercategory: {supercategory})")
          
          # Notice we DO NOT manually update category_id_to_name here anymore!
          # We let the code below handle it so it syncs properly.
    ...
    if label:
        ...
        # === ONTOLOGY: open attribute editor if category has attributes ===
        if _ONTOLOGY_AVAILABLE and _onto_mgr and not auto:
            ann_id = coco_output_accumulate["annotations"][-1]["id"] if coco_output_accumulate["annotations"] else None
            if ann_id is not None:
                #open_attribute_editor(ann_id, label)
                open_attribute_editor(ann_id, label)
    else:
...
# Horizontal divider between graphics panel and button panel
_divider = ttk.Separator(root, orient="horizontal")
_divider.grid(row=1, column=0, sticky="ew", pady=(2, 0))

# Button frame with vertical scroll (200px height, buttons in compact grid)
button_frame = tk.Frame(root, height=200)
button_frame.grid(row=2, column=0, sticky="ew")
button_frame.grid_propagate(False)

# Vertical scrollbar on the right, canvas fills the rest
v_scrollbar = tk.Scrollbar(button_frame, orient="vertical")
v_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

button_canvas = tk.Canvas(button_frame, yscrollcommand=v_scrollbar.set)
button_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
v_scrollbar.config(command=button_canvas.yview)
...
# Mouse wheel scrolling for the button area
def _on_btn_mousewheel(event):
    button_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")

def _on_btn_mousewheel_linux(event):
    if event.num == 4:
        button_canvas.yview_scroll(-1, "units")
    elif event.num == 5:
        button_canvas.yview_scroll(1, "units")

button_canvas.bind("<MouseWheel>", _on_btn_mousewheel)
button_canvas.bind("<Button-4>", _on_btn_mousewheel_linux)
button_canvas.bind("<Button-5>", _on_btn_mousewheel_linux)
scrollable_frame.bind("<MouseWheel>", _on_btn_mousewheel)
scrollable_frame.bind("<Button-4>", _on_btn_mousewheel_linux)
scrollable_frame.bind("<Button-5>", _on_btn_mousewheel_linux)
...
# v70: save keypoints directly, without needing point-prompt mode, a segmentation
# prediction, or Apply Label first.  The marmot (save & close) button only reaches
# save_points_to_via2() when point_prompt_mode is True, which is why pure landmark
# annotation sessions appeared to save nothing.
def save_keypoints_now():
    if not point_coords:
        messagebox.showinfo("Save Keypoints", "No keypoints placed yet.")
        return
    try:
        save_points_to_via2()
    except Exception as e:
        messagebox.showerror("Save Keypoints", f"Could not save keypoints: {e}")

save_keypoints_btn = tk.Button(scrollable_frame, text="Save Keypoints",
                               command=save_keypoints_now, bg="#1baf7a", fg="white")
save_keypoints_btn.grid(row=0, column=13, padx=0, pady=0)

# v70: folder browsing for annotation — Load Folder / Prev / Next / counter.
load_folder_btn = tk.Button(scrollable_frame, text="Load Folder",
                            command=load_image_folder, bg="#2a78d6", fg="white")
load_folder_btn.grid(row=0, column=14, padx=0, pady=0)

folder_prev_btn = tk.Button(scrollable_frame, text="◀ Prev",
                            command=lambda: folder_goto(-1))
folder_prev_btn.grid(row=0, column=15, padx=0, pady=0)

folder_next_btn = tk.Button(scrollable_frame, text="Next ▶",
                            command=lambda: folder_goto(1))
folder_next_btn.grid(row=0, column=16, padx=0, pady=0)

folder_status_label = tk.Label(scrollable_frame, text="no folder loaded",
                               width=34, anchor="w")
folder_status_label.grid(row=0, column=17, padx=2, pady=0)

_folder_autosave_var = tk.BooleanVar(value=True)
def _toggle_folder_autosave():
    folder_autosave["on"] = bool(_folder_autosave_var.get())
tk.Checkbutton(scrollable_frame, text="auto-save KPs",
               variable=_folder_autosave_var,
               command=_toggle_folder_autosave).grid(row=0, column=18, padx=0, pady=0)

# Keyboard navigation: PageDown/PageUp step through the folder.
try:
    root.bind("<Next>",  lambda e: folder_goto(1))
    root.bind("<Prior>", lambda e: folder_goto(-1))
except Exception:
    pass

_enable_drag_and_drop(canvas)
...
brush_size_menu.grid(row=1, column=8, padx=0, pady=0)
...
load_annotations_btn.grid(row=1, column=9, padx=0, pady=0)
...
prev_image_button.grid(row=2, column=12, padx=0, pady=0)

# Next Button
next_image_button = tk.Button(scrollable_frame, text="Next Image", command=load_next_image, bg="orange") #state=tk.DISABLED)
next_image_button.grid(row=2, column=13, padx=0, pady=0)

# Trash button moved to row 1, col 2 (closer to Next/Previous Mask)

# === DINOLand (DINOv3 landmark transfer v51) launcher ===
# v51 loads DINOv3 through Hugging Face transformers, which only knows the dinov3_vit model
# type from transformers >= 4.56. samm and measure_env carry 4.52, so the script runs in the
# biorag env (transformers 5.x, torch cu130). Override with the DINOLAND_ENV variable.
...
def _dinoland_build_cmd(v, script_path, env=None):
    ...
    env = env or DINOLAND_ENV
    mode = v.get("mode", "pair")
    # --no-capture-output so the run log fills while the script runs, not at the end
    cmd = ["conda", "run", "-n", env, "--no-capture-output", "python",
           str(script_path), "--imgA", v["imgA"], "--outdir", v["outdir"]]
    if mode == "selftest":
        cmd.append("--selftest")
    elif mode == "batch":
        cmd += ["--batch_glob", v["batch_glob"], "--landmarks", v["landmarks"]]
        if v.get("ref_dir"):
            cmd += ["--ref_dir", v["ref_dir"]]
        if v.get("batch_n"):
        ...
        if v.get("maskB"):
            cmd += ["--maskB", v["maskB"]]
        if v.get("landmarks"):
            cmd += ["--landmarks", v["landmarks"]]
    if v.get("maskA"):
        cmd += ["--maskA", v["maskA"]]
    if v.get("mask_dir"):
        cmd += ["--mask_dir", v["mask_dir"]]
    for key, flag in (("model", "--model"), ("layers", "--layers"), ("n_lm", "--n_lm"),
                      ("radius", "--radius"), ("dim", "--dim"), ("facet", "--facet"),
                      ("align", "--align"), ("preview", "--preview"),
                      ("fig_per_page", "--fig_per_page")):
        if str(v.get(key, "")).strip():
            cmd += [flag, str(v[key]).strip()]
    if v.get("extra", "").strip():
        cmd += v["extra"].split()
    return cmd


def run_dinoland_script():
    """
    DINOLand: DINOv3 landmark transfer (dinov3_landmark_transfer_v51.py).
    Transfers COCO keypoint landmarks from annotated reference plates to unannotated
    targets with DINOv3 patch tokens (PCA-affine alignment -> local search -> cycle
    consistency -> RANSAC -> soft-argmax). Three modes: pair (A -> B with figures),
    batch (N annotated refs -> a glob of targets, writes predictions_keypoints.json that
    'Load Annotations' can read back), selftest (synthetic warp, reports pixel error).
    Runs via `conda run -n DINOLAND_ENV` (default biorag: the only env whose transformers
    knows DINOv3); the HF model id defaults to facebook/dinov3-vitb16-pretrain-lvd1689m
    (needs the cached weights or an HF token).
    """
    import subprocess
    import threading
    from pathlib import Path
    ...
    input_window = tk.Toplevel()
    input_window.title("DINOLand — DINOv3 landmark transfer (v51)")

    mode = tk.StringVar(value="pair")
    imgA = tk.StringVar(); maskA = tk.StringVar()
    imgB = tk.StringVar(); maskB = tk.StringVar()
    landmarks = tk.StringVar(); batch_glob = tk.StringVar(); batch_n = tk.StringVar(value="6")
    ref_dir = tk.StringVar(); mask_dir = tk.StringVar(); outdir = tk.StringVar(value="lm_out")
    model_id = tk.StringVar(value="facebook/dinov3-vitb16-pretrain-lvd1689m")
    layers = tk.StringVar(value="6,8,9,10,11"); n_lm = tk.StringVar(value="60")
    radius = tk.StringVar(value="4.0"); dim = tk.StringVar(value="512")
    ...
    def browse_files_joined(var, filetypes):
    ...
    def browse_glob_dir(var):
        path = filedialog.askdirectory()
        if path:
            var.set(str(Path(path) / "*.jpg"))

    row = 0
    tk.Label(input_window, text="Mode:").grid(row=row, column=0, sticky='e', padx=5, pady=5)
    mf = tk.Frame(input_window); mf.grid(row=row, column=1, sticky='w')
    for txt, val in (("Pair: reference A -> target B", "pair"),
                     ("Batch: annotated refs -> glob of targets", "batch"),
                     ("Self-test (synthetic warp)", "selftest")):
        tk.Radiobutton(mf, text=txt, variable=mode, value=val).pack(side=tk.LEFT, padx=3)

    def add_row(label, var, browse=None, width=50):
        nonlocal row
        row += 1
        tk.Label(input_window, text=label).grid(row=row, column=0, sticky='e', padx=5, pady=3)
        tk.Entry(input_window, textvariable=var, width=width).grid(row=row, column=1, padx=5, pady=3, sticky='w')
        if browse is not None:
            tk.Button(input_window, text="Browse", command=browse).grid(row=row, column=2, padx=5, pady=3)

    add_row("Reference image A (required):", imgA, lambda: browse_file(imgA, IMG_TYPES))
    add_row("Mask A (optional):", maskA, lambda: browse_file(maskA, IMG_TYPES))
    add_row("Target image B (pair mode):", imgB, lambda: browse_file(imgB, IMG_TYPES))
    add_row("Mask B (optional):", maskB, lambda: browse_file(maskB, IMG_TYPES))
    add_row("Landmark COCO JSON(s) (batch: required; pair: optional):", landmarks,
            lambda: browse_files_joined(landmarks, JSON_TYPES))
    add_row("Target glob (batch mode, e.g. /dir/*.jpg):", batch_glob, lambda: browse_glob_dir(batch_glob))
    add_row("Reference image folder (--ref_dir, batch):", ref_dir, lambda: browse_directory(ref_dir))
    add_row("Mask folder (<stem>_mask.png, optional):", mask_dir, lambda: browse_directory(mask_dir))
    add_row("Output folder:", outdir, lambda: browse_directory(outdir))
    add_row("HF model id:", model_id)
    add_row("Layers (comma list):", layers, width=20)
    add_row("Landmarks per selftest / n_lm:", n_lm, width=10)
    add_row("Search radius (patches):", radius, width=10)
    add_row("Input size (px):", dim, width=10)
    add_row("Batch: targets per figure page:", fig_per_page, width=10)
    add_row("Batch: max targets (--batch_n):", batch_n, width=10)

    row += 1
    tk.Label(input_window, text="Facet / Align / Preview:").grid(row=row, column=0, sticky='e', padx=5, pady=3)
    of = tk.Frame(input_window); of.grid(row=row, column=1, sticky='w')
    tk.OptionMenu(of, facet, "token", "key").pack(side=tk.LEFT)
    ...
    def submit():
        v = dict(mode=mode.get(), imgA=imgA.get().strip(), maskA=maskA.get().strip(),
                 imgB=imgB.get().strip(), maskB=maskB.get().strip(),
                 landmarks=landmarks.get().strip(), batch_glob=batch_glob.get().strip(),
                 batch_n=batch_n.get().strip(), ref_dir=ref_dir.get().strip(),
        ...
        if not v["imgA"]:
            ...
            return
        if v["mode"] == "pair" and not v["imgB"]:
            messagebox.showerror("Input Error", "Pair mode needs a target image B.")
            return
        if v["mode"] == "batch" and (not v["batch_glob"] or not v["landmarks"]):
            messagebox.showerror("Input Error", "Batch mode needs a target glob and at least one landmark JSON.")
            return
        ...
        def run_script():
            try:
                loading = tk.Toplevel()
                loading.title("Running DINOLand")
                tk.Label(loading, text=f"DINOv3 landmark transfer running ({v['mode']} mode)...\n"
                                       f"log: {log_path}").pack(padx=20, pady=20)
                loading.update()
                with open(log_path, "w") as fh:
                    fh.write(" ".join(cmd) + "\n\n")
                    fh.flush()
                    process = subprocess.Popen(cmd, stdout=fh, stderr=subprocess.STDOUT, text=True)

                def read_output():
                    process.wait()
                    loading.destroy()
                    if process.returncode == 0:
                        made = sorted(p.name for p in out_path.iterdir()
                                      if p.suffix in (".png", ".json", ".txt") and p.name != log_path.name)
                        messagebox.showinfo("DINOLand finished",
                        ...
                        try:
                            ...
                            pass
                        messagebox.showerror("DINOLand error",
                                             f"Exit code {process.returncode}. Last lines of {log_path}:\n\n{tail}")
    ...
    row += 1
    tk.Button(input_window, text="Run", command=submit, bg="lightgreen").grid(row=row, column=1, sticky='e', padx=5, pady=10)
    tk.Button(input_window, text="Cancel", command=input_window.destroy).grid(row=row, column=2, sticky='w', padx=5, pady=10)


# === ROW 3: Optional individual pipeline steps (recolored to indicate optional) ===
measure_button = tk.Button(scrollable_frame, text="Run Measurement Script", command=run_measurement_script, bg="#d5dbdb")
measure_button.grid(row=3, column=0, padx=0, pady=0)

# HIDDEN v67: Run 3D DfD Measurement (legacy v1 step)
...
semilandmark_button = tk.Button(scrollable_frame, text="Semi-Landmarking", command=run_semi_landmarking_script, bg="#d5dbdb")
semilandmark_button.grid(row=3, column=1, padx=0, pady=0)

color_segmentation_button = tk.Button(scrollable_frame, text="Color Segmentation", command=run_color_segmentation_script, bg="#d5dbdb")
color_segmentation_button.grid(row=3, column=2, padx=0, pady=0)

# HIDDEN v67: Color Extraction (superseded by BioRAG pipeline)
...
# HIDDEN v67: Color Pattern Analysis (superseded by BioRAG pipeline)
...
texture_phenomics_button = tk.Button(scrollable_frame, text="Texture Phenomics", command=run_texture_phenomics_script, bg="#d5dbdb")
texture_phenomics_button.grid(row=3, column=3, padx=0, pady=0)

texture_homology_button = tk.Button(scrollable_frame, text="Texture (Homology)", command=run_texture_homology_script, bg="#d5dbdb")
texture_homology_button.grid(row=3, column=4, padx=0, pady=0)

color_homology_button = tk.Button(scrollable_frame, text="Color (Homology)", command=run_color_homology_script, bg="#d5dbdb")
color_homology_button.grid(row=3, column=5, padx=0, pady=0)

try:
    from descriptron_species_delimiter_gui_v3 import open_species_delimiter_popup

    _btn_species_delim = tk.Button(
        scrollable_frame,
        text="Species Delimitation (v24)",
        command=lambda: open_species_delimiter_popup(root),
        bg="#e74c3c",
        fg="lightyellow",
        font=("Arial", 9, "bold")
    )
    _btn_species_delim.grid(row=4, column=0, padx=2, pady=2)

except ImportError:
    print("Species Delimitation not available")

# DINOLand: DINOv3 landmark transfer (gui/dinov3_landmark_transfer_v51.py)
btn_dinoland = tk.Button(scrollable_frame, text="DINOLand (v51)", command=run_dinoland_script,
                         bg="#16a085", fg="white", font=("Arial", 9, "bold"))
btn_dinoland.grid(row=4, column=1, padx=2, pady=2)



# HIDDEN v67: Visualize Contours & Bbox (legacy v1 step)
...
# HIDDEN v67: Convert TSV to JSONL (legacy v1 step)
...
# HIDDEN v67: Fine-tune GPT-4o Model (replaced by RAG in v2)
...
# HIDDEN v67: GPT-4 Featurize (replaced by BioRAG pipeline)
...
# HIDDEN v67: GPT-4 Specimens Examined (replaced by BioRAG pipeline)
...
# HIDDEN v67: Parse Material Examined (replaced by BioRAG pipeline)
...
# HIDDEN v67: Concat Description (replaced by BioRAG pipeline)
...
# HIDDEN v67: Generate Species Descriptions (replaced by BioRAG pipeline)
...
# HIDDEN v67: Refine Output and Generate Dichotomous Key (replaced by BioRAG pipeline)
...
# HIDDEN v67: Setup Models Comparison CSV (legacy v1 step)
...
# HIDDEN v67: Calculate Rogue Stats (legacy v1 step)
...
# === ONTOLOGY ROSETTA — Row 5 Buttons ===
if _ONTOLOGY_AVAILABLE:
    btn_load_template = tk.Button(scrollable_frame, text="Load Template",
                                   command=open_template_popup,
                                   bg="#2ecc71", fg="white", font=("Arial", 9, "bold"))
    btn_load_template.grid(row=5, column=0, padx=2, pady=2)

    btn_import_gbif = tk.Button(scrollable_frame, text="Import GBIF COCO",
                                 command=import_gbif_coco,
                                 bg="#3498db", fg="white", font=("Arial", 9, "bold"))
    btn_import_gbif.grid(row=5, column=1, padx=2, pady=2)

    btn_export_enhanced = tk.Button(scrollable_frame, text="Export Enhanced COCO",
                                     command=export_enhanced_coco,
                                     bg="#9b59b6", fg="white", font=("Arial", 9, "bold"))
    btn_export_enhanced.grid(row=5, column=2, padx=2, pady=2)

    btn_export_kg = tk.Button(scrollable_frame, text="Export Knowledge Graph",
                               command=export_knowledge_graph,
                               bg="#e67e22", fg="white", font=("Arial", 9, "bold"))
    btn_export_kg.grid(row=5, column=3, padx=2, pady=2)
else:
    tk.Label(scrollable_frame, text="Ontology: not available",
             fg="gray", font=("Arial", 8)).grid(row=5, column=0, columnspan=4, padx=2, pady=2)

# Hide / Show keypoints toggle (supports multiple sets via popup)
hide_keypoints_btn = tk.Button(scrollable_frame, text="Hide Keypoints", command=toggle_hide_keypoints, bg="#d9d9d9")
hide_keypoints_btn.grid(row=5, column=4, padx=2, pady=2)

# === BioRAG PIPELINE + ZENODO — Row 6 ===
btn_biorag = tk.Button(scrollable_frame, text="BioRAG Pipeline",
                       command=open_biorag_popup,
                       bg="#27ae60", fg="white", font=("Arial", 9, "bold"))
btn_biorag.grid(row=6, column=0, padx=2, pady=2)

btn_delimit = tk.Button(scrollable_frame, text="QC + comparisons",
                        command=open_delimitation_popup,
                        bg="#2980b9", fg="white", font=("Arial", 9, "bold"))
btn_delimit.grid(row=6, column=2, padx=2, pady=2)

btn_zenodo = tk.Button(scrollable_frame, text="Upload to Zenodo",
                       command=open_zenodo_popup,
                       bg="#2980b9", fg="white", font=("Arial", 9, "bold"))
btn_zenodo.grid(row=6, column=1, padx=2, pady=2)

# Video button (moved to row 3 in v67 to resolve collision with COCO->MinCOCO)
btn_load_video = tk.Button(scrollable_frame, text="Load Video for SAM2",
                          command=open_video_remote_control, bg="lightyellow",
                          font=("Arial", 9, "bold"))
btn_load_video.grid(row=3, column=6, padx=0, pady=0)
...
marmot_button.grid(row=4, column=1, padx=0, pady=0)

# === v74: format converter dialog (measure/descriptron_convert.py) ================
_CONVERSIONS = [  # (label, subcommand, input is a folder?, output extension)
    ("tpsDig .tps -> COCO",                                "tps2coco",         False, ".json"),
    ("COCO -> tpsDig .tps",                                "coco2tps",         False, ".tps"),
    ("MorphoJ text -> COCO",                               "morphoj2coco",     False, ".json"),
    ("COCO -> MorphoJ text",                               "coco2morphoj",     False, ".txt"),
    ("StereoMorph shape files (folder) -> COCO",           "stereomorph2coco", True,  ".json"),
    ("Landmark table (image, landmark, x, y) -> COCO",     "table2coco",       False, ".json"),
    ("VIA 2 project / region JSON / VIA COCO export -> COCO", "via2coco",      False, ".json"),
]


def open_convert_formats_dialog():
    """Convert tpsDig, MorphoJ, StereoMorph, landmark tables and VIA 2 files to Descriptron COCO and back."""
    here = os.path.dirname(os.path.abspath(__file__))
    script = os.path.join(here, "measure", "descriptron_convert.py")
    if not os.path.isfile(script):
        try:
            from descriptron_core.cli import tools_dir as _td
            script = str(_td() / "descriptron_convert.py")
        except Exception:
            pass
    win = tk.Toplevel(root)
    win.title("Convert annotation / landmark formats")
    conv = tk.StringVar(value=_CONVERSIONS[0][0])
    inp, imgs, out, names, per_img = (tk.StringVar() for _ in range(5))
    category = tk.StringVar(value="landmarks")
    yorig = tk.StringVar(value="default")
    r = 0
    tk.Label(win, text="Conversion:").grid(row=r, column=0, sticky="e", padx=5, pady=4)
    tk.OptionMenu(win, conv, *[c[0] for c in _CONVERSIONS]).grid(row=r, column=1, columnspan=2, sticky="w", padx=5)

    def spec():
        return next(c for c in _CONVERSIONS if c[0] == conv.get())

    def browse(var, kind):
        if kind == "in":
            p = filedialog.askdirectory(title="Folder of StereoMorph shape files") if spec()[2] else \
                filedialog.askopenfilename(title="Input file")
        elif kind == "dir":
            p = filedialog.askdirectory(title="Folder")
        elif kind == "names":
            p = filedialog.askopenfilename(title="Landmark names (one per line)", filetypes=[("Text", "*.txt"), ("All", "*.*")])
        else:
            p = filedialog.asksaveasfilename(title="Output file", defaultextension=spec()[3])
        if p:
            var.set(p)
    rows = [("Input file (or folder):", inp, "in"), ("Images folder:", imgs, "dir"), ("Output file:", out, "out"),
            ("Landmark names file (optional):", names, "names"),
            ("Also one file per image in (optional):", per_img, "dir")]
    for label, var, kind in rows:
        r += 1
        tk.Label(win, text=label).grid(row=r, column=0, sticky="e", padx=5, pady=3)
        tk.Entry(win, textvariable=var, width=52).grid(row=r, column=1, padx=5)
        tk.Button(win, text="Browse", command=lambda v=var, k=kind: browse(v, k)).grid(row=r, column=2, padx=5)
    r += 1
    tk.Label(win, text="Landmark category name:").grid(row=r, column=0, sticky="e", padx=5, pady=3)
    tk.Entry(win, textvariable=category, width=24).grid(row=r, column=1, sticky="w", padx=5)
    r += 1
    tk.Label(win, text="y = 0 in the file is at:").grid(row=r, column=0, sticky="e", padx=5, pady=3)
    tk.OptionMenu(win, yorig, "default", "top", "bottom").grid(row=r, column=1, sticky="w", padx=5)
    r += 1
    tk.Label(win, fg="gray30", justify="left", wraplength=560, text=(
        "Default: bottom for tpsDig and MorphoJ (tpsDig counts y upwards), top for StereoMorph, tables and VIA. "
        "The images folder is needed for the image sizes and to flip y. Old Descriptron .tps files: choose 'top'. "
        "VIA: points become numbered landmarks and polylines become line annotations (VIA's own COCO export "
        "drops both - convert the VIA project JSON to keep them). 'One file per image' writes the form "
        "DINOLand uses for references.")).grid(row=r, column=0, columnspan=3, sticky="w", padx=8, pady=4)

    def run():
        sub, is_dir = spec()[1], spec()[2]
        if not inp.get() or not out.get():
            messagebox.showerror("Convert", "Choose an input and an output file.", parent=win); return
        cmd = [sys.executable, script, sub, inp.get(), "--out", out.get()]
        if imgs.get():
            cmd += ["--images", imgs.get()]
        if yorig.get() != "default":
            cmd += ["--y-origin", yorig.get()]
        if sub not in ("coco2tps", "coco2morphoj"):
            if names.get():
                cmd += ["--names", names.get()]
            if category.get().strip():
                cmd += ["--category", category.get().strip()]
            if per_img.get():
                cmd += ["--per-image", per_img.get()]
        try:
            p = subprocess.run(cmd, capture_output=True, text=True, timeout=3600)
        except Exception as e:
            messagebox.showerror("Convert", str(e), parent=win); return
        msg = (p.stdout.strip() + ("\n\n" + p.stderr.strip() if p.stderr.strip() else ""))[-2500:]
        (messagebox.showinfo if p.returncode == 0 else messagebox.showerror)(
            "Convert" + ("" if p.returncode == 0 else " failed"), msg or "done", parent=win)
    r += 1
    tk.Button(win, text="Convert", command=run, bg="lightgreen").grid(row=r, column=1, sticky="e", padx=5, pady=8)
    tk.Button(win, text="Close", command=win.destroy).grid(row=r, column=2, sticky="w", padx=5, pady=8)


btn_convert_formats = tk.Button(scrollable_frame, text="Convert formats (TPS, MorphoJ, VIA...)",
                                command=open_convert_formats_dialog, bg="#6c8893", fg="white",
                                font=("Arial", 9, "bold"))

# === v74: housekeeping dialogs (measure/descriptron_coco_tools.py) ===============
def _tool_script(name):
    here = os.path.dirname(os.path.abspath(__file__))
    p = os.path.join(here, "measure", name)
...
def _run_tool_show(cmd, title, parent):
    """Run a tool and show everything it printed in a scrollable window (no freezing: runs in a thread)."""
    import tkinter.scrolledtext as _st
    out = tk.Toplevel(parent); out.title(title)
    txt = _st.ScrolledText(out, width=110, height=30, font=("Courier", 9)); txt.pack(fill="both", expand=True)
    txt.insert("end", "running...\n")
    ...
    def work():
        try:
            ...
            res = f"failed: {e}"
        def show():
            txt.delete("1.0", "end"); txt.insert("end", res or "done")
        out.after(0, show)
    threading.Thread(target=work, daemon=True).start()


def _entry_row(win, r, label, var, browse=None):
    tk.Label(win, text=label).grid(row=r, column=0, sticky="e", padx=5, pady=3)
    tk.Entry(win, textvariable=var, width=55).grid(row=r, column=1, padx=5)
    if browse:
        tk.Button(win, text="Browse", command=browse).grid(row=r, column=2, padx=5)


def open_prepare_d2_dialog():
    win = tk.Toplevel(root); win.title("Prepare for Detectron2")
    ins, imgs, outd, regex, frac = tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(), tk.StringVar(value="0.2")
    mode = tk.StringVar(value="all")
    _entry_row(win, 0, "COCO JSON(s):", ins, lambda: ins.set(";".join(filedialog.askopenfilenames(title="COCO JSON (several = combine first)"))))
    _entry_row(win, 1, "Images folder:", imgs, lambda: imgs.set(filedialog.askdirectory(title="Images folder") or imgs.get()))
    _entry_row(win, 2, "Output folder:", outd, lambda: outd.set(filedialog.askdirectory(title="Output folder") or outd.get()))
    _entry_row(win, 3, "Validation share:", frac)
    _entry_row(win, 4, "Specimen ID pattern (regex):", regex)
    tk.Label(win, text="Train:").grid(row=5, column=0, sticky="e")
    fr = tk.Frame(win); fr.grid(row=5, column=1, sticky="w")
    for val, lab in (("all", "masks + keypoints"), ("seg", "masks only"), ("kp", "keypoints only")):
        tk.Radiobutton(fr, text=lab, variable=mode, value=val).pack(side="left")
    tk.Label(win, fg="gray30", justify="left", wraplength=620, text=(
        "Runs the existing Combine COCO (if several files) and COCO->MinCOCO steps unchanged, then splits into "
        "train.json and val.json. Give a specimen ID pattern, e.g. (CASENT\\d+), so all images of one specimen stay "
        "on the same side. Then train Detectron2 with train.json and put val.json in 'Validation COCO JSON'.")
             ).grid(row=6, column=0, columnspan=3, sticky="w", padx=8, pady=4)

    def go():
        files = [x for x in ins.get().split(";") if x.strip()]
        if not files or not outd.get():
            messagebox.showerror("Prepare for Detectron2", "Choose COCO JSON(s) and an output folder.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_coco_tools.py"), "prepare-d2", *files, "--out-dir", outd.get(),
               "--val-fraction", frac.get() or "0.2"]
        if imgs.get():
            cmd += ["--images", imgs.get()]
        if regex.get().strip():
            cmd += ["--group-regex", regex.get().strip()]
        if mode.get() == "seg":
            cmd.append("--segmentation-only")
        elif mode.get() == "kp":
            cmd.append("--keypoints-only")
        _run_tool_show(cmd, "Prepare for Detectron2", win)
    tk.Button(win, text="Prepare", command=go, bg="lightgreen").grid(row=7, column=1, sticky="e", pady=8)
    tk.Button(win, text="Close", command=win.destroy).grid(row=7, column=2, sticky="w", pady=8)


def open_check_annotations_dialog():
    win = tk.Toplevel(root); win.title("Check annotations / rename categories")
    js, imgs, tmpl = tk.StringVar(), tk.StringVar(), tk.StringVar()
    _entry_row(win, 0, "COCO JSON:", js, lambda: js.set(filedialog.askopenfilename(title="COCO JSON") or js.get()))
    _entry_row(win, 1, "Images folder (optional):", imgs, lambda: imgs.set(filedialog.askdirectory(title="Images folder") or imgs.get()))
    _entry_row(win, 2, "SAM2-PAL template image (optional):", tmpl, lambda: tmpl.set(filedialog.askopenfilename(title="Template image") or tmpl.get()))

    def check():
        if not js.get():
            messagebox.showerror("Check", "Choose a COCO JSON.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_coco_tools.py"), "check", js.get(), "--show", "50"]
        if imgs.get():
            cmd += ["--images", imgs.get()]
        if tmpl.get():
            cmd += ["--template", tmpl.get()]
        _run_tool_show(cmd, "Annotation check", win)
    tk.Button(win, text="Check", command=check, bg="lightgreen").grid(row=3, column=1, sticky="e", pady=6)
    tk.Label(win, text="Rename / merge categories (one 'old name=new name' per line; the same new name merges):"
             ).grid(row=4, column=0, columnspan=3, sticky="w", padx=8, pady=(10, 2))
    box = tk.Text(win, width=70, height=8); box.grid(row=5, column=0, columnspan=3, padx=8)

    def fill():
        try:
            with open(js.get()) as fh:
                names = [c["name"] for c in json.load(fh).get("categories", [])]
        except Exception as e:
            messagebox.showerror("Rename", str(e), parent=win); return
        box.delete("1.0", "end"); box.insert("end", "\n".join(f"{n}={n}" for n in names))

    def rename():
        maps = [l.strip() for l in box.get("1.0", "end").splitlines() if "=" in l and l.split("=", 1)[0] != l.split("=", 1)[1]]
        if not js.get() or not maps:
            messagebox.showerror("Rename", "Choose a COCO JSON and change at least one name.", parent=win); return
        outp = filedialog.asksaveasfilename(title="Save renamed COCO as", defaultextension=".json", parent=win)
        if not outp:
            return
        _run_tool_show([sys.executable, _tool_script("descriptron_coco_tools.py"), "rename", js.get(), "--out", outp,
                        "--map", *maps], "Rename categories", win)
    fr = tk.Frame(win); fr.grid(row=6, column=0, columnspan=3, pady=6)
    tk.Button(fr, text="List current categories", command=fill).pack(side="left", padx=4)
    tk.Button(fr, text="Save renamed copy...", command=rename).pack(side="left", padx=4)
    tk.Button(fr, text="Close", command=win.destroy).pack(side="left", padx=4)


def open_folder_housekeeping_dialog():
    win = tk.Toplevel(root); win.title("Folder housekeeping")
    folder = tk.StringVar()
    dot, masks, tiff = tk.BooleanVar(value=True), tk.BooleanVar(value=True), tk.BooleanVar(value=False)
    _entry_row(win, 0, "Image folder:", folder, lambda: folder.set(filedialog.askdirectory(title="Image folder") or folder.get()))
    tk.Checkbutton(win, text="Delete macOS '._' files", variable=dot).grid(row=1, column=1, sticky="w")
    tk.Checkbutton(win, text="Move mask PNGs (*_mask.png) into _masks_moved_out/", variable=masks).grid(row=2, column=1, sticky="w")
    tk.Checkbutton(win, text="Write 8-bit PNG copies of TIFFs into _png8/ (originals kept)", variable=tiff).grid(row=3, column=1, sticky="w")

    def run(apply):
        if not folder.get():
            messagebox.showerror("Folders", "Choose a folder.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_coco_tools.py"), "folders", folder.get()]
        cmd += (["--remove-dot-underscore"] if dot.get() else []) + (["--move-masks"] if masks.get() else []) + \
               (["--tiff8"] if tiff.get() else []) + (["--apply"] if apply else [])
        if apply and not messagebox.askyesno("Folders", "Apply these changes to the folder?", parent=win):
            return
        _run_tool_show(cmd, "Folder housekeeping" + ("" if apply else " (preview)"), win)
    fr = tk.Frame(win); fr.grid(row=4, column=1, sticky="e", pady=8)
    tk.Button(fr, text="Preview", command=lambda: run(False)).pack(side="left", padx=4)
    tk.Button(fr, text="Apply", command=lambda: run(True), bg="#f1948a").pack(side="left", padx=4)
    tk.Button(fr, text="Close", command=win.destroy).pack(side="left", padx=4)


btn_prepare_d2 = tk.Button(scrollable_frame, text="Prepare for Detectron2", command=open_prepare_d2_dialog,
                           bg="#6c8893", fg="white", font=("Arial", 9, "bold"))
btn_check_annotations = tk.Button(scrollable_frame, text="Check annotations / rename categories",
                                  command=open_check_annotations_dialog, bg="#6c8893", fg="white", font=("Arial", 9, "bold"))
btn_folder_housekeeping = tk.Button(scrollable_frame, text="Folder housekeeping", command=open_folder_housekeeping_dialog,
                                    bg="#6c8893", fg="white", font=("Arial", 9, "bold"))

# === v74: automatic centre lines (measure/descriptron_centerline.py) - extra data, nothing replaced ===
def open_centerline_dialog():
    win = tk.Toplevel(root); win.title("Centre lines of thin structures (veins, antennae, legs, setae)")
    js, cats, imgs, scales, ppm, outd = (tk.StringVar() for _ in range(6))
    branches, overlays = tk.BooleanVar(value=False), tk.BooleanVar(value=True)
    minb, smooth = tk.StringVar(), tk.StringVar(value="2")
    _entry_row(win, 0, "COCO JSON with masks:", js, lambda: js.set(filedialog.askopenfilename(title="COCO JSON") or js.get()))
    _entry_row(win, 1, "Categories (space separated; empty = all):", cats)
    _entry_row(win, 2, "Images folder (for overlays):", imgs, lambda: imgs.set(filedialog.askdirectory(title="Images") or imgs.get()))
    _entry_row(win, 3, "Scales CSV (e.g. combined_scales.csv):", scales, lambda: scales.set(filedialog.askopenfilename(title="Scales CSV") or scales.get()))
    _entry_row(win, 4, "...or one scale for all (px per mm):", ppm)
    _entry_row(win, 5, "Output folder:", outd, lambda: outd.set(filedialog.askdirectory(title="Output folder") or outd.get()))
    _entry_row(win, 6, "Smoothing (px):", smooth)
    tk.Checkbutton(win, text="Write overlay images", variable=overlays).grid(row=7, column=1, sticky="w")
    tk.Checkbutton(win, text="Keep branches (branch graph instead of the longest single path)",
                   variable=branches).grid(row=8, column=1, sticky="w")
    _entry_row(win, 9, "Branch mode: ignore spurs shorter than (px):", minb)

    def list_cats():
        try:
            with open(js.get()) as fh:
                names = [c["name"] for c in json.load(fh).get("categories", []) if not c.get("keypoints")]
            messagebox.showinfo("Categories", ", ".join(names) or "(none)", parent=win)
        except Exception as e:
            messagebox.showerror("Categories", str(e), parent=win)
    tk.Button(win, text="List categories", command=list_cats).grid(row=1, column=2, padx=5)
    tk.Label(win, fg="gray30", justify="left", wraplength=640, text=(
        "Adds curved length, straight (chord) length, sinuosity, width and curvature along each mask's centre line - "
        "for veins, antennae, legs and setae, where a straight measurement under-reads curvature (by 17-23% on test "
        "arcs and S-curves). The existing measurements are not changed. Results: centerline_measurements.csv, plus "
        "centerlines_coco.json to check the lines in the GUI (Load Annotations).")
             ).grid(row=10, column=0, columnspan=3, sticky="w", padx=8, pady=4)

    def go():
        if not js.get() or not outd.get():
            messagebox.showerror("Centre lines", "Choose a COCO JSON and an output folder.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_centerline.py"), js.get(), "--out-dir", outd.get(),
               "--smooth", smooth.get() or "2"]
        if cats.get().strip():
            cmd += ["--categories", *cats.get().split()]
        if imgs.get():
            cmd += ["--images", imgs.get()]
        if scales.get():
            cmd += ["--scales", scales.get()]
        if ppm.get().strip():
            cmd += ["--px-per-mm", ppm.get().strip()]
        if overlays.get() and imgs.get():
            cmd.append("--overlays")
        if branches.get():
            cmd.append("--branches")
            if minb.get().strip():
                cmd += ["--min-branch", minb.get().strip()]
        _run_tool_show(cmd, "Centre lines", win)
    tk.Button(win, text="Measure", command=go, bg="lightgreen").grid(row=11, column=1, sticky="e", pady=8)
    tk.Button(win, text="Close", command=win.destroy).grid(row=11, column=2, sticky="w", pady=8)


btn_centerline = tk.Button(scrollable_frame, text="Centre lines (thin structures)", command=open_centerline_dialog,
                           bg="#d5dbdb")

# === v74: joints (measure/descriptron_joints.py) - angles as characters + pose standardisation ===
def open_joints_dialog():
    win = tk.Toplevel(root); win.title("Joints: angles and pose standardisation")
    js, outp, joints, rotate, target = (tk.StringVar() for _ in range(5))
    mode = tk.StringVar(value="angles")
    _entry_row(win, 0, "COCO keypoints JSON:", js, lambda: js.set(filedialog.askopenfilename(title="COCO keypoints") or js.get()))
    fr = tk.Frame(win); fr.grid(row=1, column=1, sticky="w")
    tk.Radiobutton(fr, text="Measure joint angles", variable=mode, value="angles").pack(side="left")
    tk.Radiobutton(fr, text="Standardise pose (new COCO for GPA)", variable=mode, value="std").pack(side="left")
    _entry_row(win, 2, "Joints (angles; empty = from skeleton):", joints)
    _entry_row(win, 3, "Rotate (standardise), e.g. 2:1,3 :", rotate)
    _entry_row(win, 4, "Target angle, degrees (optional):", target)
    _entry_row(win, 5, "Output folder (angles) / file (standardise):", outp,
               lambda: outp.set((filedialog.askdirectory(title="Output folder") if mode.get() == "angles"
                                 else filedialog.asksaveasfilename(title="Standardised COCO", defaultextension=".json")) or outp.get()))
    tk.Label(win, fg="gray30", justify="left", wraplength=620, text=(
        "Angles: one value per joint (pivot landmark between two neighbours), from the pose skeleton or listed as "
        "PIVOT:A,B (landmark numbers), several separated by spaces. Standardise: rotates the part beyond the pivot so "
        "every specimen has the same angle A-PIVOT-B (sample mean unless a target is given), like geomorph fixed.angle; "
        "writes a new COCO file to run the landmark GPA on. The original file is not changed.")
             ).grid(row=6, column=0, columnspan=3, sticky="w", padx=8, pady=4)

    def go():
        if not js.get() or not outp.get():
            messagebox.showerror("Joints", "Choose a COCO file and an output.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_joints.py")]
        if mode.get() == "angles":
            cmd += ["angles", js.get(), "--out-dir", outp.get()]
            for j in joints.get().split():
                cmd += ["--joint", j]
        else:
            if not rotate.get().strip():
                messagebox.showerror("Joints", "Give the joint to standardise, e.g. 2:1,3", parent=win); return
            cmd += ["standardise", js.get(), "--out", outp.get()]
            for r in rotate.get().split():
                cmd += ["--rotate", r]
            if target.get().strip():
                cmd += ["--target-angle", target.get().strip()]
        _run_tool_show(cmd, "Joints", win)
    tk.Button(win, text="Run", command=go, bg="lightgreen").grid(row=7, column=1, sticky="e", pady=8)
    tk.Button(win, text="Close", command=win.destroy).grid(row=7, column=2, sticky="w", pady=8)


btn_joints = tk.Button(scrollable_frame, text="Joints (angles, pose)", command=open_joints_dialog, bg="#d5dbdb")

# === v74: specimen metadata editor + metadata-driven shape statistics ===========================
# (measure/descriptron_metadata.py, measure/descriptron_shape_stats.py; additions, existing analyses unchanged)
def _load_tool_module(name):
    import importlib.util as _iu
    p = _tool_script(name + ".py")
...
def open_metadata_dialog():
    dm = _load_tool_module("descriptron_metadata")
    win = tk.Toplevel(root); win.title("Specimen metadata and shape statistics"); win.geometry("1150x760")
    state = {"cols": [], "rows": [], "schema": {}}
    top = tk.Frame(win); top.pack(fill="x", padx=6, pady=4)
    table_frame = tk.Frame(win); table_frame.pack(fill="both", expand=True, padx=6)
    roles_frame = tk.LabelFrame(win, text="What each column is (roles used by the analyses)", padx=4, pady=2)
    roles_frame.pack(fill="x", padx=6, pady=4)
    ...
    def refresh():
        ...
        tree["columns"] = state["cols"]
        for c in state["cols"]:
            tree.heading(c, text=c); tree.column(c, width=120, stretch=False)
        for i, r in enumerate(state["rows"]):
            tree.insert("", "end", iid=str(i), values=[r.get(c, "") for c in state["cols"]])
        for w in roles_frame.winfo_children():
            w.destroy()
        role_vars.clear()
        for j, c in enumerate(state["cols"]):
            fr = tk.Frame(roles_frame); fr.grid(row=j // 4, column=j % 4, sticky="w", padx=4, pady=1)
            tk.Label(fr, text=(c if len(c) <= 26 else c[:24] + ".."), width=26, anchor="w").pack(side="left")
            v = tk.StringVar(value=state["schema"].get(c, "ignore")); role_vars[c] = v
            ttk.Combobox(fr, textvariable=v, values=dm.ROLES, width=11, state="readonly").pack(side="left")

    def sync_roles():
        state["schema"] = {c: role_vars[c].get() for c in state["cols"] if c in role_vars}

    def edit_cell(event):
        item, col = tree.identify_row(event.y), tree.identify_column(event.x)
        if not item or not col:
            return
        ci = int(col[1:]) - 1; cname = state["cols"][ci]
        x, y, w, h = tree.bbox(item, col)
        e = tk.Entry(tree); e.place(x=x, y=y, width=w, height=h)
        e.insert(0, state["rows"][int(item)].get(cname, "")); e.focus_set()
        def done(_=None):
            state["rows"][int(item)][cname] = e.get()
            tree.set(item, cname, e.get()); e.destroy()
        e.bind("<Return>", done); e.bind("<FocusOut>", done); e.bind("<Escape>", lambda _e: e.destroy())
    tree.bind("<Double-1>", edit_cell)

    def do_import():
        p = filedialog.askopenfilename(parent=win, title="Metadata: table (CSV/TSV/Excel) or Darwin Core (occurrence.txt / DwC-A .zip)",
                                       filetypes=[("Tables and Darwin Core", "*.csv *.tsv *.txt *.xlsx *.zip"), ("All", "*.*")])
        if not p:
            return
        try:
            cols, rows, kind = dm.read_any(p)
        except SystemExit as e:
            messagebox.showerror("Import", str(e), parent=win); return
        state.update(cols=cols, rows=rows, schema=dm.default_schema(cols, rows)); refresh()
        messagebox.showinfo("Import", f"{len(rows)} rows, {len(cols)} columns ({kind}). Check the roles below.", parent=win)

    def new_from_coco():
        p = filedialog.askopenfilename(parent=win, title="COCO JSON: one row per image", filetypes=[("COCO JSON", "*.json")])
        if not p:
            return
        ims = json.load(open(p)).get("images", [])
        state.update(cols=["image", "specimen", "species", "locality", "sex"],
                     rows=[{"image": im["file_name"], "specimen": os.path.splitext(im["file_name"])[0]} for im in ims],
                     schema={"image": "image", "specimen": "ignore", "species": "group", "locality": "factor", "sex": "factor"})
        refresh()

    def add_row():
        state["rows"].append({c: "" for c in state["cols"]}); refresh(); tree.see(str(len(state["rows"]) - 1))

    def del_rows():
        for item in sorted((int(i) for i in tree.selection()), reverse=True):
            del state["rows"][item]
        refresh()

    def add_col():
        name = simpledialog.askstring("Add column", "Column name:", parent=win)
        if name and name not in state["cols"]:
            sync_roles(); state["cols"].append(name); state["schema"][name] = "factor"; refresh()

    def save():
        if not state["cols"]:
            return None
        sync_roles()
        d = filedialog.askdirectory(parent=win, title="Folder for specimen_metadata.csv + metadata_schema.json")
        if not d:
            return None
        dm.write_table(state["cols"], state["rows"], os.path.join(d, "specimen_metadata.csv"))
        json.dump(state["schema"], open(os.path.join(d, "metadata_schema.json"), "w"), indent=1)
        state["folder"] = d
        messagebox.showinfo("Save", f"Saved specimen_metadata.csv and metadata_schema.json in {d}", parent=win)
        return d

    def check_link():
        sync_roles()
        p = filedialog.askopenfilename(parent=win, title="COCO JSON to link", filetypes=[("COCO JSON", "*.json")])
        if not p:
            return
        names = [im["file_name"] for im in json.load(open(p)).get("images", [])]
        m, un = dm.link_images(state["rows"], state["schema"], names)
        messagebox.showinfo("Link", f"{len(m)} of {len(names)} images linked to {len(set(m.values()))} specimens."
                            + (f"\nNot linked, e.g.: {', '.join(un[:5])}" if un else ""), parent=win)

    for txt, cmd in (("Import table / Darwin Core...", do_import), ("New table from COCO images...", new_from_coco),
                     ("Add row", add_row), ("Delete selected rows", del_rows), ("Add column...", add_col),
                     ("Check links to images...", check_link), ("Save...", save)):
        tk.Button(top, text=txt, command=cmd).pack(side="left", padx=2)
    tk.Label(top, text="double-click a cell to edit", fg="gray40").pack(side="left", padx=8)

    # ---- analyses ----
    an = tk.LabelFrame(win, text="Shape statistics (in addition to the existing GPA / semilandmark analyses)", padx=4, pady=4)
    an.pack(fill="x", padx=6, pady=4)
    shapes, formula, pairs, modules, treef, outd, category = (tk.StringVar() for _ in range(7))
    iters = tk.StringVar(value="999")
    r = 0
    for label, var, br in (("Landmarks (COCO keypoints or .tps):", shapes, lambda: shapes.set(filedialog.askopenfilename(parent=win) or shapes.get())),
                           ("Formula (empty = group + factors + covariates):", formula, None),
                           ("Paired landmarks for asymmetry (e.g. 1-5,2-6):", pairs, None),
                           ("Modules (e.g. 1,2,3|4,5,6):", modules, None),
                           ("Tree (Newick; tips = group names):", treef, lambda: treef.set(filedialog.askopenfilename(parent=win) or treef.get())),
                           ("Output folder:", outd, lambda: outd.set(filedialog.askdirectory(parent=win) or outd.get()))):
        tk.Label(an, text=label).grid(row=r, column=0, sticky="e")
        tk.Entry(an, textvariable=var, width=60).grid(row=r, column=1, sticky="w", padx=4)
        if br:
            tk.Button(an, text="Browse", command=br).grid(row=r, column=2)
        r += 1
    tk.Label(an, text="Permutations:").grid(row=r, column=0, sticky="e"); tk.Entry(an, textvariable=iters, width=8).grid(row=r, column=1, sticky="w", padx=4)
    r += 1
    checks = {}
    cf = tk.Frame(an); cf.grid(row=r, column=0, columnspan=3, sticky="w")
    for i, nm in enumerate(["anova", "allometry", "trajectory", "disparity", "pls", "mantel", "asymmetry", "modularity", "assign", "phylosignal"]):
        checks[nm] = tk.BooleanVar(value=nm in ("anova", "allometry", "disparity", "assign"))
        tk.Checkbutton(cf, text=nm, variable=checks[nm]).grid(row=0, column=i, sticky="w")

    def run():
        folder = state.get("folder") or save()
        if not folder or not shapes.get() or not outd.get():
            messagebox.showerror("Statistics", "Save the metadata, and choose the landmark file and an output folder.", parent=win); return
        cmd = [sys.executable, _tool_script("descriptron_shape_stats.py"), shapes.get(),
               "--metadata", os.path.join(folder, "specimen_metadata.csv"), "--schema", os.path.join(folder, "metadata_schema.json"),
               "--out-dir", outd.get(), "--iterations", iters.get() or "999",
               "--analyses", *[k for k, v in checks.items() if v.get()]]
        for flag, var in (("--formula", formula), ("--pairs", pairs), ("--modules", modules), ("--tree", treef)):
            if var.get().strip():
                cmd += [flag, var.get().strip()]
        _run_tool_show(cmd, "Shape statistics", win)
    tk.Button(an, text="Run analyses", command=run, bg="lightgreen").grid(row=r + 1, column=1, sticky="w", pady=6)
    tk.Label(an, fg="gray30", justify="left", wraplength=1050, text=(
        "Results: CSV tables per analysis and shape_stats_report.md in the output folder. Mirror-image specimens are "
        "detected and reflected before alignment (listed in specimens.csv). Statistics checked against R's RRPP on "
        "identical data.")).grid(row=r + 2, column=0, columnspan=3, sticky="w")


btn_metadata_stats = tk.Button(scrollable_frame, text="Metadata & shape statistics", command=open_metadata_dialog,
                               bg="#b39ddb")

# === v74: video - individuals and poses (descriptron_video_track.py); the SAM2 video tool is unchanged ===
def open_video_track_dialog():
    here = os.path.dirname(os.path.abspath(__file__))
    script = os.path.join(here, "descriptron_video_track.py")
    if not os.path.isfile(script):          # pip install: it ships in descriptron-vision
        try:
            from descriptron_vision.cli import tools_dir as _vtd
            script = str(_vtd() / "descriptron_video_track.py")
        except Exception:
            pass
    win = tk.Toplevel(root); win.title("Video: track individuals and their pose")
    video, frames, init, labels, outd = (tk.StringVar() for _ in range(5))
    ckpt = tk.StringVar(value=os.path.join(os.path.dirname(here), "checkpoints", "sam2_hiera_large.pt"))
    every = tk.StringVar(value="1")
    rows = [("Video file:", video, lambda: video.set(filedialog.askopenfilename(parent=win, filetypes=[("Video", "*.mp4 *.avi *.mov *.mkv"), ("All", "*.*")]) or video.get())),
            ("Frames folder:", frames, lambda: frames.set(filedialog.askdirectory(parent=win) or frames.get())),
            ("Individuals on annotated frame(s) (COCO masks):", init, lambda: init.set(filedialog.askopenfilename(parent=win) or init.get())),
            ("Poses on labelled frame(s) (COCO keypoints):", labels, lambda: labels.set(filedialog.askopenfilename(parent=win) or labels.get())),
            ("Output folder:", outd, lambda: outd.set(filedialog.askdirectory(parent=win) or outd.get())),
            ("SAM2 checkpoint:", ckpt, lambda: ckpt.set(filedialog.askopenfilename(parent=win) or ckpt.get())),
            ("Use every Nth frame:", every, None)]
    for r, (label, var, br) in enumerate(rows):
        _entry_row(win, r, label, var, br)
    tk.Label(win, fg="gray30", justify="left", wraplength=640, text=(
        "1 Extract frames from the video. Open a frame in the GUI and mask each individual (one mask each; "
        "their order gives the individual numbers), save; place the pose skeleton's keypoints on the same or "
        "other frames, save. 2 SAM2 follows every individual through the frames. 3 The keypoints are carried "
        "forwards and backwards inside each individual, with a confidence per point; add labels on frames where "
        "points are dropped. 4 DeepLabCut CSV. 5 Overlay video to check.")).grid(row=len(rows), column=0, columnspan=3, sticky="w", padx=8, pady=4)

    def out(name):
        return os.path.join(outd.get(), name)

    def step(n):
        if not outd.get():
            messagebox.showerror("Video", "Choose an output folder.", parent=win); return
        os.makedirs(outd.get(), exist_ok=True)
        if not frames.get():
            frames.set(out("frames"))
        base = [sys.executable, script]
        if n == 1:
            cmd = base + ["frames", video.get(), "--out-dir", frames.get(), "--every", every.get() or "1"]
        elif n == 2:
            cmd = base + ["track", frames.get(), "--init", init.get(), "--out", out("tracks.json"),
                          "--sam2-checkpoint", ckpt.get(), "--sam2-config", "sam2_hiera_l.yaml"]
        elif n == 3:
            cmd = base + ["pose", frames.get(), "--tracks", out("tracks.json"), "--labels", labels.get(), "--out", out("pose.json")]
        elif n == 4:
            cmd = base + ["export-dlc", out("pose.json"), "--out", out("CollectedData_descriptron.csv")]
        else:
            cmd = base + ["overlay", frames.get(), "--tracks", out("tracks.json"), "--out", out("overlay.mp4")] + \
                  (["--pose", out("pose.json")] if os.path.exists(out("pose.json")) else [])
        _run_tool_show(cmd, f"Video step {n}", win)
    fr = tk.Frame(win); fr.grid(row=len(rows) + 1, column=0, columnspan=3, pady=8)
    for n, txt in enumerate(("1 Extract frames", "2 Track individuals", "3 Carry poses", "4 DeepLabCut CSV", "5 Overlay video"), 1):
        tk.Button(fr, text=txt, command=lambda n=n: step(n), bg="lightgreen" if n in (2, 3) else None).pack(side="left", padx=3)
    tk.Button(fr, text="Close", command=win.destroy).pack(side="left", padx=3)


btn_video_track = tk.Button(scrollable_frame, text="Video: individuals + pose", command=open_video_track_dialog,
                            bg="lightyellow")

# === v74: BINDER-TAB LAYOUT =====================================================
# An always-visible annotation toolbar (tools used in every workflow) above a row of
# coloured, numbered tabs, one per workflow. Every widget below was created earlier in
# this file with its own command and variables; here they are only *displayed* inside
# the toolbar / tab frames (grid in_=...), which are children of the same parent, so no
# button changes what it does. v73 keeps the previous (v68 compact) layout.

# Treatment DOCX builder (Material examined + Diagnosis + Description + Key + range maps)
btn_treatment_docx = tk.Button(scrollable_frame, text="Build Treatment DOCX",
                               command=run_build_treatment_docx_script,
                               bg="#8e44ad", fg="white", font=("Arial", 9, "bold"))
btn_dinoland.configure(text="DINOLand")          # runs v52 (v51 fallback); the old label said v51

button_frame.configure(height=290)

_TOOLBAR_BG = "#eef1f4"
_TABS = [  # (key, title, background)
    ("load",     "1  Load & navigate",          "#dbe6f2"),
    ("predict",  "2  Predict & propagate",      "#d3e6f7"),
    ("review",   "3  Review predictions",       "#eadcc9"),
    ("measure",  "4  Measure & analyse",        "#e4dcf1"),
    ("describe", "5  Describe & publish",       "#d3eeea"),
    ("utils",    "Utilities",                   "#e4e4e4"),
]


def _place(widget, frame, row, col, **kw):
    """Show an existing widget inside `frame` (a child of its parent) and keep it on top."""
    kw.setdefault("padx", 1); kw.setdefault("pady", 1)
    widget.grid(in_=frame, row=row, column=col, **kw)
    widget.lift()


def _row(frame, row, tag, bg, widgets, start=1):
    """One row = its own frame, so buttons sit tightly left to right (no shared column widths)."""
    rf = tk.Frame(scrollable_frame, bg=bg)
    rf.grid(in_=frame, row=row, column=0, sticky="w")
    rf.lift()
    lbl = tk.Label(scrollable_frame, text=tag, font=("Arial", 7, "bold"), fg="#555", bg=bg, width=11, anchor="e")
    _place(lbl, rf, 0, 0, sticky="e", padx=(0, 4))
    for c, w in enumerate([w for w in widgets if w is not None], start=start):
        _place(w, rf, 0, c)
    return rf


# ---- always-visible annotation toolbar ----
_toolbar = tk.Frame(scrollable_frame, bg=_TOOLBAR_BG, bd=0)
_toolbar.grid(row=1, column=0, sticky="ew", padx=2, pady=(0, 0))

# brush size: keep the Small/Big/Jumbo dropdown and add a slider (both set the same size)
_brush_slider_var = tk.IntVar(value=brush_sizes.get(brush_size_var.get(), 5))
brush_size_slider = tk.Scale(scrollable_frame, from_=1, to=150, orient="horizontal", length=110,
                             showvalue=True, variable=_brush_slider_var, bg=_TOOLBAR_BG,
                             highlightthickness=0, font=("Arial", 7),
                             command=lambda v: set_brush_size(int(float(v))))
def _sync_slider_from_menu(*_):
    try:
        _brush_slider_var.set(brush_sizes[brush_size_var.get()])
    except (KeyError, tk.TclError):
        pass
brush_size_var.trace("w", _sync_slider_from_menu)

_row(_toolbar, 0, "EDIT", _TOOLBAR_BG,
     [zoom_in_btn, zoom_out_btn, multi_mask_btn, paintbrush_btn, eraser_btn, brush_size_menu, brush_size_slider,
      keypoint_edit_btn, keypoints_to_mask_btn, keypoints_to_line_btn, hide_keypoints_btn])
_row(_toolbar, 1, "SAM2", _TOOLBAR_BG,
     [bbox_button, remove_bbox_btn, point_label_dropdown, point_prompt_btn, clear_points_btn,
      apply_points_prompt_btn, apply_prompt_btn])
_row(_toolbar, 2, "MASKS", _TOOLBAR_BG,
     [prev_btn, next_btn, trash_button, label_dropdown, apply_label_btn, save_keypoints_btn])
_toolbar.grid_columnconfigure(0, weight=1)

# v74: colour the toolbar by FUNCTION (the same colour means the same kind of tool in every row).
#   blue = view / navigate, orange = brush editing, green = SAM2 prompts + mask labels
#   (darker green = the Apply actions), purple = keypoints, red = trash (unchanged).
_TOOL_COLOURS = {
    "#cfe2f3": [zoom_in_btn, zoom_out_btn, multi_mask_btn, prev_btn, next_btn],
    "#fbdcb8": [paintbrush_btn, eraser_btn, brush_size_menu, brush_size_slider],
    "#d4edda": [bbox_button, remove_bbox_btn, point_label_dropdown, point_prompt_btn, clear_points_btn,
                label_dropdown],
    "#9fd8b0": [apply_points_prompt_btn, apply_prompt_btn, apply_label_btn],
    "#e4d4f4": [keypoint_edit_btn, keypoints_to_mask_btn, keypoints_to_line_btn, hide_keypoints_btn],
}
_KP_BG = "#e4d4f4"


def _darker(hexcol, f=0.88):
    r, g, b = (int(hexcol[i:i + 2], 16) for i in (1, 3, 5))
    return "#%02x%02x%02x" % (int(r * f), int(g * f), int(b * f))


for _col, _ws in _TOOL_COLOURS.items():
    for _w in _ws:
        try:
            _w.configure(bg=_col, activebackground=_darker(_col))
            if isinstance(_w, tk.Scale):
                _w.configure(troughcolor=_darker(_col, 0.8))
        except (tk.TclError, NameError):
            pass
try:   # Save Keypoints: the keypoint family's action button (was green)
    save_keypoints_btn.configure(bg="#7d5ba6", fg="white", activebackground="#6a4d8e", activeforeground="white")
except tk.TclError:
    pass
marmot_img_small = marmot_img.resize((48, 48), Image.LANCZOS)
marmot_thumb_small = ImageTk.PhotoImage(marmot_img_small)
marmot_button.configure(image=marmot_thumb_small)
marmot_button.image = marmot_thumb_small
_place(marmot_button, _toolbar, 0, 1, rowspan=3, sticky="ne", padx=(8, 2))

# ---- coloured binder tabs ----
_tabbar = tk.Frame(scrollable_frame)
_tabbar.grid(row=2, column=0, sticky="w", padx=2, pady=(4, 0))
_tab_frames, _tab_buttons = {}, {}
for key, title, bg in _TABS:
    _tab_frames[key] = tk.Frame(scrollable_frame, bg=bg, bd=1, relief="flat", padx=4, pady=3)


_GUI_STATE = os.path.join(os.path.expanduser("~"), ".config", "descriptron", "gui_state.json")


def _load_gui_state():
    try:
        with open(_GUI_STATE) as _fh:
            return json.load(_fh)
    except Exception:
        return {}


def _save_gui_state(**kw):
    """Last tab and window size, per user; purely a convenience, failures are ignored."""
    try:
        st = _load_gui_state(); st.update(kw)
        os.makedirs(os.path.dirname(_GUI_STATE), exist_ok=True)
        with open(_GUI_STATE, "w") as _fh:
            json.dump(st, _fh)
    except Exception:
        pass
...
def show_tab(key):
    for k, fr in _tab_frames.items():
        if k == key:
            fr.grid(row=3, column=0, sticky="ew", padx=2, pady=(0, 2))
        else:
            fr.grid_remove()
    for k, b in _tab_buttons.items():
        on = (k == key)
        b.configure(relief="sunken" if on else "raised", font=("Arial", 9, "bold" if on else "normal"),
                    bd=2 if on else 1)
    _save_gui_state(tab=key)


for i, (key, title, bg) in enumerate(_TABS):
    b = tk.Button(_tabbar, text=title, bg=bg, activebackground=bg, fg="#1f1f1c",
                  padx=10, pady=2, command=lambda k=key: show_tab(k))
    b.grid(row=0, column=i, padx=(0, 2))
    _tab_buttons[key] = b


def _fill(key, rows):
    fr, bg = _tab_frames[key], dict((k, c) for k, _, c in _TABS)[key]
    return [_row(fr, r, tag, bg, widgets) for r, (tag, widgets) in enumerate(rows)]


_load_rows = _fill("load", [
    ("FOLDER", [load_folder_btn, folder_prev_btn, folder_next_btn, folder_status_label]),
    ("OPEN",   [load_prompt_btn, load_btn, load_annotations_btn, load_labels_btn, btn_load_video,
                save_unlabeled_btn]),
])
# the auto-save checkbox was created without a name; find it among the frame's children
for _w in scrollable_frame.winfo_children():
    if isinstance(_w, tk.Checkbutton) and str(_w.cget("text")) == "auto-save KPs":
        _place(_w, _load_rows[0], 0, 5)
_fill("predict", [
    ("MASKS",     [btn_sam2_pal, train_button, predict_btn]),
    ("LANDMARKS", [btn_dinoland]),
    ("VIDEO", [btn_video_track]),
])
_fill("review", [
    ("VIEW", [view_predictions_btn, select_prediction_btn, prev_image_button, next_image_button]),
    ("FIX",  [category_label_dropdown, apply_category_label_btn, reannotate_btn]),
])
_fill("measure", [
    ("SHAPE", [measure_button, semilandmark_button, btn_centerline, btn_joints]),
    ("COLOUR", [color_segmentation_button, color_homology_button, texture_phenomics_button,
                texture_homology_button, btn_delimit]),
    ("STATS", [btn_metadata_stats]),
])
_describe_row = [btn_biorag, btn_treatment_docx, btn_zenodo]
_export_row = [btn_export_enhanced, btn_export_kg] if _ONTOLOGY_AVAILABLE else []
_describe_rows = _fill("describe", [("WRITE", _describe_row), ("EXPORT", _export_row)])
for _w in scrollable_frame.winfo_children():          # "Ontology: not available" note, if shown
    if isinstance(_w, tk.Label) and str(_w.cget("text")).startswith("Ontology: not available"):
        _place(_w, _describe_rows[1], 0, 1, sticky="w")
_util_row = [coco_to_min_btn, combine_json_btn, remove_files_from_json_btn]
_util_row2 = [btn_load_template, btn_import_gbif] if _ONTOLOGY_AVAILABLE else []
try:
    _util_row2.append(_btn_species_delim)          # legacy species delimiter (superseded by DINOSAR kNN)
except NameError:
    pass
_fill("utils", [("CONVERT", [btn_convert_formats]), ("COCO", [btn_prepare_d2] + _util_row),
                ("CHECK", [btn_check_annotations, btn_folder_housekeeping]), ("OTHER", _util_row2)])

_st = _load_gui_state()
_start_tab = _st.get("tab", "load")
show_tab(_start_tab if _start_tab in _tab_frames else "load")
if isinstance(_st.get("geometry"), str):
    try:
        root.geometry(_st["geometry"])
    except tk.TclError:
        pass


def _remember_geometry_and_close():
    _save_gui_state(geometry=root.geometry())
    close_window()

# === v74: ergonomics - undo for brush/eraser, keyboard shortcuts, tooltips, status bar =========
# Nothing above is changed: paint/erase are wrapped (select_paintbrush/select_eraser bind the global
# names at call time), shortcuts call the same functions as the buttons, and the status bar only reads.
import time as _time
_UNDO_STACK, _UNDO_MAX = [], 20
_stroke = {"t": 0.0, "idx": None}


def _snapshot_if_new_stroke():
    """A stroke starts when >0.35 s passed since the last brush event or the mask changed."""
    now = _time.monotonic()
    if segmentation_masks and (now - _stroke["t"] > 0.35 or _stroke["idx"] != current_mask_index):
        m = segmentation_masks[current_mask_index].get("segmentation")
        if m is not None:
            _UNDO_STACK.append((current_mask_index, m.copy()))
            del _UNDO_STACK[:-_UNDO_MAX]
    _stroke["t"], _stroke["idx"] = now, current_mask_index


_paint_orig, _erase_orig = paint, erase


def paint(event):                     # noqa: F811  (wraps the original; see note above)
    _snapshot_if_new_stroke()
    _paint_orig(event)


def erase(event):                     # noqa: F811
    _snapshot_if_new_stroke()
    _erase_orig(event)


def undo_last_stroke(event=None):
    global current_mask_index
    while _UNDO_STACK:
        idx, saved = _UNDO_STACK.pop()
        if idx < len(segmentation_masks) and segmentation_masks[idx].get("segmentation") is not None \
                and segmentation_masks[idx]["segmentation"].shape == saved.shape:
            current_mask_index = idx
            segmentation_masks[idx]["segmentation"][...] = saved
            redraw_masks()
            update_annotation_in_cache()
            return "break"
    root.bell()
...
def _typing(event):
    return event.widget.winfo_class() in ("Entry", "Text", "Spinbox", "TEntry", "TCombobox")


def _brush_step(delta):
    v = max(1, min(150, int(brush_size) + delta))
    _brush_slider_var.set(v); set_brush_size(v)


_SHORTCUTS = [  # (key(s), description, action)
    (("n",), "next mask", lambda e: next_mask()),
    (("p",), "previous mask", lambda e: previous_mask()),
    (("b",), "paintbrush on/off", lambda e: select_paintbrush()),
    (("e",), "eraser on/off", lambda e: select_eraser()),
    (("bracketleft",), "brush smaller  ( [ )", lambda e: _brush_step(-3)),
    (("bracketright",), "brush bigger  ( ] )", lambda e: _brush_step(3)),
    (("k",), "keypoint edit on/off", lambda e: toggle_keypoint_edit_mode()),
    (("plus", "equal"), "zoom in  ( + )", lambda e: zoom_in()),
    (("minus",), "zoom out  ( - )", lambda e: zoom_out()),
    (("1", "2", "3", "4", "5"), "show tab 1-5", None),
    (("Control-z",), "undo last brush/eraser stroke", lambda e: undo_last_stroke()),
    (("Next", "Prior"), "next / previous image in folder (PageDown / PageUp)", None),
    (("question", "F1"), "this list", None),
]


def show_shortcuts(event=None):
    lines = ["Keyboard shortcuts (not active while typing in a text box)", ""]
    lines += [f"  {' / '.join(k if len(k) > 1 or not k.isalpha() else k.upper() for k in keys):22s} {desc}"
              for keys, desc, _ in _SHORTCUTS]
    lines += ["", "Mouse: middle-drag pans, right-click on a line or keypoint for its menu."]
    messagebox.showinfo("Shortcuts", "\n".join(lines))
    return "break"


def _bind_shortcut(key, fn):
    seq = f"<{key}>" if (len(key) > 1) else f"<Key-{key}>"
    def handler(e, fn=fn):
        if _typing(e):
            return None
        fn(e)
...
for _keys, _desc, _fn in _SHORTCUTS:
    for _k in _keys:
        if _fn is not None:
            _bind_shortcut(_k, _fn)
for _i, (_key, _t, _c) in enumerate(_TABS[:5], start=1):
    _bind_shortcut(str(_i), lambda e, k=_key: show_tab(k))
_bind_shortcut("question", show_shortcuts)
_bind_shortcut("F1", show_shortcuts)


class _ToolTip:
    """Small hover help; appears after 0.6 s, disappears on leave or click."""
    def __init__(self, widget, text):
        self.w, self.text, self.tip, self.job = widget, text, None, None
        widget.bind("<Enter>", self._schedule, add="+"); widget.bind("<Leave>", self._hide, add="+")
    ...
    def _show(self):
        if self.tip or not self.w.winfo_ismapped():
            return
        ...
        tk.Label(self.tip, text=self.text, bg="#ffffe6", relief="solid", bd=1, justify="left",
                 wraplength=380, font=("Arial", 9), padx=5, pady=3).pack()
...
_TIPS = {
    "zoom_in_btn": "Zoom in (+). Middle-drag to pan.", "zoom_out_btn": "Zoom out (-).",
    "paintbrush_btn": "Paint onto the current mask (B). Ctrl+Z undoes a stroke.",
    "eraser_btn": "Erase from the current mask (E). Ctrl+Z undoes a stroke.",
    "brush_size_menu": "Preset brush sizes.", "brush_size_slider": "Brush radius in image pixels ( [ and ] ).",
    "keypoint_edit_btn": "Place, move and number landmarks (K). Right-click a landmark for its menu.",
    "keypoints_to_mask_btn": "Turn the placed keypoints into a mask outline.",
    "keypoints_to_line_btn": "Turn the placed keypoints into a smooth line annotation (e.g. a vein or seta).",
    "hide_keypoints_btn": "Hide or show the current landmark set.",
    "multi_mask_btn": "Show all masks at once instead of one at a time.",
    "bbox_button": "Draw a box around a structure for SAM2.", "remove_bbox_btn": "Remove the last box.",
    "point_prompt_btn": "Click points for SAM2 (positive = inside, negative = outside).",
    "clear_points_btn": "Remove all prompt points.",
    "apply_points_prompt_btn": "Run SAM2 with the points and boxes.", "apply_prompt_btn": "Run SAM2 with the prompts.",
    "prev_btn": "Previous mask (P).", "next_btn": "Next mask (N).", "trash_button": "Delete the current mask (cannot be undone).",
    "label_dropdown": "Choose the structure name for the current mask.", "apply_label_btn": "Give the current mask the chosen name.",
    "save_keypoints_btn": "Save the landmarks of this image now.",
    "marmot_button": "Save all annotations of this session to COCO JSON and close.",
    "load_folder_btn": "Open a folder of images to annotate one after another (PageDown / PageUp).",
    "load_prompt_btn": "Open one image to annotate with SAM2 prompts.",
    "load_btn": "Open an image and let SAM2 segment everything automatically.",
    "load_annotations_btn": "Load a COCO JSON onto the current image for checking or editing.",
    "load_labels_btn": "Load a list of structure names.", "btn_load_video": "Annotate frames of a video with SAM2.",
    "save_unlabeled_btn": "Save masks that have no name yet.",
    "btn_sam2_pal": "Copy masks from a few annotated images to a whole folder (SAM2-PAL).",
    "train_button": "Train a Detectron2 model on your annotations (uses a held-out validation set).",
    "predict_btn": "Predict masks with a trained Detectron2 model.",
    "btn_dinoland": "Copy landmarks from a few reference images to a whole folder (DINOLand).",
    "view_predictions_btn": "Open predictions to check them image by image.",
    "select_prediction_btn": "Jump to a prediction image by name.",
    "prev_image_button": "Previous prediction image.", "next_image_button": "Next prediction image.",
    "category_label_dropdown": "Structure name for the selected prediction.",
    "apply_category_label_btn": "Rename the selected prediction.", "reannotate_btn": "Edit the predicted masks.",
    "measure_button": "Lengths, widths, areas in mm (scale bar read automatically).",
    "semilandmark_button": "Outline shape: semilandmarks and Procrustes (GPA).",
    "color_segmentation_button": "Colour classes per structure.", "color_homology_button": "Colour on a grid of homologous cells.",
    "texture_phenomics_button": "Texture features per structure.", "texture_homology_button": "Texture on homologous cells.",
    "btn_delimit": "Quality checks and comparisons between species.",
    "btn_biorag": "Character matrix, key and audited species descriptions (BioRAG).",
    "btn_treatment_docx": "Formatted species treatments (Word).", "btn_zenodo": "Deposit results on Zenodo.",
    "btn_export_enhanced": "COCO with ontology terms.", "btn_export_kg": "Knowledge graph (JSON-LD).",
    "btn_convert_formats": "tpsDig, MorphoJ, StereoMorph, landmark tables and VIA 2 to Descriptron COCO, and back.",
    "btn_prepare_d2": "Combine + MinCOCO + train/validation split by specimen, ready for Detectron2.",
    "coco_to_min_btn": "Minimal COCO for Detectron2.", "combine_json_btn": "Merge several COCO files.",
    "remove_files_from_json_btn": "Drop images from a COCO file.",
    "btn_check_annotations": "Report problems (missing files, name clashes, empty or misplaced masks) and rename/merge categories.",
    "btn_folder_housekeeping": "Remove macOS ._ files, move mask PNGs out, 8-bit PNG copies of TIFFs (preview first).",
    "btn_video_track": "Follow individual specimens through a video with SAM2 and carry their pose keypoints (DeepLabCut-style CSV).",
    "btn_metadata_stats": "Import or type specimen metadata (tables, Darwin Core), set column roles, and run shape statistics that use it.",
    "btn_joints": "Joint angles from the pose skeleton, and rotating articulated parts to a common angle before GPA.",
    "btn_centerline": "Curved length, width and curvature along thin structures (veins, antennae, legs). Adds to the measurements.",
}
for _name, _text in _TIPS.items():
    ...
    if _w is not None:
        try:
            ...
            pass
for _key, _title, _c in _TABS:
    _ToolTip(_tab_buttons[_key], f"{_title.strip()}  (key {_title.strip()[0]})" if _title.strip()[0].isdigit() else _title)

# ---- guided landmarking: the status bar names the next landmark to place ----
_LANDMARK_NAMES = []


def load_landmark_names():
    p = filedialog.askopenfilename(title="Landmark names: text file (one per line) or a COCO JSON with keypoint names",
                                   filetypes=[("Text or JSON", "*.txt *.json"), ("All", "*.*")])
    if not p:
        return
    try:
        if p.lower().endswith(".json"):
            with open(p) as fh:
                cats = [c for c in json.load(fh).get("categories", []) if c.get("keypoints")]
            names = cats[0]["keypoints"] if cats else []
        else:
            names = [l.strip() for l in open(p) if l.strip()]
    except Exception as e:
        messagebox.showerror("Landmark names", str(e)); return
    if not names:
        messagebox.showerror("Landmark names", "No landmark names found."); return
    _LANDMARK_NAMES[:] = names
    messagebox.showinfo("Landmark names", f"{len(names)} landmarks loaded. While you place keypoints, the bar above "
                                          f"the buttons shows which one comes next.")


btn_landmark_names = tk.Button(scrollable_frame, text="Landmark names...", command=load_landmark_names)
_place(btn_landmark_names, _load_rows[1], 0, 7)
_ToolTip(btn_landmark_names, "Load the list of landmark names; the status bar then says which landmark to place next.")

# ---- pose skeleton: landmark names + connections ("sticks"), COCO keypoints/skeleton ----
# Landmark k is the k-th point placed (the numbering keypoint saving uses). The skeleton is stored the
# COCO way, 1-based pairs of landmark numbers, in the "keypoints" category of this session, so every
# keypoint save carries it. Drawing is added by wrapping redraw_points / pick_point (unchanged inside).
_SKELETON = {"edges": [], "names": []}


def _draw_sticks():
    canvas.delete("skeleton_stick")
    if not _SKELETON["edges"] or not point_coords:
        return
    vis = _sync_kp_vis()
    for a, b in _SKELETON["edges"]:
        if 1 <= a <= len(point_coords) and 1 <= b <= len(point_coords) and vis[a - 1] > 0 and vis[b - 1] > 0:
            (xa, ya), (xb, yb) = point_coords[a - 1], point_coords[b - 1]
            canvas.create_line(xa * zoom_level, ya * zoom_level, xb * zoom_level, yb * zoom_level,
                               fill="#00c8e0", width=2, tags="skeleton_stick")
    canvas.tag_raise("point"); canvas.tag_raise("point_number")


_redraw_points_orig, _pick_point_orig = redraw_points, pick_point


def redraw_points():                  # noqa: F811  (wraps the original)
    _redraw_points_orig()
    _draw_sticks()


def pick_point(event):                # noqa: F811
    _pick_point_orig(event)
    _draw_sticks()


def _keypoint_category():
    """The 'keypoints' category of this session (created if missing, as saving expects it)."""
    for store in (coco_output_mask, coco_output_accumulate):
        cats = store.setdefault("categories", [])
        if not any(c.get("name") == "keypoints" for c in cats):
            cid = category_name_to_id.get("keypoints") or (max([c["id"] for c in cats] + [0]) + 1)
            cats.append({"id": cid, "name": "keypoints", "supercategory": "keypoints"})
            category_name_to_id["keypoints"] = cid
    return [c for store in (coco_output_mask, coco_output_accumulate)
            for c in store["categories"] if c.get("name") == "keypoints"]


def parse_edges(text, names):
    """'1-2', '2 3', 'head-thorax' (names) -> [[1,2],[2,3],...]; unknown names raise ValueError."""
    import re
    edges = []
    for raw in re.split(r"[,;\n]+", text):
        ...
        if not raw:
            continue
        parts = [p.strip() for p in re.split(r"\s*(?:-|->|\s)\s*", raw, maxsplit=1)] if "-" in raw or " " in raw else []
        if len(parts) != 2:
            raise ValueError(f"cannot read connection {raw!r} (write e.g. 1-2 or base-tip)")
        pair = []
        for p in parts:
            if p.isdigit():
                pair.append(int(p))
            elif p in names:
                pair.append(names.index(p) + 1)
            else:
                raise ValueError(f"unknown landmark {p!r} in {raw!r}")
        edges.append(pair)
    return edges


def apply_skeleton(names, edges):
    _SKELETON["names"], _SKELETON["edges"] = list(names), [list(e) for e in edges]
    if names:
        _LANDMARK_NAMES[:] = names                    # guided landmarking follows the template
    for c in _keypoint_category():
        c["keypoints"] = list(names) if names else [str(i) for i in range(1, max([max(e) for e in edges] + [0]) + 1)]
        c["skeleton"] = [list(e) for e in edges]
    redraw_points()


def open_skeleton_dialog():
    import tkinter.scrolledtext as _st
    win = tk.Toplevel(root); win.title("Pose skeleton: landmarks and connections")
    tk.Label(win, text="Landmark names, in numbering order (one per line):").grid(row=0, column=0, sticky="w", padx=8, pady=(8, 2))
    tk.Label(win, text="Connections (one per line: 1-2  or  base-tip):").grid(row=0, column=1, sticky="w", padx=8, pady=(8, 2))
    names_box = _st.ScrolledText(win, width=32, height=16); names_box.grid(row=1, column=0, padx=8)
    edges_box = _st.ScrolledText(win, width=32, height=16); edges_box.grid(row=1, column=1, padx=8)
    names_box.insert("end", "\n".join(_SKELETON["names"] or _LANDMARK_NAMES or
                                      [str(i + 1) for i in range(len(point_coords))]))
    edges_box.insert("end", "\n".join(f"{a}-{b}" for a, b in _SKELETON["edges"]))

    def current():
        names = [l.strip() for l in names_box.get("1.0", "end").splitlines() if l.strip()]
        return names, parse_edges(edges_box.get("1.0", "end"), names)

    def chain():
        n = [l.strip() for l in names_box.get("1.0", "end").splitlines() if l.strip()]
        edges_box.delete("1.0", "end"); edges_box.insert("end", "\n".join(f"{i}-{i + 1}" for i in range(1, len(n))))

    def load():
        p = filedialog.askopenfilename(title="Skeleton: template JSON, COCO JSON, or DeepLabCut config.yaml",
                                       filetypes=[("JSON or YAML", "*.json *.yaml *.yml"), ("All", "*.*")], parent=win)
        if not p:
            return
        try:
            if p.lower().endswith((".yaml", ".yml")):
                ...
                names = list(cfg.get("bodyparts") or cfg.get("multianimalbodyparts") or [])
                edges = [[names.index(a) + 1, names.index(b) + 1] for a, b in (cfg.get("skeleton") or [])]
            else:
                d = json.load(open(p))
                if "categories" in d:
                    d = next((c for c in d["categories"] if c.get("keypoints")), {})
                names, edges = list(d.get("keypoints", [])), [list(e) for e in d.get("skeleton", [])]
        except Exception as e:
            messagebox.showerror("Skeleton", f"Could not read {p}:\n{e}", parent=win); return
        names_box.delete("1.0", "end"); names_box.insert("end", "\n".join(names))
    ...
    def save():
        try:
            ...
            messagebox.showerror("Skeleton", str(e), parent=win); return
        p = filedialog.asksaveasfilename(title="Save skeleton template", defaultextension=".json", parent=win)
        if p:
            json.dump({"keypoints": names, "skeleton": edges}, open(p, "w"), indent=1)
    ...
    def use():
        try:
            ...
            messagebox.showerror("Skeleton", str(e), parent=win); return
        bad = [e for e in edges if max(e) > len(names) or min(e) < 1] if names else []
        if bad:
            messagebox.showerror("Skeleton", f"Connections refer to landmarks that are not in the list: {bad}", parent=win); return
        apply_skeleton(names, edges)
        messagebox.showinfo("Skeleton", f"{len(names)} landmarks, {len(edges)} connections. Sticks are drawn as you place "
                                        f"points, and the skeleton is saved with the keypoints (COCO 'skeleton').", parent=win)

    fr = tk.Frame(win); fr.grid(row=2, column=0, columnspan=2, pady=8)
    for txt, cmd in (("Connect in order", chain), ("Load...", load), ("Save template...", save), ("Use", use),
                     ("Clear", lambda: apply_skeleton([], [])), ("Close", win.destroy)):
        tk.Button(fr, text=txt, command=cmd, bg="lightgreen" if txt == "Use" else None).pack(side="left", padx=3)
    tk.Label(win, fg="gray30", justify="left", wraplength=560, text=(
        "Landmark k is the k-th point you place. Connections are drawn as sticks between placed points and saved as the "
        "COCO 'skeleton' of the keypoints category, the format DeepLabCut, SLEAP and pose-estimation models read. "
        "A DeepLabCut config.yaml (bodyparts + skeleton) can be loaded directly.")).grid(row=3, column=0, columnspan=2,
                                                                                     sticky="w", padx=8, pady=(0, 8))


btn_skeleton = tk.Button(scrollable_frame, text="Pose skeleton...", command=open_skeleton_dialog)
_place(btn_skeleton, _load_rows[1], 0, 8)
_ToolTip(btn_skeleton, "Define landmark names and the connections between them; sticks are drawn while you place points.")

# ---- status bar (read-only view of the current state) ----
status_var = tk.StringVar(value="")
_status = tk.Label(scrollable_frame, textvariable=status_var, anchor="w", font=("Arial", 8), fg="#333",
                   bg="#f4f6f8", padx=6)
_status.grid(row=0, column=0, sticky="ew", padx=2, pady=(2, 1))
...
def _refresh_status():
    try:
        parts = []
        if file_path_base:
            parts.append(os.path.basename(str(file_path_base)))
        if folder_images:
            parts.append(f"folder {folder_index + 1}/{len(folder_images)}")
        if segmentation_masks:
            lab = labels.get(current_mask_index, "unlabeled")
            parts.append(f"mask {current_mask_index + 1}/{len(segmentation_masks)}: {lab}")
        if point_coords:
            parts.append(f"{len(point_coords)} keypoints")
        if _LANDMARK_NAMES and (keypoint_edit_mode or point_coords):
            n = len(point_coords) + 1
            parts.append(f"next landmark {n}/{len(_LANDMARK_NAMES)}: {_LANDMARK_NAMES[n - 1]}" if n <= len(_LANDMARK_NAMES)
                         else f"all {len(_LANDMARK_NAMES)} landmarks placed")
        mode_s = current_tool or ("keypoint edit" if keypoint_edit_mode else "")
        if mode_s:
            parts.append(f"tool: {mode_s} (size {brush_size})" if current_tool else f"tool: {mode_s}")
        parts.append(f"zoom {zoom_level:.1f}x")
        if _UNDO_STACK:
            parts.append(f"undo: {len(_UNDO_STACK)}")
        status_var.set("   |   ".join(parts) + "      press ? for shortcuts")
    except Exception:
        pass
    root.after(400, _refresh_status)


_refresh_status()

root.protocol("WM_DELETE_WINDOW", _remember_geometry_and_close)

canvas.bind("<ButtonPress-2>", start_pan)
canvas.bind("<B2-Motion>", pan_image)
# Right-click is always active: line scale-bar outside keypoint mode,
# keypoint actions inside keypoint mode (on_right_click checks the flag internally).
canvas.bind("<Button-3>", on_right_click)
