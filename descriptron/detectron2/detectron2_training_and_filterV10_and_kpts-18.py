...
###############################################################################
def _split_by_group(coco, val_fraction, group_regex, seed):
    """v18: whole groups (specimens) go to train or val, never both. Same rule as
    measure/descriptron_coco_tools.py split (kept inline so this script runs on its own)."""
    import random, re
    from collections import defaultdict
    groups = defaultdict(list)
    for im in coco["images"]:
        stem = os.path.splitext(os.path.basename(im["file_name"]))[0]
        m = re.search(group_regex, stem) if group_regex else None
        key = (m.group(1) if m.groups() else m.group(0)) if m else stem
        groups[key].append(im["id"])
    keys = sorted(groups); random.Random(seed).shuffle(keys)
    n_val = min(max(int(round(len(keys) * val_fraction)), 1), len(keys) - 1) if (val_fraction > 0 and len(keys) >= 2) else 0
    val_ids = {i for g in keys[:n_val] for i in groups[g]}
    def subset(keep):
        return {**{k: v for k, v in coco.items() if k not in ("images", "annotations")},
                "images": [im for im in coco["images"] if keep(im["id"])],
                "annotations": [a for a in coco["annotations"] if keep(a["image_id"])]}
    return subset(lambda i: i not in val_ids), subset(lambda i: i in val_ids), len(keys), n_val
...
def train_model(coco_json_path, img_dir, output_dir, total_iters, checkpoint_period,
                ...
                val_json_path=None, val_fraction=0.2, group_regex=None, split_seed=0, legacy_val_is_train=False):
    # Set up logging
    ...
    _val_source = val_json_path                       # v18: a held-out file given by the user, if any
    val_json_path = os.path.join(val_dir, 'val.json')
    # v18: v17 copied the SAME JSON to train.json and val.json, so validation AP, the EvalHook and
    # early stopping were all measured on training images. Now the validation images are held out.
    if legacy_val_is_train:
        shutil.copy(coco_json_path, train_json_path)
        shutil.copy(coco_json_path, val_json_path)
        logger.warning("--legacy-val-is-train: validation = the training images (v17 behaviour, in-sample scores)")
    elif _val_source:
        shutil.copy(coco_json_path, train_json_path)
        shutil.copy(_val_source, val_json_path)
        with open(_val_source) as _f:
            _vc = json.load(_f)
        _vn = [c["name"] for c in sorted(_vc.get("categories", []), key=lambda c: c["id"])]
        if _vn and _vn != category_names:
            logger.warning(f"validation categories differ from training categories: {_vn} vs {category_names}")
        logger.info(f"validation: {_val_source} ({len(_vc.get('images', []))} held-out images)")
    else:
        with open(coco_json_path) as _f:
            _full = json.load(_f)
        _tr, _va, _ng, _nv = _split_by_group(_full, val_fraction, group_regex, split_seed)
        if not _va["images"]:
            shutil.copy(coco_json_path, train_json_path)
            shutil.copy(coco_json_path, val_json_path)
            logger.warning("too few images/specimens to hold any out: validation = training images (in-sample)")
        else:
            with open(train_json_path, "w") as _f:
                json.dump(_tr, _f)
            with open(val_json_path, "w") as _f:
                json.dump(_va, _f)
            logger.info(f"held-out split: {_ng} {'specimens (--group-regex)' if group_regex else 'images'}, "
                        f"{_nv} to validation -> train {len(_tr['images'])} / val {len(_va['images'])} images")
            with open(os.path.join(output_dir, "split_manifest.json"), "w") as _f:
                json.dump({"val_fraction": val_fraction, "group_regex": group_regex, "seed": split_seed,
                           "train_images": [i["file_name"] for i in _tr["images"]],
                           "val_images": [i["file_name"] for i in _va["images"]]}, _f, indent=1)
...
if __name__ == "__main__":
    ...
    # v18: a real held-out validation set
    parser.add_argument('--val-json', type=str, default=None,
                        help='Held-out validation COCO JSON (e.g. from descriptron_coco_tools.py prepare-d2). '
                             'Without it, --val-fraction of the images/specimens in --coco-json are held out.')
    parser.add_argument('--val-fraction', type=float, default=0.2, help='Share held out when no --val-json (default 0.2)')
    parser.add_argument('--group-regex', type=str, default=None,
                        help='Regex whose first group is the specimen ID in the file name, so all images of one '
                             'specimen stay on the same side of the split (default: each image on its own)')
    parser.add_argument('--split-seed', type=int, default=0)
    parser.add_argument('--legacy-val-is-train', action='store_true',
                        help='v17 behaviour: validate on the training images (in-sample; for comparisons only)')
    ...
    train_model(
        ...
        val_json_path=args.val_json, val_fraction=args.val_fraction, group_regex=args.group_regex,
        split_seed=args.split_seed, legacy_val_is_train=args.legacy_val_is_train
    )
