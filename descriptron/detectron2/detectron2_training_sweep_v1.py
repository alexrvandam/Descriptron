#!/usr/bin/env python3
"""
detectron2_training_sweep_v1.py — the training script, fixed-budget for comparisons
===================================================================================
A byte-for-byte copy of detectron2_training_and_filterV10_and_kpts-17.py EXCEPT for
two lines, both of which exist only so that two backends can be compared fairly:

  1. TEST.EVAL_PERIOD 1000 -> 0.  The original evaluates every 1000 iterations on a
     copy of its own TRAINING set (train.json and val.json are the same file), so the
     numbers are in-sample and, at 20,000 iterations, twenty of them cost real time.
     With 0 the evaluation runs once at the end.
  2. The EarlyStoppingHook (patience 5 on bbox/AP50) is removed.  At 3,000 iterations
     it could never fire — only 2 evaluations happen.  At 20,000 there are 20, so it
     would stop this arm early and the comparison would measure the stopping rule
     instead of the model.

Use the ORIGINAL script for real training: early stopping is a feature there.
Use this one only inside tv_sweep_v1.py.
"""
import numpy as np
import os
import json
...
def train_model(coco_json_path, img_dir, output_dir, total_iters, checkpoint_period,
    ...
    cfg.TEST.EVAL_PERIOD = 0   # sweep copy: evaluate once at the end, not every 1000
    ...
    class AugmentedTrainer(DefaultTrainer):
        ...
        def build_hooks(self):
            ...
            # sweep copy: no early stopping — every arm must get the same budget
            # (original: hooks_list.insert(-1, EarlyStoppingHook(patience=5,
