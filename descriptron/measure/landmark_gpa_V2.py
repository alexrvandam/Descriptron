#!/usr/bin/env python
"""
landmark_gpa_V2.py — Generalized Procrustes Analysis on COCO keypoints

V2 (2026-09-25) = V1 + two fixes found by checking against R's geomorph (gpagen):
  - MIRROR IMAGES: specimens photographed from the other side (or the opposite body side) cannot be
    superimposed without reflection, so in V1 they dominated PC1 (Diaphorina forewings: 19 of 96 wings,
    PC1 95.7% -> 30.0% once reflected). V2 finds them with a reflection-allowed GPA (minority whose best fit
    is a reflection), reflects them BEFORE the GPA, lists them in <prefix>_reflected.csv and logs them.
    --no_reflect_mirrored restores V1 behaviour.
  - the aligned sample is finally rotated to the consensus' principal axes, as gpagen does (only the
    orientation of plots changes; all distances and statistics are the same).
...
def gpa(configs, max_iter=200, tol=1e-8):
    ...
    # V2: rotate the whole sample to the consensus' principal axes, as geomorph's gpagen does
    _, _, vt = np.linalg.svd(mean_shape - mean_shape.mean(axis=0))
    final_aligned = [a @ vt.T for a in final_aligned]
    mean_shape = mean_shape @ vt.T

    return np.array(final_aligned), mean_shape, np.array(sizes)


def mirrored_specimens(configs, iters=20):
    """V2: specimens that fit the consensus better when reflected. A GPA that allows reflection; the
    minority whose best rotation is a reflection (det < 0) are the mirror images."""
    Z = []
    for c in configs:
        cc = center(np.asarray(c, float))
        Z.append(cc / max(centroid_size(cc), 1e-12))
    Z = np.array(Z)
    M = Z[0].copy()
...
def main():
    ...
    for cat_name, (configs, filenames) in all_results.items():
        ...
        # V2: reflect mirror-image specimens before GPA
        if not args.no_reflect_mirrored and len(configs) >= 3:
            mir = mirrored_specimens(configs)
            pd.DataFrame({"filename": filenames, "reflected": mir.astype(int)}).to_csv(
                os.path.join(cat_dir, f"{prefix}_reflected.csv"), index=False)
            if mir.any():
                log.warning(f"{int(mir.sum())} of {len(configs)} specimens are mirror images and were reflected "
                            f"before GPA: {', '.join(f for f, m in zip(filenames, mir) if m)}")
                configs = [np.column_stack([-c[:, 0], c[:, 1]]) if m else c for c, m in zip(configs, mir)]
