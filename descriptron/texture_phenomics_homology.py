...
def _masked_glcm_single(gray_q, mask_bool, distance, angle, levels=64):
...
def _haralick(glcm, levels=64):
    if glcm.sum() == 0: return [0., 0., 0.]
    I, J = np.meshgrid(np.arange(levels), np.arange(levels), indexing='ij')
    return [float(np.sum(glcm * (I - J) ** 2)),
            float(np.sum(glcm / (1.0 + (I - J) ** 2))),
            float(np.sum(glcm ** 2))]

def extract_cell_features(gray, img_lab, cell_mask):
    """Extract texture vector from one warped grid cell.
    Only pixels where cell_mask > 0 are used. Returns list of 8 features."""
    fg = cell_mask > 0
    if fg.sum() < 25:
        return None
    features = []
    ...
    gray_q = (crop_g // 4).astype(np.uint8)
    h_vals = []
    for angle in [0, np.pi/4, np.pi/2, 3*np.pi/4]:
        g = _masked_glcm_single(gray_q, crop_m, 3, angle)
        h_vals.append(_haralick(g))
    ...
    # L*a*b* means
    for ci in range(3):
        features.append(float(img_lab[:, :, ci][fg].mean()))
...
def process_specimen(fn_gpa, specimen_contour, mean_shape, grid_info,
                     image_dir, mask_lookup, ctrl_step=5):
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            if not cell_inside[r, c]:
                cell_features.append([np.nan] * len(CELL_FEATURE_NAMES))
                continue
...
def main():
    ...
    for r in range(ci.shape[0]):
        for c in range(ci.shape[1]):
            base = (r * ci.shape[1] + c) * len(CELL_FEATURE_NAMES)
            if ci[r, c]:
                inside_indices.extend(range(base, base + len(CELL_FEATURE_NAMES)))
    ...
    labels = fcluster(Z, args.n_clusters, 'maxclust')

    fig, ax = plt.subplots(figsize=(16, 6))
    dendrogram(Z, labels=fns, leaf_rotation=90, leaf_font_size=4, ax=ax)
    ax.set_title(f"Texture dendrogram (homology-aware) — {cat_name}")
    plt.tight_layout()
    plt.savefig(os.path.join(args.output_dir,
    ...
    logging.info(f"Done: {len(results)} specimens, {n_cells} cells × "
                 f"{len(CELL_FEATURE_NAMES)} features = "
                 f"{n_cells * len(CELL_FEATURE_NAMES)} features/specimen")
