...
def parse_args():
    ...
    parser.add_argument('--extracted_json_dir', type=str, default='extracted_gbif_jsons', help='Directory to save extracted gbifData JSON files.')
...
def process_material_examined(material_examined_file, q_key, extracted_json_dir):
    """
    Processes the material examined TSV file, extracts gbifData from each JSON file based on q_key,
    saves the extracted gbifData to new JSON files, and returns a mapping of species to new JSON files.

    :param material_examined_file: Path to the input material_examined.tsv file.
    :param q_key: The key to extract from the JSON files (e.g., 'Q41').
    :param extracted_json_dir: Directory to save the extracted gbifData JSON files.
    :return: A dictionary mapping species names to lists of extracted JSON file paths.
    """
    extracted_data = []
    ...
    species_to_json = defaultdict(list)

    with open(material_examined_file, 'r', encoding='utf-8') as tsv_file:
        ...
        for row_number, row in enumerate(reader, start=2):  # Starting at 2 to account for header
            ...
            json_file_path = row.get('label_jsonl_path', '').strip()

            if not species_name or not json_file_path:
                logging.warning(f"Skipping invalid row {row_number}: {row}")
                continue  # Skip rows with empty fields

            if not os.path.isfile(json_file_path):
                logging.warning(f"File not found: '{json_file_path}'. Skipping.")
            ...
            if json_file_path not in processed_files:
                # Extract gbifData using the provided extraction logic
                extracted = extract_gbif_data(json_file_path, q_key)
                if extracted:
                    # Define the output JSON file path based on input JSON file name and q_key
                    input_basename = os.path.basename(json_file_path)
                    input_name, _ = os.path.splitext(input_basename)
                    output_json_path = os.path.join(extracted_json_dir, f"{input_name}_{q_key}_extracted.json")

                    try:
                        with open(output_json_path, 'w', encoding='utf-8') as out_json:
                            json.dump(extracted, out_json, indent=4)
                        logging.info(f"Saved extracted gbifData to '{output_json_path}'.")
                    ...
                    logging.warning(f"No gbifData extracted from file '{json_file_path}' for species '{species_name}'. Skipping.")
                    continue

            # Retrieve the output JSON file path for this species
            output_json_path = processed_files.get(json_file_path)
            if output_json_path:
                species_to_json[species_name].append(output_json_path)
                logging.debug(f"Mapped species '{species_name}' to JSON file '{output_json_path}'.")
            else:
                logging.warning(f"Failed to map species '{species_name}' due to previous errors.")

    return species_to_json

def extract_gbif_data(json_file_path, q_key):
    """
    Extracts the gbifData section from a JSON file based on the specified q_key.

    Args:
        json_file_path (str): Path to the JSON file.
        q_key (str): The key to extract (e.g., 'Q41').

    Returns:
        dict or None: The extracted gbifData dictionary, or None if not found or extraction fails.
    """
    extracted_info = {}
    try:
        ...
        print(f"Debug: Cleaned JSON String for '{os.path.basename(json_file_path)}':")
        ...
        # Step 4: Extract 'gbifData' based on q_key
        gbif_data = data.get(q_key)

        if gbif_data:
            print(f"Debug: Extracted '{q_key}' for '{os.path.basename(json_file_path)}':")
            print(json.dumps(gbif_data, indent=4))
            print("\n" + "="*50 + "\n")
            extracted_info = gbif_data
        else:
            logging.warning(f"'{q_key}' key not found in the JSON file: {json_file_path}")

    except FileNotFoundError as e:
        logging.error(f"JSON file not found: {json_file_path}\n{e}")
    except Exception as e:
        logging.error(f"Unexpected error while processing {json_file_path}: {e}")
    return extracted_info

def load_material_examined(material_examined_file, species_to_json):
    """
    Load material examined data from the species_to_json mapping.

    :param material_examined_file: Path to the original material_examined.tsv file.
    :param species_to_json: Dictionary mapping species names to lists of extracted JSON file paths.
    :return: A dictionary mapping species names to lists of extracted JSON file paths.
    """
    # Since extraction is already done, we can use the species_to_json mapping directly
    return species_to_json
    ...
    for species_name, categories in measurements_by_species_and_category.items():
        ...
        for gbif_data in material_examined_entries:
            if gbif_data:
                ...
                logging.warning(f"No 'gbifData' found in entry for species '{species_name}'")
...
def main():
    ...
    logging.info("Starting species description generation process...")

    # Step 1: Extract gbifData from material_examined.tsv based on q_key
    logging.info("Extracting gbifData from material_examined TSV...")
    species_to_json = process_material_examined(args.material_examined, args.q_key, args.extracted_json_dir)
    logging.info(f"Extraction complete. Extracted gbifData for {len(species_to_json)} species.")

    # Step 2: Load measurements, colors, QA data, and other data sources
    ...
    # Step 3: Load material examined data (species to extracted JSON files)
    logging.info("Loading material examined data...")
    material_examined = load_material_examined(args.material_examined, species_to_json)
    ...
    # Step 6: Write the prompts to the output file
    if args.output:
    ...
    logging.info("Species description generation process completed.")
