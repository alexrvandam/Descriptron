...
# -- START NEW CODE --
def fix_image_ids(input_json_path, output_json_path):
    """
    Loads a COCO-style JSON, reassigns integer IDs to each image (starting from 1),
    and updates all annotations so their 'image_id' matches the new IDs.
    """
    ...
    # Assign integer IDs to each image
    for i, img in enumerate(images):
        new_id = i + 1  # start integer IDs from 1
        old_id = img["id"]  # whatever was in "id" (e.g. file path)
        img["id"] = new_id
        new_images.append(img)
        old_id_to_new[old_id] = new_id

    # Update annotations to reference the new image IDs
    for ann in annotations:
        old_img_id = ann["image_id"]
        if old_img_id in old_id_to_new:
            ann["image_id"] = old_id_to_new[old_img_id]
        else:
            # You could remove or warn if there's no match
            logging.warning(f"Annotation {ann.get('id')} references unknown image_id: {old_img_id}")
    ...
    logging.info(f"Fixed IDs saved to {output_json_path}")
# -- END NEW CODE --
...
def main():
    ...
    parser.add_argument(
        '--reassign-image-ids',
        action='store_true',
        help='Reassign integer image IDs so that annotations match them (fixes mismatched ID references).'
    )
    # -- END NEW CODE --
    ...
    # -- START NEW CODE --
    # If reassigning image IDs, run the fix before the cleanup
    if args.reassign_image_ids:
        reassigned_json = os.path.join(output_dir, f"{base_name}_reassigned.json")
        try:
            fix_image_ids(input_path, reassigned_json)
            # Now use the reassigned JSON as the new input
            input_path = reassigned_json
        except Exception as e:
            logger.error(f"Error during reassign_image_ids: {e}")
            sys.exit(1)
    # -- END NEW CODE --
