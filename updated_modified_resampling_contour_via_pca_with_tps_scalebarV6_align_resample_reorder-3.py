...
train_json_file_path = config['Paths']['train_json_file_path']
json_input = config['Paths']['json_input']
json_output = config['Paths']['json_output']
coco_json_path = config['Paths']['coco_json_path']
...
# Function to find scale bar length in pixels using color detection
def find_scale_bar_length_in_pixels(image):
...
# Improved function to find the scale length from the extracted text
def find_scale_length(text):
...
# Main function to process the image and generate the description
def process_image(image_path):
    image = cv2.imread(image_path)
    image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    
    scale_bar_length_pixels = find_scale_bar_length_in_pixels(image_rgb)
    extracted_text = extract_text(image_rgb)
    known_scale_length_mm = find_scale_length(extracted_text)
    
    if known_scale_length_mm and scale_bar_length_pixels:
        pixels_per_mm = scale_bar_length_pixels / known_scale_length_mm
        scale_length_um = known_scale_length_mm * 1000  # Convert to micrometers
    else:
        pixels_per_mm = None
    ...
    result = {
        ...
        "scale_length_mm": known_scale_length_mm,
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith('.jpg'):
            ...
            try:
                ...
                save_annotations(base_name, result, output_folder)
            except Exception as e:
                print(f"Error occurred while processing {image_path}: {e}")

# Function to save annotations to a text file
def save_annotations(base_name, result, output_folder):
    annotation_file_path = os.path.join(output_folder, base_name + '_scalebar.txt')
    with open(annotation_file_path, 'w') as f:
        f.write(f"Scale bar length in pixels: {result['scale_bar_length_pixels']}\n")
        f.write(f"Scale length in mm: {result['scale_length_mm']}\n")
        f.write(f"Scale length in um: {result['scale_length_um']}\n")
        f.write(f"Pixels per mm: {result['pixels_per_mm']}\n")
    print(f"Annotations saved to: {annotation_file_path}")

# Function to read contour files and calculate metrics
def process_contours(contours_input_dir, output_dir):
    contour_files = [f for f in os.listdir(contours_input_dir) if fnmatch.fnmatch(f, 'binary_mask_*_contours.txt')]
    contours = []
    for filename in contour_files:
        contour_data = []
        with open(os.path.join(contours_input_dir, filename), 'r') as file:
            for line in file:
    ...
    for contour, filename in zip(contours, contour_files):
        base_name = filename.replace('binary_mask_', '').replace('_contours.txt', '')
        ...
        if os.path.exists(image_result_file):
            ...
            if contour.shape[0] > 0:
                contour = np.array(contour, dtype=np.float32)  # Ensure the contour is in the correct format
                ...
                print(f"Contour in file {filename} is empty or invalid.")
    return contours
...
# Function to save contours as TPS
def save_contours_as_tps(contours, filenames, output_file):
    with open(output_file, 'w') as f:
        for contour, filename in zip(contours, filenames):
            f.write(f"LM={1}\n")  # One landmark
            f.write(f"{contour[0][0]} {contour[0][1]}\n")  # The landmark
            f.write("OUTLINES=1\n")  # One outline
            f.write(f"POINTS={len(contour) - 1}\n")  # Number of points in the outline
            for point in contour[1:]:
                f.write(f"{point[0]} {point[1]}\n")
            f.write(f"IMAGE={filename}\n\n")

# Function to save contours as CSV
def save_contours_as_csv(contours, filenames, output_dir):
    for contour, filename in zip(contours, filenames):
        csv_filename = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}_pca_oriented_contours.csv")
        np.savetxt(csv_filename, contour, delimiter=",", fmt="%f")

# Function to save procrustes contours as CSV
def save_procrustes_contours_as_csv(contours, filenames, output_dir):
    for contour, filename in zip(contours, filenames):
        csv_filename = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}_procrustes_contours.csv")
        np.savetxt(csv_filename, contour, delimiter=",", fmt="%f")

# Function to save contours as npy
def save_contours_as_npy(contours, filenames, output_dir, suffix):
    for contour, filename in zip(contours, filenames):
        npy_filename = os.path.join(output_dir, f"{os.path.splitext(filename)[0]}_{suffix}.npy")
        np.save(npy_filename, contour)

# Function to save vectorized contours as npy
def save_vectorized_contours_as_npy(contours, output_dir, filename):
    flattened_contours = np.array([matrix.flatten() for matrix in contours])
    scaler = MinMaxScaler()
    vectorized_contours = scaler.fit_transform(flattened_contours)
    np.save(os.path.join(output_dir, filename), vectorized_contours)

# Function to resample and save contours
def resample_and_save_contours(input_folder, contours_input_folder, output_folder):
    contour_files = [f for f in os.listdir(contours_input_folder) if fnmatch.fnmatch(f, 'binary_mask_*_contours.txt')]
...
#                if j in [0, 99, 199, 299, 399, 499, 599, 699]:  # Adjust for zero-indexing
#                    plt.text(contour[j, 0], contour[j, 1], str(j + 1), fontsize=12, color='red')
#
#        # Label the second plot
#        if i == 1:
#            for j in range(len(contour)):
#                if j in [0, 99, 199, 299, 399, 499, 599, 699]:  # Adjust for zero-indexing
#                    plt.text(contour[j, 0], contour[j, 1], str(j + 1), fontsize=12, color='blue')
...
def visualize_contours(contours, title):
    ...
    for i, contour in enumerate(contours):
        ...
        # Label the points corresponding to the indices 1, 100, 200, 300, 400, 500, 600, 700
        for j in range(len(contour)):
            if j in [0, 99, 199, 299, 399, 499, 599, 699]:  # Adjust for zero-indexing
...
contours_input_folder = json_data  


# Process images and save annotations
process_folder(input_folder, output_folder)

# Resample and save contours
resample_and_save_contours(input_folder, contours_input_folder, output_folder)

# Process contours and save metrics
process_contours(output_folder, output_folder)
